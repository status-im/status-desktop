...
proc makePrivateRpcCall*(
  ...
  if DB_BLOCKED_DUE_TO_PROFILE_MIGRATION:
    debug "DB blocked due to profile migration, unable to proceed with the rpc call",
...
proc callPrivateRPCWithChainId*(
  ...
  let inputJSON =
    %*{"jsonrpc": "2.0", "method": methodName, "chainId": chainId, "params": %payload}
...
proc migrateKeyStoreDir*(
    account: string,
    hashedPassword: string,
    oldKeystoreDir: string,
    multiaccountKeystoreDir: string,
) =
  discard status_go.migrateKeyStoreDir(
    account, hashedPassword, oldKeystoreDir, multiaccountKeystoreDir
  )
