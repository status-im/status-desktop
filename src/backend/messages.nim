...
proc addReaction*(
    chatId: string, messageId: string, emojiId: int
): RpcResponse[JsonNode] =
...
proc pinUnpinMessage*(
    chatId: string, messageId: string, pin: bool
): RpcResponse[JsonNode] =
  let payload = %*[{"message_id": messageId, "pinned": pin, "chat_id": chatId}]
...
proc fetchAllMessagesFromChatWhichMatchTerm*(
    chatId: string, searchTerm: string, caseSensitive: bool
): RpcResponse[JsonNode] =
  let payload = %*[chatId, searchTerm, caseSensitive]
  result = callPrivateRPC("allMessagesFromChatWhichMatchTerm".prefix, payload)

proc fetchAllMessagesFromChatsAndCommunitiesWhichMatchTerm*(
  ...
  result =
    callPrivateRPC("allMessagesFromChatsAndCommunitiesWhichMatchTerm".prefix, payload)
...
proc markCertainMessagesFromChatWithIdAsRead*(
    chatId: string, messageIds: seq[string]
): RpcResponse[JsonNode] =
...
proc getCommunityMemberAllMessages*(
    communityId: string, memberPublicKey: string
): RpcResponse[JsonNode] =
...
proc deleteCommunityMemberMessages*(
    communityId: string, memberPubKey: string, messageId: string, chatId: string
): RpcResponse[JsonNode] =
  var messages: seq[JsonNode] = @[]
  if messageId != "" and chatId != "":
    messages.add(%*{"id": messageId, "chat_id": chatId})
