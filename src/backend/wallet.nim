...
## `res` represents a json object that contains the signature if the call was successful, or `nil`
...
proc signMessage*(res: var JsonNode, message: string, address: string, hashedPassword: string): string =
  try:
...
## `res` represents a json object that corresponds to the status go `transfer.TxResponse` type, or `nil` if the call was unsuccessful
## `chainId` is the chain id of the network
## `txArgsJSON` is the json string of the tx, corresponds to the status go `transactions.SendTxArgs` type
## returns the error message if any, or an empty string
proc buildTransaction*(res: var JsonNode, chainId: int, sendTxArgsJson: string): string =
  try:
...
## `res` represents a json object that contains the tx hash if the call was successful, or `nil`
## `chainId` is the chain id of the network
...
## returns the error message if any, or an empty string
proc sendTransactionWithSignature*(res: var JsonNode, chainId: int, txType: string, sendTxArgsJson: string,
