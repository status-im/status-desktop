...
proc signMessage*(
    resultOut: var JsonNode, message: string, address: string, hashedPassword: string
): string =
  try:
...
proc buildTransaction*(
    resultOut: var JsonNode, chainId: int, sendTxArgsJson: string
): string =
  try:
...
proc buildRawTransaction*(
    resultOut: var JsonNode, chainId: int, sendTxArgsJson: string, signature: string
): string =
  try:
...
proc sendTransactionWithSignature*(
  ...
  try:
    let response =
      sendTransactionWithSignature(chainId, txType, sendTxArgsJson, signature)
    return prepareResponse(resultOut, response)
  except Exception as e:
    warn e.msg
    return e.msg
