...
proc muteCategory*(
    communityId: string, categoryId: string, interval: int
): RpcResponse[JsonNode] =
  result = callPrivateRPC(
    "muteCommunityCategory".prefix,
    %*[{"communityId": communityId, "categoryId": categoryId, "mutedType": interval}],
  )
...
proc generateJoiningCommunityRequestsForSigning*(
    memberPubKey: string, communityId: string, addressesToReveal: seq[string]
): RpcResponse[JsonNode] =
...
proc editSharedAddresses*(
  ...
  result = callPrivateRPC(
    "editSharedAddressesForCommunity".prefix,
    %*[
      {
        "communityId": communityId,
        "addressesToReveal": addressesToShare,
...
proc getRevealedAccountsForMember*(
    communityId: string, memberPubkey: string
): RpcResponse[JsonNode] =
  result = callPrivateRPC("getRevealedAccounts".prefix, %*[communityId, memberPubkey])

proc getRevealedAccountsForAllMembers*(communityId: string): RpcResponse[JsonNode] =
  result = callPrivateRPC("getRevealedAccountsForAllMembers".prefix, %*[communityId])

proc checkPermissionsToJoinCommunity*(
    communityId: string, addresses: seq[string]
): RpcResponse[JsonNode] =
  result = callPrivateRPC(
    "checkPermissionsToJoinCommunity".prefix,
    %*[{"communityId": communityId, "addresses": addresses}],
  )
...
proc reevaluateCommunityMembersPermissions*(
  ...
  result = callPrivateRPC(
    "reevaluateCommunityMembersPermissions".prefix, %*[{"communityId": communityId}]
  )

proc checkCommunityChannelPermissions*(
    communityId: string, chatId: string
): RpcResponse[JsonNode] =
  result = callPrivateRPC(
    "checkCommunityChannelPermissions".prefix,
    %*[{"communityId": communityId, "chatId": chatId}],
  )

proc checkAllCommunityChannelsPermissions*(
    communityId: string, addresses: seq[string]
): RpcResponse[JsonNode] =
  result = callPrivateRPC(
    "checkAllCommunityChannelsPermissions".prefix,
    %*[{"communityId": communityId, "addresses": addresses}],
  )
...
proc cancelRequestToJoinCommunity*(requestId: string): RpcResponse[JsonNode] =
  result = callPrivateRPC("cancelRequestToJoinCommunity".prefix, %*[{"id": requestId}])
    ...
    %*[
      {
        ...
        "ensOnly": false,
          # TODO ensOnly is no longer supported. Remove this when we remove it in status-go
        "color": color,
        "tags": parseJson(tags),
        "image": imageUrl,
        "imageAx": aX,
...
proc requestImportDiscordCommunity*(
  ...
  result = callPrivateRPC(
    "requestImportDiscordCommunity".prefix,
    %*[
      {
        # TODO this will need to be renamed membership (small m)
        "Membership": access,
...
proc requestImportDiscordChannel*(
  ...
  result = callPrivateRPC(
    "requestImportDiscordChannel".prefix,
    %*[
      {
        "name": name,
...
proc createCommunityTokenPermission*(
    communityId: string,
    permissionType: int,
  ...
  result = callPrivateRPC(
    "createCommunityTokenPermission".prefix,
    %*[
      {
        "communityId": communityId,
        "type": permissionType,
...
proc editCommunityTokenPermission*(
    communityId: string,
    permissionId: string,
    permissionType: int,
    tokenCriteria: string,
  ...
  result = callPrivateRPC(
    "editCommunityTokenPermission".prefix,
    %*[
      {
        "communityId": communityId,
...
proc deleteCommunityTokenPermission*(
    communityId: string, permissionId: string
): RpcResponse[JsonNode] =
  result = callPrivateRPC(
    "deleteCommunityTokenPermission".prefix,
    %*[{"communityId": communityId, "permissionId": permissionId}],
  )
...
proc requestCancelDiscordChannelImport*(
  ...
  result =
    callPrivateRPC("requestCancelDiscordChannelImport".prefix, %*[discordChannelId])
...
proc reorderCommunityCategories*(
    communityId: string, categoryId: string, position: int
): RpcResponse[JsonNode] =
  result = callPrivateRPC(
    "reorderCommunityCategories".prefix,
    %*[{"communityId": communityId, "categoryId": categoryId, "position": position}],
  )

proc reorderCommunityChat*(
    communityId: string, categoryId: string, chatId: string, position: int
): RpcResponse[JsonNode] =
  result = callPrivateRPC(
    "reorderCommunityChat".prefix,
    %*[
      {
        "communityId": communityId,
...
proc deleteCommunityChat*(communityId: string, chatId: string): RpcResponse[JsonNode] =
  result = callPrivateRPC("deleteCommunityChat".prefix, %*[communityId, chatId])

proc createCommunityCategory*(
    communityId: string, name: string, channels: seq[string]
): RpcResponse[JsonNode] =
  result = callPrivateRPC(
    "createCommunityCategory".prefix,
    %*[{"communityId": communityId, "categoryName": name, "chatIds": channels}],
  )

proc editCommunityCategory*(
    communityId: string, categoryId: string, name: string, channels: seq[string]
): RpcResponse[JsonNode] =
  result = callPrivateRPC(
    "editCommunityCategory".prefix,
    %*[
      {
        "communityId": communityId,
        "categoryId": categoryId,
...
proc toggleCollapsedCommunityCategory*(
    communityId: string, categoryId: string, collapsed: bool
): RpcResponse[JsonNode] =
  result = callPrivateRPC(
    "toggleCollapsedCommunityCategory".prefix,
...
proc deleteCommunityCategory*(
  ...
  result = callPrivateRPC(
    "deleteCommunityCategory".prefix,
    %*[{"communityId": communityId, "categoryId": categoryId}],
  )

proc collectCommunityMetrics*(
    communityId: string, metricsType: int, intervals: JsonNode
): RpcResponse[JsonNode] =
  result = callPrivateRPC(
    "collectCommunityMetrics".prefix,
    %*[{"communityId": communityId, "type": metricsType, "intervals": intervals}],
  )

proc requestCommunityInfo*(
    communityId: string, tryDatabase: bool, shardCluster: int, shardIndex: int
): RpcResponse[JsonNode] =
  if shardCluster != -1 and shardIndex != -1:
    result = callPrivateRPC(
      "fetchCommunity".prefix,
      %*[
        {
          "communityKey": communityId,
          "tryDatabase": tryDatabase,
          "shard": {"shardCluster": shardCluster, "shardIndex": shardIndex},
...
proc removeUserFromCommunity*(
    communityId: string, pubKey: string
): RpcResponse[JsonNode] =
...
proc acceptRequestToJoinCommunity*(requestId: string): RpcResponse[JsonNode] =
  return callPrivateRPC("acceptRequestToJoinCommunity".prefix, %*[{"id": requestId}])

proc declineRequestToJoinCommunity*(requestId: string): RpcResponse[JsonNode] =
  return callPrivateRPC("declineRequestToJoinCommunity".prefix, %*[{"id": requestId}])

proc banUserFromCommunity*(
    communityId: string, pubKey: string, deleteAllMessages: bool
): RpcResponse[JsonNode] =
  return callPrivateRPC(
    "banUserFromCommunity".prefix,
    %*[
      {
        "communityId": communityId,
        "user": pubKey,
...
proc unbanUserFromCommunity*(
    communityId: string, pubKey: string
): RpcResponse[JsonNode] =
  return callPrivateRPC(
    "unbanUserFromCommunity".prefix, %*[{"communityId": communityId, "user": pubKey}]
  )

proc setCommunityMuted*(communityId: string, mutedType: int): RpcResponse[JsonNode] =
  return callPrivateRPC(
    "setCommunityMuted".prefix, %*[{"communityId": communityId, "mutedType": mutedType}]
  )

proc shareCommunityToUsers*(
    communityId: string, pubKeys: seq[string], inviteMessage: string
): RpcResponse[JsonNode] =
  return callPrivateRPC(
    "shareCommunity".prefix,
    %*[{"communityId": communityId, "users": pubKeys, "inviteMessage": inviteMessage}],
  )
...
proc shareCommunityChannelUrlWithChatKey*(
  ...
  return callPrivateRPC(
    "shareCommunityChannelURLWithChatKey".prefix,
...
proc shareCommunityChannelUrlWithData*(
  ...
  return callPrivateRPC(
    "shareCommunityChannelURLWithData".prefix,
    %*[{"communityId": communityId, "channelId": channelId}],
  )
...
proc requestExtractDiscordChannelsAndCategories*(
  ...
  return callPrivateRPC(
    "requestExtractDiscordChannelsAndCategories".prefix, %*[filesToImport]
  )
...
proc getCommunityPublicKeyFromPrivateKey*(
  ...
  return callPrivateRPC(
    "getCommunityPublicKeyFromPrivateKey".prefix, %*[communityPrivateKey]
  )

proc getCommunityMembersForWalletAddresses*(
    communityId: string, chainId: int
): RpcResponse[JsonNode] =
  return callPrivateRPC(
    "getCommunityMembersForWalletAddresses".prefix, %*[communityId, chainId]
  )
...
proc setCommunityShard*(communityId: string, index: int): RpcResponse[JsonNode] =
  if index != -1:
    result = callPrivateRPC(
      "setCommunityShard".prefix,
      %*[
        {
          "communityId": communityId,
          "shard": {"cluster": MAIN_STATUS_SHARD_CLUSTER_ID, "index": index},
        }
      ],
    )
  else: # unset community shard
    result =
      callPrivateRPC("setCommunityShard".prefix, %*[{"communityId": communityId}])
