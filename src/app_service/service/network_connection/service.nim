...
QtObject:
  ...
  proc updateMultichainStatus(
    self: Service,
    website: string,
    completelyDown: bool,
    chaindIdsDown: seq[int],
    at: int,
  )

  proc getChainIdsDown(
    self: Service, chainStatusTable: ConnectionStatusNotification
  ): (bool, bool, seq[int], int)
  ...
  proc newService*(
    ...
    result.connectionStatus = {
      BLOCKCHAINS: newConnectionStatus(),
      MARKET: newConnectionStatus(),
      COLLECTIBLES: newConnectionStatus(),
    }.toTable
  ...
  proc init*(self: Service) =
    self.events.on(SignalType.Wallet.event) do(e: Args):
      ...
      of "wallet-blockchain-status-changed":
        if self.nodeService.isConnected():
          ...
          let (allKnown, allDown, chainsDown, at) =
            self.getChainIdsDown(chainStateTable)
      ...
      of "wallet-collectible-status-changed":
        if self.nodeService.isConnected():
          let chainStateTable =
            fromJson(parseJson(data.message), ConnectionStatusNotification)
          let (allKnown, allDown, chainsDown, at) =
            self.getChainIdsDown(chainStateTable)
          if allKnown:
    ...
    self.events.on(SIGNAL_WALLET_ACCOUNT_TOKENS_REBUILT) do(e: Args):
      if self.connectionStatus.hasKey(MARKET):
        ...
        self.updateSimpleStatus(
          MARKET, connectionStatus.completelyDown, connectionStatus.lastCheckedAt
        )

      if self.connectionStatus.hasKey(BLOCKCHAINS):
        let connectionStatus = self.connectionStatus[BLOCKCHAINS]
        self.updateMultichainStatus(
          BLOCKCHAINS, connectionStatus.completelyDown, connectionStatus.chainIds,
          connectionStatus.lastCheckedAt,
        )
  ...
  proc getChainStatusTable(message: string): ConnectionStatusNotification =
    ...
    if chainStatusTable.kind != JNull:
      for k, v in chainStatusTable.pairs:
        result[k] =
          connection_status_backend.initConnectionState(value = getStateValue(v.getStr))
  ...
  proc getChainIdsDown(
    ...
    var lastSuccessAt: int = connection_status_backend.INVALID_TIMESTAMP
      # latest succesful connectinon between the down chains

    let allChainIds = self.networkService.getCurrentNetworksChainIds()
    for id in allChainIds:
      if chainStatusTable.hasKey($id) and
          chainStatusTable[$id].value != connection_status_backend.StateValue.Unknown:
  ...
  proc convertConnectionStatusToNetworkConnectionsArgs(
      self: Service, website: string, connectionStatus: ConnectionStatus
  ): NetworkConnectionsArgs =
    ...
    if self.connectionStatus.hasKey(website):
      if isDown:
        ...
        self.events.emit(
          ...
          self.convertConnectionStatusToNetworkConnectionsArgs(
            website, self.connectionStatus[website]
          ),
        )
      else:
    ...
    if self.connectionStatus.hasKey(website):
      ...
      if completelyDown:
        self.updateConnectionStatus(
          website, ConnectionState.Failed, true, chaindIdsDown, at
        )
        self.events.emit(
          SIGNAL_CONNECTION_UPDATE,
          self.convertConnectionStatusToNetworkConnectionsArgs(
      ...
      # if all the networks are not down for the website
        ...
        if self.connectionStatus[website].completelyDown or
            (
              chaindIdsDown.len == 0 and self.connectionStatus[website].chainIds.len != 0
            ):
        ...
        if chaindIdsDown.len > 0:
          self.updateConnectionStatus(
            website, ConnectionState.Failed, false, chaindIdsDown, at
          )
          self.events.emit(
            SIGNAL_CONNECTION_UPDATE,
            self.convertConnectionStatusToNetworkConnectionsArgs(
  ...
  proc blockchainsRetry*(self: Service) {.slot.} =
    if (self.connectionStatus.hasKey(BLOCKCHAINS)):
      ...
      self.events.emit(
        SIGNAL_CONNECTION_UPDATE,
        self.convertConnectionStatusToNetworkConnectionsArgs(
          BLOCKCHAINS, self.connectionStatus[BLOCKCHAINS]
        ),
      )
  ...
  proc marketRetry*(self: Service) {.slot.} =
    if (self.connectionStatus.hasKey(MARKET)):
      ...
      self.events.emit(
        SIGNAL_CONNECTION_UPDATE,
        self.convertConnectionStatusToNetworkConnectionsArgs(
          MARKET, self.connectionStatus[MARKET]
        ),
      )
  ...
  proc collectiblesRetry*(self: Service) {.slot.} =
    if (self.connectionStatus.hasKey(COLLECTIBLES)):
      ...
      self.events.emit(
        SIGNAL_CONNECTION_UPDATE,
        self.convertConnectionStatusToNetworkConnectionsArgs(
          COLLECTIBLES, self.connectionStatus[COLLECTIBLES]
        ),
      )
  ...
  proc checkIfConnected*(self: Service, website: string): bool =
    if self.connectionStatus.hasKey(website) and
        self.connectionStatus[website].completelyDown:
      return false
    return true
