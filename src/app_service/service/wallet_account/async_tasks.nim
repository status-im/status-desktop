...
proc prepareTokensTask(argEncoded: string) {.gcsafe, nimcall.} =
  ...
  try:
    let response = backend.fetchOrGetCachedWalletBalances(arg.accounts, true) # TODO: think should we need to use arg.storeResult or not and if yes, is it everywhere set proprely
