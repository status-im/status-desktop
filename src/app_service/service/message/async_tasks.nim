...
proc getCountAndCountWithMentionsFromResponse(
    chatId: string, seenAndUnseenMessagesBatch: JsonNode
): (int, int) =
...
# Async load messages
#################################################
type AsyncFetchChatMessagesTaskArg = ref object of QObjectTaskArg
  chatId: string
  msgCursor: string
  limit: int
...
proc asyncFetchChatMessagesTask(argEncoded: string) {.gcsafe, nimcall.} =
  ...
  try:
    var responseJson = %*{"chatId": arg.chatId}
    ...
    arg.finish(%*{"chatId": arg.chatId, "error": e.msg})
...
proc asyncFetchPinnedChatMessagesTask(argEncoded: string) {.gcsafe, nimcall.} =
  ...
  try:
    ...
    let pinnedMsgsResponse =
      status_go.fetchPinnedMessages(arg.chatId, arg.msgCursor, arg.limit)
...
type AsyncSearchMessagesTaskArg = ref object of QObjectTaskArg
  searchTerm: string
  caseSensitive: bool
...
#################################################
type AsyncSearchMessagesInChatTaskArg = ref object of AsyncSearchMessagesTaskArg
  chatId: string
...
proc asyncSearchMessagesInChatTask(argEncoded: string) {.gcsafe, nimcall.} =
  ...
  try:
    let response = status_go.fetchAllMessagesFromChatWhichMatchTerm(
      arg.chatId, arg.searchTerm, arg.caseSensitive
    )

    let responseJson =
      %*{"chatId": arg.chatId, "messages": response.result, "error": response.error}
    arg.finish(responseJson)
  except Exception as e:
    arg.finish(%*{"chatId": arg.chatId, "error": e.msg})
...
#################################################
type AsyncSearchMessagesInChatsAndCommunitiesTaskArg = ref object of AsyncSearchMessagesTaskArg
  communityIds: seq[string]
  chatIds: seq[string]

proc asyncSearchMessagesInChatsAndCommunitiesTask(
    argEncoded: string
) {.gcsafe, nimcall.} =
  let arg = decode[AsyncSearchMessagesInChatsAndCommunitiesTaskArg](argEncoded)

  try:
    let rpcResponse = status_go.fetchAllMessagesFromChatsAndCommunitiesWhichMatchTerm(
      arg.communityIds, arg.chatIds, arg.searchTerm, arg.caseSensitive
    )
    arg.finish(
      %*{
    ...
    arg.finish(
      %*{"communityIds": arg.communityIds, "chatIds": arg.chatIds, "error": e.msg}
    )

#################################################
# Async mark all messages read
#################################################
type AsyncMarkAllMessagesReadTaskArg = ref object of QObjectTaskArg
  chatId: string
...
proc asyncMarkAllMessagesReadTask(argEncoded: string) {.gcsafe, nimcall.} =
  ...
  try:
    ...
    discard rpcResponse.result.getProp(
...
#################################################
type AsyncMarkCertainMessagesReadTaskArg = ref object of QObjectTaskArg
  chatId: string
...
proc asyncMarkCertainMessagesReadTask(argEncoded: string) {.gcsafe, nimcall.} =
  ...
  try:
    let rpcResponse =
      status_go.markCertainMessagesFromChatWithIdAsRead(arg.chatId, arg.messagesIds)
    ...
    discard
      rpcResponse.result.getProp("seenAndUnseenMessages", seenAndUnseenMessagesBatch)
    let (count, countWithMentions) =
      getCountAndCountWithMentionsFromResponse(arg.chatId, seenAndUnseenMessagesBatch)
    ...
    arg.finish(%*{"chatId": arg.chatId, "messagesIds": arg.messagesIds, "error": e.msg})

#################################################
# Async get first unseen message id
#################################################
type AsyncGetFirstUnseenMessageIdForTaskArg = ref object of QObjectTaskArg
  chatId: string
...
proc asyncGetFirstUnseenMessageIdForTaskArg(argEncoded: string) {.gcsafe, nimcall.} =
  ...
  let responseJson = %*{"messageId": "", "chatId": arg.chatId, "error": ""}

  try:
    let response = status_go.firstUnseenMessageID(arg.chatId)

    if (not response.error.isNil):
      error "error getFirstUnseenMessageIdFor: ",
    ...
    error "error: ",
      procName = "getFirstUnseenMessageIdFor",
      errName = e.name,
      errDesription = e.msg,
      chatId = arg.chatId
...
type AsyncGetTextURLsToUnfurlTaskArg = ref object of QObjectTaskArg
  text*: string
  requestUuid*: string
...
proc asyncGetTextURLsToUnfurlTask(argEncoded: string) {.gcsafe, nimcall.} =
  ...
  var output = %*{"error": "", "response": "", "requestUuid": arg.requestUuid}
  try:
...
type AsyncUnfurlUrlsTaskArg = ref object of QObjectTaskArg
  urls*: seq[string]
  requestUuid*: string
...
type AsyncGetMessageByMessageIdTaskArg = ref object of QObjectTaskArg
  requestId*: string
  messageId*: string
...
type AsyncMarkMessageAsUnreadTaskArg = ref object of QObjectTaskArg
  messageId*: string
  chatId*: string
...
proc asyncMarkMessageAsUnreadTask(argEncoded: string) {.gcsafe, nimcall.} =
  ...
  try:
    ...
    var activityCenterNotifications: JsonNode = newJObject()
    discard response.result.getProp(
      "activityCenterNotifications", activityCenterNotifications
    )
...
#################################################
type AsyncLoadCommunityMemberAllMessagesTaskArg = ref object of QObjectTaskArg
  communityId*: string
  memberPubKey*: string
...
proc asyncLoadCommunityMemberAllMessagesTask(argEncoded: string) {.gcsafe, nimcall.} =
  ...
  var responseJson = %*{"communityId": arg.communityId, "error": ""}

  try:
    let response =
      status_go.getCommunityMemberAllMessages(arg.communityId, arg.memberPubKey)
    ...
    error "error: ",
      procName = "asyncLoadCommunityMemberAllMessagesTask",
      errName = e.name,
      errDesription = e.msg,
