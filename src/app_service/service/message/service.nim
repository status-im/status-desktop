...
QtObject:
  ...
  proc initOrGetMessageCursor(self: Service, chatId: string): MessageCursor =
    if (not self.msgCursor.hasKey(chatId)):
      self.msgCursor[chatId] =
        initMessageCursor(value = "", pending = false, mostRecent = false)
  ...
  proc asyncLoadMoreMessagesForChat*(
      self: Service, chatId: string, limit = MESSAGES_PER_PAGE
  ): bool =
    if (chatId.len == 0):
  ...
  proc asyncLoadInitialMessagesForChat*(self: Service, chatId: string) =
    if self.isChatCursorInitialized(chatId):
      let data = MessagesLoadedArgs(chatId: chatId, messages: @[], reactions: @[])
  ...
  proc asyncLoadCommunityMemberAllMessages*(
      self: Service, communityId: string, memberPublicKey: string
  ) =
  ...
  proc handleMessagesUpdate(
      self: Service, chats: var seq[ChatDto], messages: var seq[MessageDto]
  ) =
  ...
  proc handlePinnedMessagesUpdate(
    ...
    for pm in pinnedMessages:
      ...
      let data = MessagePinUnpinArgs(
        chatId: chatId, messageId: pm.messageId, actionInitiatedBy: pm.pinnedBy
      )
      if (pm.pinned):
        if (chatId != "" and pm.pinnedBy != singletonInstance.userProfile.getPubKey()):
          self.numOfPinnedMessagesPerChat[chatId] =
            self.getNumOfPinnedMessages(chatId) + 1
  ...
  proc handleRemovedMessagesUpdate(
      self: Service, removedMessages: seq[RemovedMessageDto]
  ) =
    for rm in removedMessages:
      let data = MessageRemovedArgs(
        chatId: rm.chatId, messageId: rm.messageId, deletedBy: rm.deletedBy
      )
      self.events.emit(SIGNAL_MESSAGE_REMOVED, data)

  proc handleDeletedMessagesUpdate(
      self: Service, deletedMessages: Table[string, seq[string]], communityId: string
  ) =
    let data =
      MessagesDeletedArgs(deletedMessages: deletedMessages, communityId: communityId)
  ...
  proc handleEmojiReactionsUpdate(self: Service, emojiReactions: seq[ReactionDto]) =
    for r in emojiReactions:
      let data = MessageAddRemoveReactionArgs(
        chatId: r.localChatId,
  ...
  proc handleMessagesReload(self: Service, communityId: string) =
    ...
    self.events.emit(
      SIGNAL_RELOAD_MESSAGES, ReloadMessagesArgs(communityId: communityId)
    )

  proc init*(self: Service) =
    self.events.on(SignalType.MessageDelivered.event) do(e: Args):
      let receivedData = MessageDeliveredSignal(e)
      let data = MessageDeliveredArgs(
        chatId: receivedData.chatId, messageId: receivedData.messageId
      )
    ...
    self.events.on(SIGNAL_APPEND_CHAT_MESSAGES) do(e: Args):
      ...
      if args.messages != nil and args.messages.kind != JNull:
        ...
        messages = map(
          args.messages.getElems(),
          proc(x: JsonNode): MessageDto =
            x.toMessageDto(),
        )
        ...
        self.events.emit(
          SIGNAL_MESSAGES_LOADED,
          MessagesLoadedArgs(chatId: args.chatId, messages: messages, reactions: @[]),
        )
  ...
  proc getTransactionDetails*(self: Service, message: MessageDto): (string, string) =
    ...
    if message.transactionParameters.contract != "":
      for chainId in chainIds:
        let tokenFound = self.tokenService.findTokenByAddress(
          chainId, message.transactionParameters.contract
        )
        if tokenFound == nil:
          continue
    ...
    var weiStr =
      service_conversion.wei2Eth(message.transactionParameters.value, token.decimals)
    weiStr.trimZeros()
  ...
  proc onAsyncLoadPinnedMessagesForChat*(self: Service, response: string) {.slot.} =
    try:
      ...
      if responseObj.kind != JObject:
        raise newException(
          CatchableError, "load pinned messages response is not a json object"
        )
      ...
      if (responseObj.getProp("pinnedMessages", pinnedMsgArr)):
        pinnedMessages = map(
          pinnedMsgArr.getElems(),
          proc(x: JsonNode): PinnedMessageDto =
            x.toPinnedMessageDto(),
        )
      ...
      let data =
        PinnedMessagesLoadedArgs(chatId: chatId, pinnedMessages: pinnedMessages)
  ...
  proc onAsyncLoadMoreMessagesForChat*(self: Service, response: string) {.slot.} =
    try:
      ...
      if (responseObj.kind != JObject):
        raise newException(
          CatchableError, "load more messages response is not a json object"
        )
  ...
  proc onAsyncLoadCommunityMemberAllMessages*(
    ...
    try:
      ...
      var messages = map(
        rpcResponseObj{"messages"}.getElems(),
        proc(x: JsonNode): MessageDto =
          x.toMessageDto(),
      )
      if messages.len > 0:
      ...
      let data =
        CommunityMemberMessagesArgs(communityId: communityId, messages: messages)
      self.events.emit(SIGNAL_COMMUNITY_MEMBER_ALL_MESSAGES, data)
    except Exception as e:
      error "error: ",
        procName = "onAsyncLoadCommunityMemberAllMessages",
        errName = e.name,
        errDesription = e.msg
  ...
  proc addReaction*(self: Service, chatId: string, messageId: string, emojiId: int) =
    try:
      ...
      let data = MessageAddRemoveReactionArgs(
        chatId: chatId, messageId: messageId, emojiId: emojiId, reactionId: reactionId
      )
  ...
  proc removeReaction*(
      self: Service, reactionId: string, chatId: string, messageId: string, emojiId: int
  ) =
    try:
      ...
      error "error: ",
        procName = "removeReaction", errName = e.name, errDesription = e.msg
    ...
    try:
      ...
      if response.result.getProp("pinMessages", pinMessagesObj):
        ...
        if pinnedMessagesArr.len > 0: # an array is returned
          ...
          let data = MessagePinUnpinArgs(
            chatId: chatId, messageId: messageId, actionInitiatedBy: doneBy
          )
          var pinned = false
      ...
      error "error: ",
        procName = "pinUnpinMessage", errName = e.name, errDesription = e.msg
  ...
  proc getMessageByMessageId*(self: Service, messageId: string): GetMessageResult =
    try:
      ...
      if not msgResponse.error.isNil:
        ...
        raise
          newException(RpcException, "Error resending chat message: " & error.message)
      ...
      result.error = e.msg
      error "error: ",
        procName = "getMessageByMessageId", errName = e.name, errDesription = e.msg
  ...
  proc onAsyncGetMessageById*(self: Service, response: string) {.slot.} =
    try:
      ...
      if responseObj.kind != JObject:
        raise
          newException(RpcException, "getMessageById response is not an json object")
      ...
      error "response processing failed",
        procName = "asyncGetMessageByMessageId", errName = e.name, errDesription = e.msg
  ...
  proc onAsyncSearchMessages*(self: Service, response: string) {.slot.} =
    ...
    try:
      ...
      if responseObj.kind != JObject:
        raise
          newException(CatchableError, "search messages response is not an json object")
      ...
      if (not responseObj.getProp("messages", messagesObj)):
        raise newException(
          CatchableError, "search messages response doesn't contain messages property"
        )

      var messagesArray: JsonNode
      if (not messagesObj.getProp("messages", messagesArray)):
        raise newException(
          CatchableError, "search messages response doesn't contain messages array"
        )

      if (messagesArray.kind notin {JArray, JNull}):
        raise newException(
          CatchableError,
          "expected messages json array is neither of JArray nor JNull type",
        )

      var messages = map(
        messagesArray.getElems(),
        proc(x: JsonNode): MessageDto =
          x.toMessageDto(),
      )
  ...
  proc asyncSearchMessages*(
      self: Service, chatId: string, searchTerm: string, caseSensitive: bool
  ) =
    ## Asynchronous search for messages which contain the searchTerm and belong to the chat with chatId.
    if (chatId.len == 0):
      error "error: empty channel id set for fetching more messages",
        procName = "asyncSearchMessages"
      return
  ...
  proc asyncSearchMessages*(
    ...
    if (communityIds.len == 0 and chatIds.len == 0):
      error "either community ids or chat ids or both must be set",
        procName = "asyncSearchMessages"
      return
  ...
  proc onMarkAllMessagesRead*(self: Service, response: string) {.slot.} =
    try:
      ...
      if (responseObj.kind != JObject):
        raise newException(
          CatchableError, "mark all messages read response is not an json object"
        )
      ...
      checkAndEmitACNotificationsFromResponse(
        self.events, responseObj{"activityCenterNotifications"}
      )
    except Exception as e:
  ...
  proc onAsyncMarkMessageAsUnread*(self: Service, response: string) {.slot.} =
    try:
      ...
      if responseObj.kind != JObject:
        raise newException(
          RpcException, "markMessageAsUnread response is not a json object"
        )
      ...
      error "error: ",
        procName = "markMessageAsUnread", errName = e.name, errDesription = e.msg
  ...
  proc onMarkCertainMessagesRead*(self: Service, response: string) {.slot.} =
    try:
      ...
      if count < len(messagesIds):
        warn "warning: ",
          procName = "onMarkCertainMessagesRead",
          errDescription = "not all messages has been marked as read"
  ...
  proc markCertainMessagesRead*(
      self: Service, chatId: string, messagesIds: seq[string]
  ) =
    if (chatId.len == 0):
  ...
  proc onGetFirstUnseenMessageIdFor*(self: Service, response: string) {.slot.} =
    try:
      ...
      self.events.emit(
        SIGNAL_FIRST_UNSEEN_MESSAGE_LOADED,
        FirstUnseenMessageLoadedArgs(chatId: chatId, messageId: messageId),
      )
    except Exception as e:
      error "error: ",
        procName = "onGetFirstUnseenMessageIdFor",
        errName = e.name,
        errDesription = e.msg
  ...
  proc onAsyncGetTextURLsToUnfurl*(self: Service, responseString: string) {.slot.} =
    ...
    if response.kind != JObject:
      warn "expected response is not a json object",
        methodName = "onAsyncGetTextURLsToUnfurl"
      return
  ...
  proc onAsyncUnfurlUrlsFinished*(self: Service, response: string) {.slot.} =
    ...
    if responseObj.kind != JObject:
      warn "expected response is not a json object",
        methodName = "onAsyncUnfurlUrlsFinished"
      return
    ...
    let args = LinkPreviewDataArgs(
      linkPreviews: linkPreviews, requestUuid: responseObj["requestUuid"].getStr
    )
...
proc renderInline(
    self: Service, parsedText: ParsedText, communityChats: seq[ChatDto]
): string =
  ...
  of PARSED_TEXT_CHILD_TYPE_MENTION:
    ...
    if isSystemMention(id):
      ...
      result = fmt(
        "<a href=\"//{id}\" class=\"mention\">@{contactDto.userDefaultDisplayName()}</a>"
      )
...
proc getRenderedText*(
    self: Service, parsedTextArray: seq[ParsedText], communityChats: seq[ChatDto]
): string =
  for parsedText in parsedTextArray:
    ...
    of PARSED_TEXT_TYPE_BLOCKQUOTE:
      result =
        result & "<blockquote>" & escape_html(parsedText.literal) & "</blockquote>"
...
proc deleteMessage*(self: Service, messageId: string) =
  try:
    ...
    if (
      not response.result.getProp("removedMessages", removesMessagesObj) or
      removesMessagesObj.kind != JArray
    ):
      error "error: ",
        procName = "removeMessage",
        errDesription = "no messages remove or it's not an array"
      return

    let removedMessagesArr = removesMessagesObj.getElems()
    if (removedMessagesArr.len == 0): # an array is returned
      error "error: ",
        procName = "removeMessage", errDesription = "array has no message to remove"
      return

    let removedMessageObj = removedMessagesArr[0]
    var chat_Id, message_Id: string
    if not removedMessageObj.getProp("chatId", chat_Id) or
        not removedMessageObj.getProp("messageId", message_Id):
      error "error: ",
        procName = "removeMessage",
        errDesription = "there is no set chat id or message id in response"
      return
...
proc replacePubKeysWithDisplayNames*(self: Service, message: string): string =
  let allKnownContacts =
    self.contactService.getContactsByGroup(ContactsGroup.AllKnownContacts)
...
proc bulkReplacePubKeysWithDisplayNames(self: Service, messages: var seq[MessageDto]) =
  ...
  for i in 0 ..< messages.len:
    messages[i].text =
      message_common.replacePubKeysWithDisplayNames(allKnownContacts, messages[i].text)
...
proc editMessage*(self: Service, messageId: string, msg: string) =
  try:
    ...
    if (messages.len == 0):
      error "error: ",
        procName = "editMessage", errDesription = "messages array is empty"
      return
...
proc resendChatMessage*(self: Service, messageId: string): string =
  try:
    ...
    error "error: ",
      procName = "resendChatMessage", errName = e.name, errDesription = e.msg
...
proc deleteCommunityMemberMessages*(
  ...
  try:
    let response = status_go.deleteCommunityMemberMessages(
      communityId, memberPubKey, messageId, chatId
    )
    ...
    error "error: ",
      procName = "deleteCommunityMemberMessages",
      errName = e.name,
      errDesription = e.msg
