...
type DeployState* {.pure.} = enum
  Failed
  InProgress
  Deployed

type CommunityTokenDto* = object
  tokenType*: TokenType
  communityId*: string
  address*: string
  name*: string
...
type CommunityTokenAndAmount* = object
  communityToken*: CommunityTokenDto
  amount*: Uint256 # for assets the value is converted to wei

type ContractTuple* = tuple[chainId: int, address: string]

proc `%`*(self: ContractTuple): JsonNode =
  result = %*{"address": self.address, "chainId": self.chainId}

proc toContractTuple*(json: JsonNode): ContractTuple =
  return (json["chainId"].getInt, json["address"].getStr)

type ChainWalletTuple* = tuple[chainId: int, address: string]

type WalletAndAmount* = object
  walletAddress*: string
  amount*: int

type RemoteDestroyTransactionDetails* = object
  chainId*: int
...
type OwnerTokenDeploymentTransactionDetails* = object
...
proc toOwnerTokenDeploymentTransactionDetails*(
    jsonObj: JsonNode
): OwnerTokenDeploymentTransactionDetails =
  result = OwnerTokenDeploymentTransactionDetails()
  try:
    result.ownerToken =
      (jsonObj["ownerToken"]["chainId"].getInt, jsonObj["ownerToken"]["address"].getStr)
    result.masterToken = (
      jsonObj["masterToken"]["chainId"].getInt, jsonObj["masterToken"]["address"].getStr
    )
...
proc toRemoteDestroyTransactionDetails*(
    json: JsonNode
): RemoteDestroyTransactionDetails =
  return RemoteDestroyTransactionDetails(
    chainId: json["chainId"].getInt,
    contractAddress: json["contractAddress"].getStr,
    addresses: to(json["addresses"], seq[string]),
  )

type ComputeFeeErrorCode* {.pure.} = enum
  Success
  Infura
  Balance
  Revert
