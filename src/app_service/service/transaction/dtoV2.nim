...
type SuggestedLevelsForMaxFeesPerGasDto* = ref object
  low*: UInt256
  medium*: UInt256
  high*: UInt256

type TransactionPathDtoV2* = ref object
...
proc toSuggestedLevelsForMaxFeesPerGasDto*(
    jsonObj: JsonNode
): SuggestedLevelsForMaxFeesPerGasDto =
...
proc toTransactionPathDtoV2*(jsonObj: JsonNode): TransactionPathDtoV2 =
  ...
  result.fromChain =
    Json.decode($jsonObj["FromChain"], NetworkDto, allowUnknownFields = true)
  result.toChain =
    Json.decode($jsonObj["ToChain"], NetworkDto, allowUnknownFields = true)
  result.fromToken =
    Json.decode($jsonObj["FromToken"], TokenDto, allowUnknownFields = true)
  ...
  result.suggestedLevelsForMaxFeesPerGas =
    jsonObj["SuggestedLevelsForMaxFeesPerGas"].toSuggestedLevelsForMaxFeesPerGasDto()
  ...
  result.approvalAmountRequired =
    stint.fromHex(UInt256, jsonObj{"ApprovalAmountRequired"}.getStr)
  ...
  result.approvalMaxFeesPerGas =
    stint.fromHex(UInt256, jsonObj{"ApprovalMaxFeesPerGas"}.getStr)
  result.approvalBaseFee = stint.fromHex(UInt256, jsonObj{"ApprovalBaseFee"}.getStr)
  result.approvalPriorityFee =
    stint.fromHex(UInt256, jsonObj{"ApprovalPriorityFee"}.getStr)
