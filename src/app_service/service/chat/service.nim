...
type
  ...
  ChatArgs* = ref object of Args
    communityId*: string
      # This param should be renamed to `sectionId`, that will avoid some confusions one may have.
    chatId*: string
...
const SIGNAL_CHECK_ALL_CHANNELS_PERMISSIONS_RESPONSE* =
  "checkAllChannelsPermissionsResponse"
const SIGNAL_CHECK_ALL_CHANNELS_PERMISSIONS_FAILED* =
...
QtObject:
  ...
  proc processMessengerResponse*(
    self: Service, response: RpcResponse[JsonNode]
  ): (seq[ChatDto], seq[MessageDto])
    ...
    self.events.on(SignalType.Message.event) do(e: Args):
      ...
      if (receivedData.chats.len > 0):
        ...
        for chatDto in receivedData.chats:
          if (chatDto.active):
            ...
            if not isCommunityChat and self.chats.hasKey(chatDto.id) and
                self.chats[chatDto.id].members != chatDto.members:
              self.events.emit(
                SIGNAL_CHAT_MEMBERS_CHANGED,
                ChatMembersChangedArgs(chatId: chatDto.id, members: chatDto.members),
              )
      ...
      if (receivedData.clearedHistories.len > 0):
        for clearedHistoryDto in receivedData.clearedHistories:
          self.events.emit(
            SIGNAL_CHAT_HISTORY_CLEARED, ChatArgs(chatId: clearedHistoryDto.chatId)
          )
  ...
  proc sectionUnreadMessagesAndMentionsCount*(
      self: Service, sectionId: string, sectionIsMuted: bool
    ...
    for _, chat in self.chats:
      ...
      if chat.unviewedMessagesCount > 0:
        result.unviewedMessagesCount =
          result.unviewedMessagesCount + chat.unviewedMessagesCount
  ...
  proc updateOrAddChat*(self: Service, chat: ChatDto) =
    ...
    self.events.emit(
      SIGNAL_CHAT_ADDED_OR_UPDATED,
      ChatArgs(communityId: chat.communityId, chatId: chat.id),
    )
  ...
  proc updateChannelMembers*(self: Service, channel: ChatDto) =
    ...
    self.events.emit(
      SIGNAL_CHAT_MEMBERS_CHANGED,
      ChatMembersChangedArgs(chatId: chat.id, members: chat.members),
    )

  proc parseChatResponse*(
      self: Service, response: RpcResponse[JsonNode]
  ): (seq[ChatDto], seq[MessageDto]) =
    var chats: seq[ChatDto] = @[]
  ...
  proc signalChatsAndMessagesUpdates*(
      self: Service, chats: seq[ChatDto], messages: seq[MessageDto]
  ) =
    ...
    for msg in messages:
      if chatMap.hasKey(msg.chatId):
        ...
        self.events.emit(
          SIGNAL_SENDING_SUCCESS, MessageSendingSuccess(message: msg, chat: chat)
        )
  ...
  proc processGroupChatResponse*(self: Service, response: RpcResponse[JsonNode]) =
    proc parseGroupChatResponse(
        response: RpcResponse[JsonNode]
    ): (ChatDto, seq[MessageDto], string) =
      var chat: ChatDto
      var messages: seq[MessageDto] = @[]
  ...
  proc processUpdateForTransaction*(
      self: Service, messageId: string, response: RpcResponse[JsonNode]
  ) =
    var (chats, _) = self.processMessengerResponse(response)
    # TODO: Signal is not handled anywhere
    self.events.emit(
      SIGNAL_MESSAGE_REMOVE, MessageArgs(id: messageId, channel: chats[0].id)
    )
  ...
  proc getChatsOfChatTypes*(
      self: Service, types: seq[chat_dto.ChatType]
  ): seq[ChatDto] =
  ...
  proc getOneToOneChatNameAndImage*(
      self: Service, chatId: string
  ): tuple[name: string, image: string, largeImage: string] =
    return self.contactService.getContactNameAndImage(chatId)

  proc createChatFromResponse(
      self: Service, response: RpcResponse[JsonNode]
  ): tuple[chatDto: ChatDto, success: bool] =
  ...
  proc createOneToOneChat*(
      self: Service, communityID: string, chatId: string, ensName: string
  ): tuple[chatDto: ChatDto, success: bool] =
    try:
      if self.hasChannel(chatId):
        # We want to show the chat to the user and for that we activate the chat
        discard
          status_group_chat.createOneToOneChat(communityID, chatId, ensName = ensName)
        result.success = true
  ...
  proc switchToOrCreateOneToOneChat*(self: Service, chatId: string, ensName: string) =
    self.events.emit(
      SIGNAL_CHAT_SWITCH_TO_OR_CREATE_1_1_CHAT,
      ChatExtArgs(chatId: chatId, ensName: ensName),
    )
  ...
  proc leaveChat*(self: Service, chatId: string) =
    try:
      ...
      discard status_chat.deactivateChat(
        chatId, preserveHistory = chat.chatType == chat_dto.ChatType.OneToOne
      )
  ...
  proc asyncSendImages*(
    ...
    try:
      ...
      self.events.emit(
        SIGNAL_SENDING_FAILED, MessageSendingFailure(chatId: chatId, error: e.msg)
      )
  ...
  proc asyncSendChatMessage*(
    ...
    try:
      ...
      let processedMsg =
        message_common.replaceMentionsWithPubKeys(allKnownContacts, msg)
  ...
  proc requestAddressForTransaction*(
    ...
    try:
      ...
      let response = status_chat_commands.requestAddressForTransaction(
        chatId, fromAddress, amount, address
      )
  ...
  proc requestTransaction*(
    ...
    try:
      let address = if (tokenAddress == ZERO_ADDRESS): "" else: tokenAddress
      let response =
        status_chat_commands.requestTransaction(chatId, fromAddress, amount, address)
      discard self.processMessengerResponse(response)
    except Exception as e:
  ...
  proc acceptRequestAddressForTransaction*(
      self: Service, messageId: string, address: string
  ) =
    try:
      let response =
        status_chat_commands.acceptRequestAddressForTransaction(messageId, address)
      self.processUpdateForTransaction(messageId, response)
    except Exception as e:
      error "Error requesting transaction", msg = e.msg

  proc acceptRequestTransaction*(
      self: Service, transactionHash: string, messageId: string, signature: string
  ) =
    try:
      let response = status_chat_commands.acceptRequestTransaction(
        transactionHash, messageId, signature
      )
      discard self.processMessengerResponse(response)
    except Exception as e:
      error "Error requesting transaction", msg = e.msg
  ...
  proc addGroupMembers*(
      self: Service, communityID: string, chatID: string, members: seq[string]
  ) =
    try:
      let response = status_group_chat.addMembers(communityID, chatId, members)
      if (response.error.isNil):
        self.events.emit(
          SIGNAL_CHAT_MEMBERS_ADDED, ChatMembersAddedArgs(chatId: chatId, ids: members)
        )
  ...
  proc removeMemberFromGroupChat*(
      self: Service, communityID: string, chatID: string, member: string
  ) =
    try:
      let response = status_group_chat.removeMember(communityID, chatId, member)
      if (response.error.isNil):
        self.events.emit(
          SIGNAL_CHAT_MEMBER_REMOVED, ChatMemberRemovedArgs(chatId: chatId, id: member)
        )
        self.processGroupChatResponse(response)
    except Exception as e:
  ...
  proc renameGroupChat*(
      self: Service, communityID: string, chatID: string, name: string
  ) =
    try:
  ...
  proc updateGroupChatDetails*(
    ...
    try:
      ...
      parsedImage["imagePath"] =
        %singletonInstance.utils.formatImagePath(parsedImage["imagePath"].getStr)
      let response =
        status_chat.editChat(communityID, chatID, name, color, $parsedImage)

      discard self.processMessengerResponse(response)

      let chat = self.chats[chatID]
      self.events.emit(
        SIGNAL_GROUP_CHAT_DETAILS_UPDATED,
        ChatUpdateDetailsArgs(
          id: chatID, newName: name, newColor: color, newImage: chat.icon
        ),
      )
    except Exception as e:
  ...
  proc makeAdmin*(
      self: Service, communityID: string, chatID: string, memberId: string
  ) =
    try:
        ...
        if (member.id == memberId):
          ...
          self.events.emit(
            ...
            ChatMemberUpdatedArgs(
              id: member.id, role: member.role, chatId: chatId, joined: member.joined
            ),
          )
          break
    except Exception as e:
  ...
  proc createGroupChatFromInvitation*(
      self: Service, groupName: string, chatId: string, adminPK: string
  ): tuple[chatDto: ChatDto, success: bool] =
    try:
      let response =
        status_group_chat.createGroupChatFromInvitation(groupName, chatId, adminPK)
      result = self.createChatFromResponse(response)
    except Exception as e:
      error "error while creating group from invitation: ", msg = e.msg

  proc createGroupChat*(
      self: Service, communityID: string, name: string, members: seq[string]
  ): tuple[chatDto: ChatDto, success: bool] =
    try:
  ...
  proc updateUnreadMessage*(
      self: Service, chatID: string, messagesCount: int, messagesWithMentionsCount: int
  ) =
  ...
  proc updateUnreadMessagesAndMentions*(
      self: Service,
      chatID: string,
      markAllAsRead: bool,
      markAsReadCount: int,
      markAsReadMentionsCount: int,
  ) =
    var chat = self.getChatById(chatID)
    if chat.id == "":
      return
    if markAllAsRead:
      ...
      chat.unviewedMentionsCount =
        max(0, chat.unviewedMentionsCount - markAsReadMentionsCount)
    self.updateOrAddChat(chat)

  proc asyncCheckChannelPermissions*(
  ...
  proc onAsyncCheckChannelPermissionsDone*(
    ...
    try:
      ...
      let checkChannelPermissionsResponse =
        rpcResponseObj["response"].toCheckChannelPermissionsResponseDto()
      self.events.emit(
        SIGNAL_CHECK_CHANNEL_PERMISSIONS_RESPONSE,
        CheckChannelPermissionsResponseArgs(
          communityId: communityId,
          chatId: chatId,
          checkChannelPermissionsResponse: checkChannelPermissionsResponse,
        ),
      )
    except Exception as e:
      let errMsg = e.msg
  ...
  proc asyncCheckAllChannelsPermissions*(
  ...
  proc onAsyncCheckAllChannelsPermissionsDone*(
      self: Service, rpcResponse: string
  ) {.slot.} =
    let rpcResponseObj = rpcResponse.parseJson
    let communityId = rpcResponseObj{"communityId"}.getStr()
    try:
      ...
      let checkAllChannelsPermissionsResponse =
        rpcResponseObj["response"].toCheckAllChannelsPermissionsResponseDto()
      # TODO save it
      self.events.emit(
        SIGNAL_CHECK_ALL_CHANNELS_PERMISSIONS_RESPONSE,
      ...
      self.events.emit(
        SIGNAL_CHECK_ALL_CHANNELS_PERMISSIONS_FAILED,
        CheckChannelsPermissionsErrorArgs(communityId: communityId, error: errMsg),
      )
