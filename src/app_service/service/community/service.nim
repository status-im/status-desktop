...
const SIGNAL_COMMUNITY_EDIT_SHARED_ADDRESSES_SUCCEEDED* =
  "communityEditSharedAddressesSucceded"
const SIGNAL_COMMUNITY_EDIT_SHARED_ADDRESSES_FAILED* =
  "communityEditSharedAddressesFailed"
const SIGNAL_COMMUNITY_MEMBER_REVEALED_ACCOUNTS_LOADED* =
  "communityMemberRevealedAccountsLoaded"
const SIGNAL_COMMUNITY_MEMBERS_REVEALED_ACCOUNTS_LOADED* =
  "communityMembersRevealedAccountsLoaded"
...
const SIGNAL_COMMUNITY_DATA_IMPORTED* = "communityDataImported"
  # This one is when just loading the data with requestCommunityInfo
...
const SIGNAL_WAITING_ON_NEW_COMMUNITY_OWNER_TO_CONFIRM_REQUEST_TO_REJOIN* =
  "waitingOnNewCommunityOwnerToConfirmRequestToRejoin"
...
const SIGNAL_COMMUNITY_HISTORY_ARCHIVES_DOWNLOAD_STARTED* =
  "communityHistoryArchivesDownloadStarted"
const SIGNAL_COMMUNITY_HISTORY_ARCHIVES_DOWNLOAD_FINISHED* =
  "communityHistoryArchivesDownloadFinished"

const SIGNAL_DISCORD_CATEGORIES_AND_CHANNELS_EXTRACTED* =
...
const SIGNAL_ACCEPT_REQUEST_TO_JOIN_FAILED_NO_PERMISSION* =
  "acceptRequestToJoinFailedNoPermission"
...
QtObject:
  ...
  proc asyncAcceptRequestToJoinCommunity*(
    self: Service, communityId: string, requestId: string
  )

  proc handleCommunityUpdates(
    self: Service,
    communities: seq[CommunityDto],
  ...
  proc handleCommunitiesRequestsToJoin(
    self: Service, membershipRequests: seq[CommunityMembershipRequestDto]
  )

  proc handleCommunitiesSettingsUpdates(
    self: Service, communitiesSettings: seq[CommunitySettingsDto]
  )

  proc getPendingRequestIndex(
    self: Service, communityId: string, requestId: string
  ): int

  proc getWaitingForSharedAddressesRequestIndex(
    self: Service, communityId: string, requestId: string
  ): int

  proc updateMembershipRequestToNewState*(
    self: Service,
    communityId: string,
    requestId: string,
    updatedCommunity: CommunityDto,
    newState: RequestToJoinType,
  )

  proc getUserPubKeyFromPendingRequest*(
    self: Service, communityId: string, requestId: string
  ): string
  ...
  proc getFilteredJoinedAndSpectatedCommunities(
      self: Service
  ): Table[string, CommunityDto] =
  ...
  proc doConnect(self: Service) =
    self.events.on(SignalType.CommunityFound.event) do(e: Args):
      ...
      self.events.emit(
        SIGNAL_COMMUNITY_DATA_IMPORTED, CommunityArgs(community: receivedData.community)
      )

      if self.communities.contains(receivedData.community.id) and
          self.communities[receivedData.community.id].listedInDirectory and
          not self.communities[receivedData.community.id].isAvailable:
        self.events.emit(
          SIGNAL_CURATED_COMMUNITY_FOUND,
          CommunityArgs(community: self.communities[receivedData.community.id]),
        )

    self.events.on(SignalType.CuratedCommunitiesUpdated.event) do(e: Args):
      var receivedData = CuratedCommunitiesSignal(e)
      self.events.emit(
        SIGNAL_CURATED_COMMUNITIES_UPDATED,
    ...
    self.events.on(SignalType.Message.event) do(e: Args):
      ...
      if (receivedData.communities.len > 0):
        ...
        self.handleCommunityUpdates(
          receivedData.communities, receivedData.chats, receivedData.removedChats
        )
    ...
    self.events.on(SignalType.DiscordCategoriesAndChannelsExtracted.event) do(e: Args):
      ...
      self.events.emit(
        SIGNAL_DISCORD_CATEGORIES_AND_CHANNELS_EXTRACTED,
        DiscordCategoriesAndChannelsArgs(
    ...
    self.events.on(SignalType.DiscordCommunityImportFinished.event) do(e: Args):
      ...
      self.events.emit(
        SIGNAL_DISCORD_COMMUNITY_IMPORT_FINISHED,
        CommunityIdArgs(communityId: receivedData.communityId),
      )
    ...
    self.events.on(SignalType.DiscordCommunityImportCancelled.event) do(e: Args):
      ...
      self.events.emit(
        SIGNAL_DISCORD_COMMUNITY_IMPORT_CANCELED,
        CommunityIdArgs(communityId: receivedData.communityId),
      )
    ...
    self.events.on(SignalType.DiscordCommunityImportCleanedUp.event) do(e: Args):
      ...
      self.events.emit(
        SIGNAL_COMMUNITY_LEFT, CommunityIdArgs(communityId: receivedData.communityId)
      )
    ...
    self.events.on(SignalType.DiscordChannelImportFinished.event) do(e: Args):
      ...
      self.events.emit(
        SIGNAL_DISCORD_CHANNEL_IMPORT_FINISHED,
        CommunityChatIdArgs(
          chatId: receivedData.channelId, communityId: receivedData.communityId
        ),
      )
    ...
    self.events.on(SignalType.DiscordChannelImportCancelled.event) do(e: Args):
      ...
      self.events.emit(
        SIGNAL_DISCORD_CHANNEL_IMPORT_CANCELED,
        ChannelIdArgs(channelId: receivedData.channelId),
      )
    ...
    self.events.on(SignalType.DiscordCommunityImportProgress.event) do(e: Args):
      ...
      self.events.emit(
        SIGNAL_DISCORD_COMMUNITY_IMPORT_PROGRESS,
    ...
    self.events.on(SignalType.DiscordChannelImportProgress.event) do(e: Args):
      ...
      self.events.emit(
        SIGNAL_DISCORD_CHANNEL_IMPORT_PROGRESS,
        DiscordImportChannelProgressArgs(
    ...
    self.events.on(SignalType.ImportingHistoryArchiveMessages.event) do(e: Args):
      ...
      if receivedData.communityId notin self.historyArchiveDownloadTaskCommunityIds:
        ...
        self.events.emit(
          SIGNAL_COMMUNITY_HISTORY_ARCHIVES_DOWNLOAD_STARTED,
          CommunityIdArgs(communityId: receivedData.communityId),
        )
    ...
    self.events.on(SignalType.DownloadingHistoryArchivesFinished.event) do(e: Args):
      ...
      if receivedData.communityId in self.historyArchiveDownloadTaskCommunityIds:
        ...
        if self.historyArchiveDownloadTaskCommunityIds.len == 0:
          ...
          self.events.emit(
            SIGNAL_COMMUNITY_HISTORY_ARCHIVES_DOWNLOAD_FINISHED,
            CommunityIdArgs(communityId: receivedData.communityId),
          )
    ...
    self.events.on(SignalType.MemberReevaluationStatus.event) do(e: Args):
      ...
      self.events.emit(
        SIGNAL_MEMBER_REEVALUATION_STATUS,
        CommunityMemberReevaluationStatusArg(
          communityId: receivedData.communityId, status: receivedData.status
        ),
      )
  ...
  proc saveUpdatedCommunity(self: Service, community: var CommunityDto) =
    ...
    community.pendingRequestsToJoin =
      self.communities[community.id].pendingRequestsToJoin
    community.declinedRequestsToJoin =
      self.communities[community.id].declinedRequestsToJoin
    community.canceledRequestsToJoin =
      self.communities[community.id].canceledRequestsToJoin
    community.waitingForSharedAddressesRequestsToJoin =
      self.communities[community.id].waitingForSharedAddressesRequestsToJoin

    # Update the joinded community list with the new data
    self.communities[community.id] = community

  proc getChatsInCategory(
      self: Service, community: var CommunityDto, categoryId: string
  ): seq[ChatDto] =
    result = @[]
    ...
    for settings in communitiesSettings:
      if self.communities.hasKey(settings.id):
        ...
        if self.communities[settings.id].joined:
          self.events.emit(
            SIGNAL_COMMUNITY_EDITED,
            CommunityArgs(community: self.communities[settings.id]),
          )

  proc checkForCategoryPropertyUpdates(
      self: Service, community: CommunityDto, prev_community: CommunityDto
  ) =
    for category in community.categories:
      ...
      if category.position != prev_category.position:
        self.events.emit(
          SIGNAL_COMMUNITY_CATEGORY_REORDERED,
      ...
      if category.name != prev_category.name:
        self.events.emit(
          SIGNAL_COMMUNITY_CATEGORY_NAME_EDITED,
          CommunityCategoryArgs(communityId: community.id, category: category),
        )

  proc communityTokensChanged(
      community: CommunityDto, prev_community: CommunityDto
  ): bool =
  ...
  proc updateMemberRole(self: Service, chatId: string, member: ChatMember) =
    self.events.emit(
      ...
      ChatMemberUpdatedArgs(
        chatId: chatId, id: member.id, role: member.role, joined: member.joined
      ),
    )
    ...
    try:
      ...
      if not self.communities.hasKey(community.id):
        ...
        if (community.joined and community.isMember):
          self.events.emit(
            SIGNAL_COMMUNITY_JOINED,
            CommunityArgs(community: community, fromUserAction: false),
          )
        return
      ...
      if prevCommunity.isOwner and not community.isOwner:
        self.events.emit(
          SIGNAL_COMMUNITY_LOST_OWNERSHIP, CommunityIdArgs(communityId: community.id)
        )
      ...
      if (community.categories.len > prevCommunity.categories.len):
        for category in community.categories:
          if findIndexById(category.id, prevCommunity.categories) == -1:
            ...
            self.events.emit(
              SIGNAL_COMMUNITY_CATEGORY_CREATED,
              CommunityCategoryArgs(
                communityId: community.id, category: category, chats: chats
              ),
            )
      ...
      # category was removed
        ...
        for prv_category in prevCommunity.categories:
          if findIndexById(prv_category.id, community.categories) == -1:
            ...
            self.events.emit(
              SIGNAL_COMMUNITY_CATEGORY_DELETED,
              CommunityCategoryArgs(
                communityId: community.id, category: Category(id: prv_category.id)
              ),
            )
        ...
        for chat in community.chats:
          if findIndexById(chat.id, prevCommunity.chats) == -1:
            self.chatService.updateOrAddChat(chat)
              # we have to update chats stored in the chat service.
            ...
            self.events.emit(
              SIGNAL_COMMUNITY_CHANNEL_REORDERED,
              CommunityChatOrderArgs(communityId: community.id, chat: chat),
            )
      ...
      # channel was removed
        ...
        for prv_chat in prevCommunity.chats:
          if findIndexById(prv_chat.id, community.chats) == -1:
            self.events.emit(
              SIGNAL_COMMUNITY_CHANNEL_DELETED,
              CommunityChatIdArgs(communityId: community.id, chatId: prv_chat.id),
            )
      # some property has changed
      else:
        for chat in community.chats:
          ...
          if chat.categoryId != prevChat.categoryId:
            ...
            self.events.emit(
              SIGNAL_COMMUNITY_CHANNEL_CATEGORY_CHANGED,
              CommunityChatOrderArgs(communityId: community.id, chat: chat),
            )
          ...
          if chat.name != prevChat.name or chat.description != prevChat.description or
              ...
              chat.canPost != prevChat.canPost or chat.canView != prevChat.canView or
            ...
            self.chatService.updateOrAddChat(updatedChat)
              # we have to update chats stored in the chat service.
      ...
      if community.members != prevCommunity.members:
        self.events.emit(
          SIGNAL_COMMUNITY_MEMBERS_CHANGED,
          CommunityMembersArgs(communityId: community.id, members: community.members),
        )

      # token metadata was added
      if community.communityTokensMetadata.len >
          prevCommunity.communityTokensMetadata.len:
        for tokenMetadata in community.communityTokensMetadata:
          if findIndexBySymbol(
            tokenMetadata.symbol, prevCommunity.communityTokensMetadata
          ) == -1:
            self.events.emit(
              SIGNAL_COMMUNITY_TOKEN_METADATA_ADDED,
              CommunityTokenMetadataArgs(
                communityId: community.id, tokenMetadata: tokenMetadata
              ),
            )
      ...
      if community.tokenPermissions.len > prevCommunity.tokenPermissions.len:
        for id, tokenPermission in community.tokenPermissions:
          if not prevCommunity.tokenPermissions.hasKey(id):
            self.events.emit(
              SIGNAL_COMMUNITY_TOKEN_PERMISSION_CREATED,
              CommunityTokenPermissionArgs(
                communityId: community.id, tokenPermission: tokenPermission
              ),
            )
        ...
        for id, prvTokenPermission in prevCommunity.tokenPermissions:
          if not community.tokenPermissions.hasKey(id):
            self.events.emit(
              SIGNAL_COMMUNITY_TOKEN_PERMISSION_DELETED,
              CommunityTokenPermissionArgs(
                communityId: community.id, tokenPermission: prvTokenPermission
              ),
            )
      else:
        for id, tokenPermission in community.tokenPermissions:
          ...
          for tc in tokenPermission.tokenCriteria:
            ...
            if index == -1:
              ...
              if tc.amount != prevTc.amount or tc.ensPattern != prevTc.ensPattern or
                  tc.symbol != prevTc.symbol or tc.name != prevTc.name or
                  tc.decimals != prevTc.decimals:
                permissionUpdated = true
                break
          ...
          if permissionUpdated:
            self.events.emit(
              SIGNAL_COMMUNITY_TOKEN_PERMISSION_UPDATED,
              CommunityTokenPermissionArgs(
                communityId: community.id, tokenPermission: tokenPermission
              ),
            )
      ...
      self.events.emit(
        SIGNAL_COMMUNITIES_UPDATE, CommunitiesArgs(communities: @[community])
      )
      ...
      if wasJoined and not community.joined and not community.isMember:
        ...
        if community.spectated:
          ...
          self.events.emit(
            SIGNAL_COMMUNITY_LEFT, CommunityIdArgs(communityId: community.id)
          )
          ...
          self.events.emit(
            SIGNAL_COMMUNITY_MEMBER_STATUS_CHANGED,
            CommunityMemberStatusUpdatedArgs(
              communityId: community.id, memberPubkey: myPublicKey, state: state
            ),
          )

      # Check if we were unbanned
      if not wasJoined and not community.joined and
          prevCommunity.pendingAndBannedMembers.hasKey(myPublicKey) and
          not community.pendingAndBannedMembers.hasKey(myPublicKey):
        self.events.emit(
          SIGNAL_COMMUNITY_MEMBER_STATUS_CHANGED,
          CommunityMemberStatusUpdatedArgs(
            communityId: community.id,
  ...
  proc handleCommunitiesRequestsToJoin(
    ...
    for membershipRequest in membershipRequests:
      if (not self.communities.contains(membershipRequest.communityId)):
        error "Received a membership request for an unknown community",
          communityId = membershipRequest.communityId
        continue

      let requestToJoinState = RequestToJoinType(membershipRequest.state)
      let noAwaitingIndex =
        self.getWaitingForSharedAddressesRequestIndex(
          membershipRequest.communityId, membershipRequest.id
        ) == -1
      if requestToJoinState == RequestToJoinType.AwaitingAddress and noAwaitingIndex:
        self.communities[membershipRequest.communityId].waitingForSharedAddressesRequestsToJoin.add(
          membershipRequest
        )
        let myPublicKey = singletonInstance.userProfile.getPubKey()
        if myPublicKey == membershipRequest.publicKey:
          self.events.emit(
            SIGNAL_WAITING_ON_NEW_COMMUNITY_OWNER_TO_CONFIRM_REQUEST_TO_REJOIN,
            CommunityIdArgs(communityId: membershipRequest.communityId),
          )
      elif requestToJoinState == RequestToJoinType.Pending and
          self.getPendingRequestIndex(
            membershipRequest.communityId, membershipRequest.id
          ) == -1:
        self.communities[membershipRequest.communityId].pendingRequestsToJoin.add(
          membershipRequest
        )
        self.events.emit(
        ...
        self.events.emit(
          SIGNAL_NEW_REQUEST_TO_JOIN_COMMUNITY_ACCEPTED,
          CommunityRequestArgs(communityRequest: membershipRequest),
        )

      try:
        self.updateMembershipRequestToNewState(
          membershipRequest.communityId,
          membershipRequest.id,
          self.communities[membershipRequest.communityId],
          requestToJoinState,
        )
      except Exception as e:
      ...
      if nonAprrovedRequestsToJoinObj{"result"}.kind != JNull:
        for jsonCommunityReqest in nonAprrovedRequestsToJoinObj["result"]:
          ...
          if not (communityRequest.communityId in self.communities):
            warn "community was not found for community request",
              communityID = communityRequest.communityId,
              requestId = communityRequest.id
            continue
          case RequestToJoinType(communityRequest.state)
          of RequestToJoinType.Pending, RequestToJoinType.AcceptedPending,
              RequestToJoinType.DeclinedPending:
            self.communities[communityRequest.communityId].pendingRequestsToJoin.add(
              communityRequest
            )
          of RequestToJoinType.Declined:
            self.communities[communityRequest.communityId].declinedRequestsToJoin.add(
              communityRequest
            )
          of RequestToJoinType.Canceled:
            self.communities[communityRequest.communityId].canceledRequestsToJoin.add(
              communityRequest
            )
          of RequestToJoinType.AwaitingAddress:
            self.communities[communityRequest.communityId].waitingForSharedAddressesRequestsToJoin.add(
              communityRequest
            )
  ...
  proc isDisplayNameDupeOfCommunityMember*(self: Service, displayName: string): bool =
    try:
      ...
      if response.error != nil:
        ...
        raise newException(
          RpcException, "Error scanning communities for member name: " & error.message
        )
  ...
  proc getCategoryById*(
      self: Service, communityId: string, categoryId: string
  ): Category =
  ...
  proc getCategories*(
      self: Service, communityId: string, order: SortOrder = SortOrder.Ascending
  ): seq[Category] =
    if (not self.communities.contains(communityId)):
      error "trying to get community categories for an unexisting community id"
      return
  ...
  proc getChats*(
      self: Service, communityId: string, categoryId = "", order = SortOrder.Ascending
  ): seq[ChatDto] =
  ...
  proc getAllChats*(
      self: Service, communityId: string, order = SortOrder.Ascending
  ): seq[ChatDto] =
  ...
  proc isUserMemberOfCommunity*(self: Service, communityId: string): bool =
    if (not self.communities.contains(communityId)):
      return false
    return
      self.communities[communityId].joined and self.communities[communityId].isMember
  ...
  proc processRequestsToJoinCommunity(self: Service, responseResult: JsonNode): bool =
    if responseResult{"requestsToJoinCommunity"} == nil or
        responseResult{"requestsToJoinCommunity"}.kind == JNull:
      return false
    ...
    for jsonCommunityReqest in responseResult["requestsToJoinCommunity"]:
      ...
      if (not self.communities.contains(communityRequest.communityId)):
        error "Request to join for an unknown community",
          communityId = communityRequest.communityId
        return false
      ...
      self.events.emit(
        SIGNAL_COMMUNITY_MY_REQUEST_ADDED,
        CommunityRequestArgs(communityRequest: communityRequest),
      )
  ...
  proc spectateCommunity*(self: Service, communityId: string): string =
    ...
    try:
      if (
        self.isUserSpectatingCommunity(communityId) or
        self.isUserMemberOfCommunity(communityId)
      ):
        return
      ...
      if not response.result.hasKey("communities") or
          response.result["communities"].kind != JArray or
          response.result["communities"].len == 0:
        error "error: ",
          procName = "spectateCommunity",
          errDesription = "no 'communities' key in response"
        return

      if not response.result.hasKey("communitiesSettings") or
          response.result["communitiesSettings"].kind != JArray or
          response.result["communitiesSettings"].len == 0:
        error "error: ",
          procName = "spectateCommunity",
          errDesription = "no 'communitiesSettings' key in response"
        return

      var updatedCommunity = response.result["communities"][0].toCommunityDto()
      let communitySettings =
        response.result["communitiesSettings"][0].toCommunitySettingsDto()

      updatedCommunity.settings = communitySettings
      self.communities[communityId] = updatedCommunity

      let ownerTokenNotification = self.activityCenterService.getNotificationForTypeAndCommunityId(
      ...
      self.events.emit(
        SIGNAL_COMMUNITIES_UPDATE, CommunitiesArgs(communities: @[updatedCommunity])
      )
      self.events.emit(
        SIGNAL_COMMUNITY_SPECTATED,
        CommunityArgs(
          community: updatedCommunity,
          fromUserAction: true,
          isPendingOwnershipRequest: (ownerTokenNotification != nil),
        ),
      )
    except Exception as e:
  ...
  proc requestImportDiscordCommunity*(
    ...
    try:
      ...
      let response = status_go.requestImportDiscordCommunity(
        name, description, introMessage, outroMessage, access, color, tagsString, image,
        aX, aY, bX, bY, historyArchiveSupportEnabled, pinMessageAllMembersEnabled,
      ...
      if response.error != nil:
        let error = Json.decode($response.error, RpcError)
        raise newException(
          RpcException, "Error importing discord community: " & error.message
        )
    except Exception as e:
  ...
  proc requestImportDiscordChannel*(
    ...
    try:
      ...
      if response.error != nil:
        let error = Json.decode($response.error, RpcError)
        raise newException(
          RpcException, "Error importing discord channel: " & error.message
        )
    except Exception as e:
  ...
  proc createCommunity*(
    ...
    try:
      ...
      bannerJson{"imagePath"} = newJString(
        singletonInstance.utils.formatImagePath(bannerJson["imagePath"].getStr)
      )
      var tagsString = tags
      if len(tagsString) == 0:
        tagsString = "[]"
  ...
  proc createCommunityChannel*(
    ...
    try:
      let response = status_go.createCommunityChannel(
        communityId, name, description, emoji, color, categoryId,
      ...
      if not response.error.isNil:
        let error = Json.decode($response.error, RpcError)
        raise newException(
          RpcException, "Error creating community channel: " & error.message
        )
      ...
      if (not response.result.getProp("chats", chatsJArr)):
        raise newException(
          RpcException,
          fmt"createCommunityChannel; there is no `chats` key in the response for community id: {communityId}",
        )

      let chatsForCategory = self.getChats(communityId, categoryId)
      let maxPosition =
        if chatsForCategory.len > 0:
          chatsForCategory[^1].position
        else:
          -1
      ...
      error "Error creating community channel",
        msg = e.msg, communityId, name, description, procName = "createCommunityChannel"
  ...
  proc editCommunityChannel*(
    ...
    try:
      let response = status_go.editCommunityChannel(
        communityId, channelId, name, description, emoji, color, categoryId, position,
        viewersCanPostReactions, hideIfPermissionsNotMet,
      )
      ...
      if response.error != nil:
        let error = Json.decode($response.error, RpcError)
        raise newException(
          RpcException, "Error editing community channel: " & error.message
        )
      ...
      if (not response.result.getProp("chats", chatsJArr)):
        raise newException(
          RpcException,
          fmt"editCommunityChannel; there is no `chats` key in the response for community id: {communityId}",
        )
      ...
      error "Error editing community channel",
        msg = e.msg, communityId, channelId, procName = "editCommunityChannel"
  ...
  proc reorderCommunityChat*(
    ...
    try:
      let response =
        status_go.reorderCommunityChat(communityId, categoryId, chatId, position)

      if not response.error.isNil:
        let error = Json.decode($response.error, RpcError)
        raise newException(
          RpcException, "Error reordering community channel: " & error.message
        )
      ...
      for chat in updatedCommunity.chats:
        ...
        if (chat.position != prev_chat.position and chat.categoryId == categoryId):
          ...
          self.communities[communityId].chats[prev_chat_idx].categoryId =
            chat.categoryId
        else:
          continue

        chatDetails.updateMissingFields(
          self.communities[communityId].chats[prev_chat_idx]
        )
        self.chatService.updateOrAddChat(chatDetails)
          # we have to update chats stored in the chat service.
        updatedChats.add(chat)

      self.events.emit(
        SIGNAL_COMMUNITY_CHANNELS_REORDERED,
        CommunityChatsOrderArgs(communityId: updatedCommunity.id, chats: updatedChats),
      )

      self.communities[communityId] = updatedCommunity
    except Exception as e:
      error "Error reordering community channel",
        msg = e.msg, communityId, chatId, position, procName = "reorderCommunityChat"
  ...
  proc deleteCommunityChat*(self: Service, communityId: string, chatId: string) =
    try:
      ...
      if not response.error.isNil:
        let error = Json.decode($response.error, RpcError)
        raise
          newException(RpcException, "Error deleting community chat: " & error.message)
      ...
      self.events.emit(
        SIGNAL_COMMUNITY_CHANNEL_DELETED,
        CommunityChatIdArgs(communityId: communityId, chatId: chatId),
      )
    except Exception as e:
      error "Error deleting community channel",
        msg = e.msg, communityId, chatId, procName = "deleteCommunityChat"

  proc createCommunityCategory*(
      self: Service, communityId: string, name: string, channels: seq[string]
  ) =
    try:
      ...
      if response.error != nil:
        let error = Json.decode($response.error, RpcError)
        raise newException(
          RpcException, "Error creating community category: " & error.message
        )
      ...
      if response.result != nil and response.result.kind != JNull:
        ...
        for chatId, v in response.result["communityChanges"].getElems()[0][
          "chatsModified"
        ].pairs():
          let fullChatId = communityId & chatId
          let idx = findIndexById(fullChatId, self.communities[communityId].chats)
          if idx > -1:
            self.communities[communityId].chats[idx].categoryId =
              v["CategoryModified"].getStr()
            self.communities[communityId].chats[idx].position =
              v["PositionModified"].getInt()
        ...
        for k, v in response.result["communityChanges"].getElems()[0]["categoriesAdded"].pairs():
          ...
          self.events.emit(
            SIGNAL_COMMUNITY_CATEGORY_CREATED,
            CommunityCategoryArgs(
              communityId: communityId, category: category, chats: chats
            ),
          )
    except Exception as e:
      error "Error creating community category", msg = e.msg, communityId, name

  proc toggleCollapsedCommunityCategoryAsync*(
      self: Service, communityId: string, categoryId: string, collapsed: bool
  ) =
  ...
  proc editCommunityCategory*(
    ...
    try:
      let response =
        status_go.editCommunityCategory(communityId, categoryId, name, channels)
      if response.error != nil:
        let error = Json.decode($response.error, RpcError)
        raise newException(
          RpcException, "Error creating community category: " & error.message
      ...
      if response.result != nil and response.result.kind != JNull:
        ...
        var updatedCommunity = response.result["communities"][0].toCommunityDto
        self.checkForCategoryPropertyUpdates(
          updatedCommunity, self.communities[communityId]
        )

        self.saveUpdatedCommunity(updatedCommunity)

        for k, v in response.result["communityChanges"].getElems()[0][
          "categoriesModified"
        ].pairs():
          let category = v.toCategory()
          self.events.emit(
            SIGNAL_COMMUNITY_CATEGORY_EDITED,
            CommunityCategoryArgs(
              communityId: communityId, category: category, chats: chats
            ),
          )
  ...
  proc deleteCommunityCategory*(
    ...
    try:
      ...
      if response.error != nil:
        let error = Json.decode($response.error, RpcError)
        raise newException(
          RpcException, "Error deleting community category: " & error.message
        )
  ...
  proc reorderCommunityCategories*(
      self: Service, communityId: string, categoryId: string, position: int
  ) =
    try:
      let response =
        status_go.reorderCommunityCategories(communityId, categoryId, position)

      if response.error != nil:
        let error = Json.decode($response.error, RpcError)
        raise newException(
          RpcException, "Error reordering community category: " & error.message
        )
      ...
      error "Error reordering category channel",
        msg = e.msg, communityId, categoryId, position
  ...
  proc asyncCommunityMetricsLoaded*(self: Service, rpcResponse: string) {.slot.} =
    ...
    self.events.emit(
      SIGNAL_COMMUNITY_METRICS_UPDATED,
      CommunityMetricsArgs(communityId: communityId, metricsType: metrics.metricsType),
    )

  proc asyncCommunityInfoLoaded*(
      self: Service, communityIdAndRpcResponse: string
  ) {.slot.} =
    ...
    if rpcResponseObj{"error"}.kind != JNull and rpcResponseObj{"error"}.getStr != "":
      ...
      self.events.emit(
        SIGNAL_COMMUNITY_LOAD_DATA_FAILED,
        CommunityArgs(
          communityId: requestedCommunityId, error: "Couldn't find community info"
        ),
      )
      return
    ...
    if community.id == "":
      ...
      self.events.emit(
        ...
        CommunityArgs(
          ...
          community: community,
          error: "Couldn't find community info",
        ),
      )
      return

    self.communities[community.id] = community
    debug "asyncRequestCommunityInfoTask finished",
      communityId = requestedCommunityId, communityName = community.name

    if rpcResponseObj{"importing"}.getBool():
      self.events.emit(SIGNAL_COMMUNITY_IMPORTED, CommunityArgs(community: community))

    self.events.emit(
      SIGNAL_COMMUNITY_DATA_IMPORTED, CommunityArgs(community: community)
    )
    self.events.emit(
      SIGNAL_COMMUNITIES_UPDATE, CommunitiesArgs(communities: @[community])
    )

  proc asyncCheckPermissionsToJoin*(
      self: Service, communityId: string, addresses: seq[string]
  ) =
  ...
  proc onAsyncCheckPermissionsToJoinDone*(self: Service, rpcResponse: string) {.slot.} =
    ...
    try:
      ...
      let checkPermissionsToJoinResponse =
        rpcResponseObj["response"].toCheckPermissionsToJoinResponseDto
      self.events.emit(
        SIGNAL_CHECK_PERMISSIONS_TO_JOIN_RESPONSE,
      ...
      self.events.emit(
        SIGNAL_CHECK_PERMISSIONS_TO_JOIN_FAILED,
        CheckPermissionsToJoinFailedArgs(communityId: communityId, error: errMsg),
      )
  ...
  proc generateJoiningCommunityRequestsForSigning*(
    ...
    try:
      let response = status_go.generateJoiningCommunityRequestsForSigning(
        memberPubKey, communityId, addressesToReveal
      )
      if not response.error.isNil:
      ...
      self.events.emit(
        SIGNAL_COMMUNITY_MY_REQUEST_FAILED,
        CommunityRequestFailedArgs(communityId: communityId, error: e.msg),
      )
  ...
  proc generateEditCommunityRequestsForSigning*(
    ...
    try:
      let response = status_go.generateEditCommunityRequestsForSigning(
        memberPubKey, communityId, addressesToReveal
      )
      if not response.error.isNil:
        raise newException(RpcException, response.error.message)
      ...
      self.events.emit(
        SIGNAL_COMMUNITY_EDIT_SHARED_ADDRESSES_FAILED,
        CommunityRequestFailedArgs(communityId: communityId, error: e.msg),
      )

  proc signCommunityRequests*(
      self: Service, communityId: string, signParams: seq[SignParamsDto]
  ): seq[string] =
    try:
      var data = %*[]
  ...
  proc asyncRequestToJoinCommunity*(
      self: Service,
      communityId: string,
      ensName: string,
  ...
  proc onAsyncRequestToJoinCommunityDone*(
      self: Service, communityIdAndRpcResponse: string
  ) {.slot.} =
    let rpcResponseObj = communityIdAndRpcResponse.parseJson
    try:
      ...
      checkAndEmitACNotificationsFromResponse(
        self.events, rpcResponse.result{"activityCenterNotifications"}
      )

      if not self.processRequestsToJoinCommunity(rpcResponse.result):
        raise
          newException(CatchableError, "no 'requestsToJoinCommunity' key in response")
    except Exception as e:
      error "Error requesting to join the community", msg = e.msg
      self.events.emit(
        SIGNAL_COMMUNITY_MY_REQUEST_FAILED,
        CommunityRequestFailedArgs(
          communityId: rpcResponseObj["communityId"].getStr, error: e.msg
        ),
      )

  proc asyncEditSharedAddresses*(
      self: Service,
      communityId: string,
      addressesToShare: seq[string],
      airdropAddress: string,
  ...
  proc onAsyncEditSharedAddressesDone*(
      self: Service, communityIdAndRpcResponse: string
  ) {.slot.} =
    let rpcResponseObj = communityIdAndRpcResponse.parseJson
    try:
      ...
      self.events.emit(
        SIGNAL_COMMUNITY_EDIT_SHARED_ADDRESSES_SUCCEEDED,
        CommunityIdArgs(communityId: rpcResponseObj["communityId"].getStr),
      )
    except Exception as e:
  ...
  proc asyncAcceptRequestToJoinCommunity*(
    ...
    try:
      ...
      self.events.emit(
        SIGNAL_ACCEPT_REQUEST_TO_JOIN_LOADING,
        CommunityMemberArgs(communityId: communityId, pubKey: userKey),
      )
  ...
  proc onAsyncAcceptRequestToJoinCommunityDone*(
    ...
    try:
      ...
      if rpcResponseObj{"error"}.kind != JNull and rpcResponseObj{"error"}.getStr != "":
        ...
        if errorMessage.contains("has no permission to join"):
          self.events.emit(
            SIGNAL_ACCEPT_REQUEST_TO_JOIN_FAILED_NO_PERMISSION,
            CommunityMemberArgs(
              communityId: communityId, pubKey: userKey, requestId: requestId
            ),
          )
        else:
          self.events.emit(
            SIGNAL_ACCEPT_REQUEST_TO_JOIN_FAILED,
            CommunityMemberArgs(
              communityId: communityId, pubKey: userKey, requestId: requestId
            ),
          )
      ...
      let requestToJoin =
        rpcResponseObj["response"]["result"]["requestsToJoinCommunity"][0].toCommunityMembershipRequestDto

      self.updateMembershipRequestToNewState(
        communityId, requestId, updatedCommunity, RequestToJoinType(requestToJoin.state)
      )
      ...
      self.events.emit(
        SIGNAL_COMMUNITY_EDITED, CommunityArgs(community: self.communities[communityId])
      )
      self.events.emit(
        SIGNAL_COMMUNITY_MEMBER_APPROVED,
        CommunityMemberArgs(
          communityId: communityId, pubKey: userKey, requestId: requestId
        ),
      )
  ...
  proc onAsyncLoadCuratedCommunitiesDone*(self: Service, response: string) {.slot.} =
    try:
      ...
      let curatedCommunities =
        parseCuratedCommunities(rpcResponseObj["response"]["result"])
      for curatedCommunity in curatedCommunities:
        self.communities[curatedCommunity.id] = curatedCommunity
      self.events.emit(
        SIGNAL_CURATED_COMMUNITIES_LOADED,
        CommunitiesArgs(communities: curatedCommunities),
      )
    except Exception as e:
      let errMsg = e.msg
  ...
  proc getCommunityMetrics*(
      self: Service, communityId: string, metricsType: CommunityMetricsType
  ): CommunityMetricsDto =
  ...
  proc collectCommunityMetricsMessagesTimestamps*(
      self: Service, communityId: string, intervals: string
  ) =
  ...
  proc collectCommunityMetricsMessagesCount*(
      self: Service, communityId: string, intervals: string
  ) =
    let arg = AsyncCollectCommunityMetricsTaskArg(
      tptr: asyncCollectCommunityMetricsTask,
  ...
  proc requestCommunityInfo*(
    ...
    if self.communityInfoRequests.hasKey(communityId):
      ...
      if actualTimeSincLastRequest < requiredTimeSinceLastRequest:
        debug "requestCommunityInfo: skipping as required time has not passed yet since last request",
          communityId, actualTimeSincLastRequest, requiredTimeSinceLastRequest
        return
  ...
  proc updateMembershipRequestToNewState*(
    ...
    let indexAwaitingAddresses =
      self.getWaitingForSharedAddressesRequestIndex(communityId, requestId)
    ...
    if (indexPending != -1):
      if @[
        RequestToJoinType.Declined, RequestToJoinType.Accepted,
        RequestToJoinType.Canceled,
      ].any(x => x == newState):
        # If the state is now declined, add to the declined requests
        if newState == RequestToJoinType.Declined:
          community.declinedRequestsToJoin.add(
            community.pendingRequestsToJoin[indexPending]
          )
        elif newState == RequestToJoinType.Canceled:
          self.events.emit(
            SIGNAL_REQUEST_TO_JOIN_COMMUNITY_CANCELED,
      ...
      if newState == RequestToJoinType.Declined:
        community.declinedRequestsToJoin.add(
          community.waitingForSharedAddressesRequestsToJoin[indexAwaitingAddresses]
        )

      let myPublicKey = singletonInstance.userProfile.getPubKey()
      let awaitingRequestToJoin =
        community.waitingForSharedAddressesRequestsToJoin[indexAwaitingAddresses]
      ...
      if awaitingRequestToJoin.publicKey == myPublicKey:
        self.events.emit(
          SIGNAL_WAITING_ON_NEW_COMMUNITY_OWNER_TO_CONFIRM_REQUEST_TO_REJOIN,
          CommunityIdArgs(communityId: communityId),
        )
  ...
  proc cancelRequestToJoinCommunity*(self: Service, communityId: string) =
    try:
      if (not self.communities.contains(communityId)):
        error "Cancel request to join community failed: unknown community",
          communityId = communityId
        return
  ...
  proc declineRequestToJoinCommunity*(
    ...
    try:
      ...
      let requestToJoin =
        response.result["requestsToJoinCommunity"][0].toCommunityMembershipRequestDto
  ...
  proc shareCommunityToUsers*(
      self: Service, communityId: string, pubKeysJson: string, inviteMessage: string
  ): string =
    try:
      ...
      let response =
        status_go.shareCommunityToUsers(communityId, pubKeys, inviteMessage)
  ...
  proc muteCategory*(
      self: Service, communityId: string, categoryId: string, interval: int
  ) =
    try:
      ...
      self.events.emit(
        SIGNAL_CATEGORY_MUTED,
        CategoryArgs(communityId: communityId, categoryId: categoryId),
      )
    except Exception as e:
  ...
  proc asyncBanUserFromCommunity*(
      self: Service, communityId, pubKey: string, deleteAllMessages: bool
  ) =
    let arg = AsyncCommunityMemberActionTaskArg(
  ...
  proc onAsyncCommunityMemberActionCompleted*(
      self: Service, response: string
  ) {.slot.} =
    try:
      let rpcResponseObj = response.parseJson
      ...
      if not rpcResponseObj["response"]{"result"}.getProp("communities", communityJArr):
        raise
          newException(RpcException, "there is no `communities` key in the response")
      ...
      if community.pendingAndBannedMembers.hasKey(memberPubkey):
        state =
          community.pendingAndBannedMembers[memberPubkey].toMembershipRequestState()
      ...
      self.events.emit(
        SIGNAL_COMMUNITY_MEMBER_STATUS_CHANGED,
        CommunityMemberStatusUpdatedArgs(
          communityId: community.id, memberPubkey: memberPubkey, state: state
        ),
      )
    except Exception as e:
      error "error while getting the community members' revealed addressesses",
        msg = e.msg
  ...
  proc isMyCommunityRequestPending*(self: Service, communityId: string): bool {.slot.} =
    if not self.communities.contains(communityId):
      error "IsMyCommunityRequestPending failed: unknown community",
        communityId = communityId
      return false
  ...
  proc waitingOnNewCommunityOwnerToConfirmRequestToRejoin*(
      self: Service, communityId: string
  ): bool {.slot.} =
    if not self.communities.contains(communityId):
      error "waitingOnNewCommunityOwnerToConfirmRequestToRejoin failed: unknown community",
        communityId = communityId
      return false
  ...
  proc requestExtractDiscordChannelsAndCategories*(
      self: Service, filesToImport: seq[string]
  ) =
    try:
  ...
  proc createOrEditCommunityTokenPermission*(
      self: Service, communityId: string, tokenPermission: CommunityTokenPermissionDto
  ) =
    try:
      let editing = tokenPermission.id != ""
      var response: RpcResponse[JsonNode]
      if editing:
        response = status_go.editCommunityTokenPermission(
          communityId,
          tokenPermission.id,
          int(tokenPermission.`type`),
          Json.encode(tokenPermission.tokenCriteria),
          tokenPermission.chatIDs,
          tokenPermission.isPrivate,
        )
      else:
        response = status_go.createCommunityTokenPermission(
          communityId,
          int(tokenPermission.`type`),
          Json.encode(tokenPermission.tokenCriteria),
          tokenPermission.chatIDs,
      ...
      self.events.emit(
        signal,
        CommunityTokenPermissionArgs(
          communityId: communityId, tokenPermission: tokenPermission
        ),
      )
    except Exception as e:
  ...
  proc deleteCommunityTokenPermission*(
      self: Service, communityId: string, permissionId: string
  ) =
    try:
      ...
      self.events.emit(
        SIGNAL_COMMUNITY_TOKEN_PERMISSION_DELETION_FAILED,
        CommunityTokenPermissionArgs(
          communityId: communityId, tokenPermission: tokenPermission
        ),
      )
  ...
  proc shareCommunityChannelUrlWithChatKey*(
      self: Service, communityId: string, chatId: string
  ): string =
    try:
  ...
  proc getCommunityPublicKeyFromPrivateKey*(
      self: Service, communityPrivateKey: string
  ): string =
    try:
  ...
  proc asyncGetRevealedAccountsForMember*(
      self: Service, communityId, memberPubkey: string
  ) =
  ...
  proc onAsyncGetRevealedAccountsForMemberCompleted*(
      self: Service, response: string
  ) {.slot.} =
    try:
      let rpcResponseObj = response.parseJson
      ...
      self.events.emit(
        SIGNAL_COMMUNITY_MEMBER_REVEALED_ACCOUNTS_LOADED,
        CommunityMemberRevealedAccountsArgs(
          communityId: rpcResponseObj["communityId"].getStr,
  ...
  proc onAsyncGetRevealedAccountsForAllMembersCompleted*(
      self: Service, response: string
  ) {.slot.} =
    try:
      let rpcResponseObj = response.parseJson
      ...
      let revealedAccounts =
        rpcResponseObj["response"]["result"].toMembersRevealedAccounts

      self.events.emit(
        SIGNAL_COMMUNITY_MEMBERS_REVEALED_ACCOUNTS_LOADED,
        CommunityMembersRevealedAccountsArgs(
          communityId: rpcResponseObj["communityId"].getStr,
  ...
  proc onAsyncReevaluateCommunityMembersPermissionsCompleted*(
      self: Service, response: string
  ) {.slot.} =
    try:
      let rpcResponseObj = response.parseJson
  ...
  proc onAsyncSetCommunityShardDone*(
      self: Service, communityIdAndRpcResponse: string
  ) {.slot.} =
    let rpcResponseObj = communityIdAndRpcResponse.parseJson
    try:
      ...
      self.events.emit(
        SIGNAL_COMMUNITY_SHARD_SET,
        CommunityShardSetArgs(communityId: rpcResponseObj["communityId"].getStr),
      )
    except Exception as e:
      error "Error setting community shard", msg = e.msg
      self.events.emit(
        SIGNAL_COMMUNITY_SHARD_SET_FAILED,
        CommunityShardSetArgs(communityId: rpcResponseObj["communityId"].getStr),
      )
  ...
  proc promoteSelfToControlNode*(self: Service, communityId: string) =
    try:
      ...
      if response.result == nil or response.result.kind == JNull or
          response.result["communities"].kind == JNull or
          response.result["communities"].len == 0:
        error "error: ",
          procName = "promoteSelfToControlNode", errDesription = "result is nil"
        return
  ...
  proc categoryHasUnreadMessages*(
      self: Service, communityId: string, categoryId: string
  ): bool =
    ...
    for chat in self.communities[communityId].chats:
      ...
      if (not chatDto.muted and chatDto.unviewedMessagesCount > 0) or
          chatDto.unviewedMentionsCount > 0:
        return true
    return false
