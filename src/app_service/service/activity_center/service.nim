import
  NimQml, json, sequtils, chronicles, strutils, strutils, stint, sugar, tables,
  json_serialization
...
const SIGNAL_ACTIVITY_CENTER_NOTIFICATIONS_COUNT_MAY_HAVE_CHANGED* =
  "activityCenterNotificationsCountMayChanged"
const SIGNAL_ACTIVITY_CENTER_UNSEEN_UPDATED* =
  "activityCenterNotificationsHasUnseenUpdated"
const SIGNAL_ACTIVITY_CENTER_MARK_NOTIFICATIONS_AS_READ* =
  "activityCenterMarkNotificationsAsRead"
const SIGNAL_ACTIVITY_CENTER_MARK_NOTIFICATIONS_AS_UNREAD* =
  "activityCenterMarkNotificationsAsUnread"
const SIGNAL_ACTIVITY_CENTER_MARK_ALL_NOTIFICATIONS_AS_READ* =
  "activityCenterMarkAllNotificationsAsRead"
const SIGNAL_ACTIVITY_CENTER_NOTIFICATIONS_REMOVED* =
  "activityCenterNotificationsRemoved"
const SIGNAL_ACTIVITY_CENTER_NOTIFICATIONS_ACCEPTED* =
  "activityCenterNotificationsAccepted"
const SIGNAL_ACTIVITY_CENTER_NOTIFICATIONS_DISMISSED* =
  "activityCenterNotificationsDismissed"
...
const ACTIVITY_GROUPS =
  @[
    ActivityCenterGroup.Mentions, ActivityCenterGroup.Replies,
    ActivityCenterGroup.Membership, ActivityCenterGroup.Admin,
    ActivityCenterGroup.ContactRequests, ActivityCenterGroup.IdentityVerification,
  ]
...
QtObject:
  ...
  proc newService*(
      events: EventEmitter, threadpool: ThreadPool, chatService: chat_service.Service
  ): Service =
    new(result, delete)
  ...
  proc handleNewNotificationsLoaded(
      self: Service, activityCenterNotifications: seq[ActivityCenterNotificationDto]
  ) =
    ...
    let filteredNotifications = filter(
      activityCenterNotifications,
      proc(notification: ActivityCenterNotificationDto): bool =
        return
          (self.readType == ActivityCenterReadType.All or not notification.read) and
          groupTypes.contains(notification.notificationType.int),
    )
    let removedNotifications = filter(
      activityCenterNotifications,
      proc(notification: ActivityCenterNotificationDto): bool =
    ...
    if (filteredNotifications.len > 0):
      self.events.emit(
        ...
        ActivityCenterNotificationsArgs(
          activityCenterNotifications: filteredNotifications
        ),
      )

    if (removedNotifications.len > 0):
      var notificationIds: seq[string] =
        removedNotifications.map(notification => notification.id)
      self.events.emit(
        SIGNAL_ACTIVITY_CENTER_NOTIFICATIONS_REMOVED,
        ActivityCenterNotificationIdsArgs(notificationIds: notificationIds),
      )
    # NOTE: this signal must fire even we have no new notifications to show
    self.events.emit(
      SIGNAL_ACTIVITY_CENTER_NOTIFICATIONS_COUNT_MAY_HAVE_CHANGED, Args()
    )
    ...
    self.events.on(SIGNAL_PARSE_RAW_ACTIVITY_CENTER_NOTIFICATIONS) do(e: Args):
      ...
      if raw.activityCenterNotifications.len > 0:
        ...
        for notificationJson in raw.activityCenterNotifications:
          activityCenterNotifications.add(
            notificationJson.toActivityCenterNotificationDto
          )
  ...
  proc parseActivityCenterState*(self: Service, response: RpcResponse) =
    ...
    if response.result.getProp("activityCenterState", activityCenterState):
      ...
      self.events.emit(
        SIGNAL_ACTIVITY_CENTER_UNSEEN_UPDATED,
        ActivityCenterNotificationHasUnseen(hasUnseen: not hasSeen),
      )
  ...
  proc getActivityCenterNotifications*(
    ...
    try:
      ...
      let activityCenterNotificationsTuple =
        parseActivityCenterNotifications(response.result)
  ...
  proc getActivityCenterNotificationsCounters(
      self: Service, activityTypes: seq[int], readType: ActivityCenterReadType
  ): Table[int, int] =
    try:
      let response = backend.activityCenterNotificationsCount(
        backend.ActivityCenterCountRequest(
          activityTypes: activityTypes, readType: readType.int
        )
      )
  ...
  proc getActivityGroupCounters*(self: Service): Table[ActivityCenterGroup, int] =
    ...
    let counters =
      self.getActivityCenterNotificationsCounters(allActivityTypes, self.readType)
  ...
  proc getUnreadActivityCenterNotificationsCount*(self: Service): int =
    ...
    let counters = self.getActivityCenterNotificationsCounters(
      activityTypes, ActivityCenterReadType.Unread
    )
    var total = 0
    for activityType in activityTypes:
  ...
  proc markActivityCenterNotificationRead*(self: Service, notificationId: string) =
    try:
      ...
      discard
        response.result.getProp("seenAndUnseenMessages", seenAndUnseenMessagesBatch)
      ...
      self.events.emit(
        SIGNAL_ACTIVITY_CENTER_MARK_NOTIFICATIONS_AS_READ,
        ActivityCenterNotificationIdsArgs(notificationIds: notificationIds),
      )
      self.events.emit(
        SIGNAL_ACTIVITY_CENTER_NOTIFICATIONS_COUNT_MAY_HAVE_CHANGED, Args()
  ...
  proc markActivityCenterNotificationUnread*(self: Service, notificationId: string) =
    try:
      ...
      self.parseActivityCenterState(response)
      self.events.emit(
        SIGNAL_ACTIVITY_CENTER_MARK_NOTIFICATIONS_AS_UNREAD,
        ActivityCenterNotificationIdsArgs(notificationIds: notificationIds),
      )
      self.events.emit(
        SIGNAL_ACTIVITY_CENTER_NOTIFICATIONS_COUNT_MAY_HAVE_CHANGED, Args()
    ...
    try:
      ...
      if seenAndUnseenMessagesBatch.len > 0:
        for seenAndUnseenMessagesRaw in seenAndUnseenMessagesBatch:
          ...
          let data = MessagesMarkedAsReadArgs(
            chatId: seenAndUnseenMessages.chatId, allMessagesMarked: true
          )
          self.events.emit(SIGNAL_MESSAGES_MARKED_AS_READ, data)
  ...
  proc asyncActivityNotificationLoaded*(self: Service, response: string) {.slot.} =
    try:
      ...
      if responseObj["activityNotifications"].kind != JNull:
        let activityCenterNotificationsTuple =
          parseActivityCenterNotifications(responseObj["activityNotifications"])
        ...
        self.events.emit(
          SIGNAL_ACTIVITY_CENTER_NOTIFICATIONS_LOADED,
          ActivityCenterNotificationsArgs(
            activityCenterNotifications: activityCenterNotificationsTuple[1]
          ),
        )
    except Exception as e:
      error "Error loading activity notification async", msg = e.msg

  proc deleteActivityCenterNotifications*(
      self: Service, notificationIds: seq[string]
  ): string =
    try:
  ...
  proc getNotificationForTypeAndCommunityId*(
      self: Service,
    ...
    for acNotification in acNotifications:
      if acNotification.notificationType == notificationType and
          acNotification.communityId == communityId:
  ...
  proc acceptActivityCenterNotification*(self: Service, notificationId: string) =
    try:
      ...
      self.parseActivityCenterState(response)
      self.events.emit(
        SIGNAL_ACTIVITY_CENTER_NOTIFICATIONS_ACCEPTED,
        ActivityCenterNotificationIdArgs(notificationId: notificationId),
      )
      self.events.emit(
        SIGNAL_ACTIVITY_CENTER_NOTIFICATIONS_COUNT_MAY_HAVE_CHANGED, Args()
      )
  ...
  proc dismissActivityCenterNotification*(self: Service, notificationId: string) =
    try:
      ...
      self.parseActivityCenterState(response)
      self.events.emit(
        SIGNAL_ACTIVITY_CENTER_NOTIFICATIONS_DISMISSED,
        ActivityCenterNotificationIdArgs(notificationId: notificationId),
      )
      self.events.emit(
        SIGNAL_ACTIVITY_CENTER_NOTIFICATIONS_COUNT_MAY_HAVE_CHANGED, Args()
