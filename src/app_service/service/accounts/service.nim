import NimQml, os, json, sequtils, strutils, uuids
...
const SIGNAL_SETUP_ACCOUNT_RESPONSE* = "setupAccountResponse"
const SIGNAL_LOGIN_ACCOUNT_RESPONSE* = "loginAccountResponse"

include utils
include async_tasks

type
  AccountsArgs* = ref object of Args
    error*: string
...
QtObject:
  ...
  proc onSavedAccount*(self: Service, response: string) {.slot.} =
    let responseObj = response.parseJson
    var error = "response doesn't contain \"error\""
    if(responseObj{"result"}.contains("error")):
      error = responseObj{"result"}{"error"}.getStr
      if error == "":
        debug "Account saved succesfully"
        self.isFirstTimeAccountLogin = true
        self.loggedInAccount = toAccountDto(responseObj{"account"})
        self.setLocalAccountSettingsFile()
        if not self.getLoggedInAccount.isValid():
          error "logged in account is not valid"
          self.events.emit(SIGNAL_SETUP_ACCOUNT_RESPONSE, AccountsArgs(error: error))
        return
    let err = "Error saving account and logging in: " & error
    error "error: ", procName="onSavedAccount", errDesription = err

  proc saveAccountAndLogin(self: Service, hashedPassword: string, account, subaccounts, settings, config: JsonNode) =
    let arg = AsyncSetupAccountTaskArg(
        tptr: cast[ByteAddress](asyncSetupAccountTask),
        vptr: cast[ByteAddress](self.vptr),
        slot: "onSavedAccount",
        hashedPassword: hashedPassword,
        accountObj: account,
        subaccountsObj: subaccounts,
  ...
  proc setupAccount*(self: Service, accountId, password, displayName: string): string =
    try:
      ...
      self.saveAccountAndLogin(hashedPassword, accountDataJson, subaccountDataJson, settingsJson, nodeConfigJson)
    except Exception as e:
  ...
  proc onLogin*(self: Service, response: string) {.slot.} =
    let responseObj = response.parseJson
    var error = "response doesn't contain \"error\""
    if(responseObj{"result"}.contains("error")):
      error = responseObj{"result"}{"error"}.getStr
      if error == "":
        ...
        # self.loggedInAccount = toAccountDto(responseObj{"account"})
        self.setLocalAccountSettingsFile()
        if not self.loggedInAccount.isValid():
          error "logged in account is not valid"
          self.events.emit(SIGNAL_LOGIN_ACCOUNT_RESPONSE, AccountsArgs(error: error))
        return
    let err = "Error saving account and logging in: " & error
    error "error: ", procName="onLogin", errDesription = err
  ...
  proc login*(self: Service, account: AccountDto, password: string) =
    try:
      ...
      let arg = AsyncLoginAccountTaskArg(
        tptr: cast[ByteAddress](asyncLoginAccountTask),
        vptr: cast[ByteAddress](self.vptr),
        slot: "onLogin",
        hashedPassword: hashedPassword,
        accountObj: toJsonNode(account),
