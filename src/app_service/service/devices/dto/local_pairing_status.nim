...
type PairingType* {.pure.} = enum
  AppSync = 0
  KeypairSync

type LocalPairingState* {.pure.} = enum
  Idle = 0
  Transferring
  Error
  Finished

type LocalPairingMode* {.pure.} = enum
  Idle = 0
  Sender
  Receiver

type LocalPairingStatus* = ref object of Args
...
proc newLocalPairingStatus*(
    pairingType: PairingType, mode: LocalPairingMode
): LocalPairingStatus =
  new(result, delete)
...
proc update*(self: LocalPairingStatus, data: LocalPairingEventArgs) =
  ...
  if self.mode == LocalPairingMode.Sender and data.eventType == EventProcessSuccess:
