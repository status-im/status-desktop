...
const
  ...
  BUILD_INFURA_TOKEN =
    getEnv(BUILD_TIME_PREFIX & BASE_NAME_INFURA_TOKEN, DEFAULT_INFURA_TOKEN)
const BUILD_INFURA_TOKEN_SECRET =
  getEnv(BUILD_TIME_PREFIX & BASE_NAME_INFURA_TOKEN_SECRET)
const
  DEFAULT_POKT_TOKEN = "849214fd2f85acead08f5184"
  BUILD_POKT_TOKEN =
    getEnv(BUILD_TIME_PREFIX & BASE_NAME_POKT_TOKEN, DEFAULT_POKT_TOKEN)
const
  DEFAULT_MIXPANEL_APP_ID = "3350627"
  BUILD_MIXPANEL_APP_ID =
    getEnv(BUILD_TIME_PREFIX & BASE_NAME_MIXPANEL_APP_ID, DEFAULT_MIXPANEL_APP_ID)
  DEFAULT_MIXPANEL_TOKEN = "5c73bda2d36a9f688a5ee45641fb6775"
  BUILD_MIXPANEL_TOKEN =
    getEnv(BUILD_TIME_PREFIX & BASE_NAME_MIXPANEL_TOKEN, DEFAULT_MIXPANEL_TOKEN)
  BUILD_SENTRY_DSN_STATUS_GO =
    getEnv(BUILD_TIME_PREFIX & BASE_NAME_SENTRY_DSN_STATUS_GO, "")
  BUILD_SENTRY_DSN_STATUS_DESKTOP =
    getEnv(BUILD_TIME_PREFIX & BASE_NAME_SENTRY_DSN_STATUS_DESKTOP, "")
const BUILD_OPENSEA_API_KEY = getEnv(BUILD_TIME_PREFIX & BASE_NAME_OPENSEA_API_KEY)
const BUILD_RARIBLE_MAINNET_API_KEY =
  getEnv(BUILD_TIME_PREFIX & BASE_NAME_RARIBLE_MAINNET_API_KEY)
const BUILD_RARIBLE_TESTNET_API_KEY =
  getEnv(BUILD_TIME_PREFIX & BASE_NAME_RARIBLE_TESTNET_API_KEY)
const BUILD_ALCHEMY_ETHEREUM_MAINNET_TOKEN =
  getEnv(BUILD_TIME_PREFIX & BASE_NAME_ALCHEMY_ETHEREUM_MAINNET_TOKEN)
const BUILD_ALCHEMY_ETHEREUM_SEPOLIA_TOKEN =
  getEnv(BUILD_TIME_PREFIX & BASE_NAME_ALCHEMY_ETHEREUM_SEPOLIA_TOKEN)
const BUILD_ALCHEMY_ARBITRUM_MAINNET_TOKEN =
  getEnv(BUILD_TIME_PREFIX & BASE_NAME_ALCHEMY_ARBITRUM_MAINNET_TOKEN)
const BUILD_ALCHEMY_ARBITRUM_SEPOLIA_TOKEN =
  getEnv(BUILD_TIME_PREFIX & BASE_NAME_ALCHEMY_ARBITRUM_SEPOLIA_TOKEN)
const BUILD_ALCHEMY_OPTIMISM_MAINNET_TOKEN =
  getEnv(BUILD_TIME_PREFIX & BASE_NAME_ALCHEMY_OPTIMISM_MAINNET_TOKEN)
const BUILD_ALCHEMY_OPTIMISM_SEPOLIA_TOKEN =
  getEnv(BUILD_TIME_PREFIX & BASE_NAME_ALCHEMY_OPTIMISM_SEPOLIA_TOKEN)

const
  DEFAULT_STATUS_PROXY_STAGE_NAME = "test"
  BUILD_STATUS_PROXY_STAGE_NAME = getEnv(
    BUILD_TIME_PREFIX & BASE_NAME_STATUS_PROXY_STAGE_NAME,
    DEFAULT_STATUS_PROXY_STAGE_NAME,
  )
const BUILD_STATUS_PROXY_USER = getEnv(BUILD_TIME_PREFIX & BASE_NAME_STATUS_PROXY_USER)
const BUILD_STATUS_PROXY_PASSWORD =
  getEnv(BUILD_TIME_PREFIX & BASE_NAME_STATUS_PROXY_PASSWORD)

const
  DEFAULT_TENOR_API_KEY = "DU7DWZ27STB2"
  BUILD_TENOR_API_KEY =
    getEnv(BUILD_TIME_PREFIX & BASE_NAME_TENOR_API_KEY, DEFAULT_TENOR_API_KEY)
const
  WALLET_CONNECT_STATUS_PROJECT_ID = "87815d72a81d739d2a7ce15c2cfdefb3"
  BUILD_WALLET_CONNECT_PROJECT_ID = getEnv(
    BUILD_TIME_PREFIX & BASE_NAME_WALLET_CONNECT_PROJECT_ID,
    WALLET_CONNECT_STATUS_PROJECT_ID,
  )
...
type StatusDesktopConfig = object # runtime counterparts vars of build vars
  infuraToken* {.
  ...
  alchemyEthereumMainnetToken* {.
    ...
    abbr: "alchemy-ethereum-mainnet-token"
  .}: string
  alchemyEthereumSepoliaToken* {.
    ...
    abbr: "alchemy-ethereum-sepolia-token"
  .}: string
  alchemyArbitrumMainnetToken* {.
    ...
    abbr: "alchemy-arbitrum-mainnet-token"
  .}: string
  alchemyArbitrumSepoliaToken* {.
    ...
    abbr: "alchemy-arbitrum-sepolia-token"
  .}: string
  alchemyOptimismMainnetToken* {.
    ...
    abbr: "alchemy-optimism-mainnet-token"
  .}: string
  alchemyOptimismSepoliaToken* {.
    ...
    abbr: "alchemy-optimism-sepolia-token"
  .}: string
  tenorApiKey* {.
  ...
  statusProxyStageName* {.
    ...
    abbr: "status-proxy-stage-name"
  .}: string
  statusProxyUser* {.
    defaultValue: BUILD_STATUS_PROXY_USER,
    desc: "Sets status proxy username",
    name: $BASE_NAME_STATUS_PROXY_USER,
    abbr: "status-proxy-user"
  .}: string
  statusProxyPassword* {.
    defaultValue: BUILD_STATUS_PROXY_PASSWORD,
    desc: "Sets status proxy password",
    name: $BASE_NAME_STATUS_PROXY_PASSWORD,
    abbr: "status-proxy-password"
  .}: string

  # runtime vars
  dataDir* {.
    defaultValue: defaultDataDir(), desc: "Status Desktop data directory", abbr: "d"
  .}: string
  uri* {.
    defaultValue: "", desc: "status-app:// URI to open a chat or other", name: "uri"
  .}: string
  testMode* {.
    defaultValue: false,
  ...
  defaultTorentConfigPort* {.
    ...
    abbr: "default-torrent-config-port"
  .}: int
  logLevel* {.
    defaultValue: DEFAULT_LOG_LEVEL,
    desc: "Sets log level",
    longdesc:
      "Can be one of: \"ERROR\", \"WARN\", \"INFO\", \"DEBUG\", \"TRACE\". \"INFO\" in production build, otherwise \"DEBUG\"",
    name: "LOG_LEVEL",
    abbr: "log-level"
  .}: string
...
if defined(macosx):
  cliParams.keepIf(
    proc(p: string): bool =
      not p.startsWith("-psn_")
  )

let desktopConfig =
  StatusDesktopConfig.load(cmdLine = cliParams, envVarsPrefix = RUN_TIME_PREFIX)
