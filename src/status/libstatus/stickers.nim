...
import json, json_serialization, tables, chronicles, strutils
...
proc getInstalledStickers*(): seq[StickerPack] =
  let setting = settings.getSetting("stickers/packs-installed").parseJson
  result = newSeq[StickerPack]()
  for i in setting.keys:
    result.add(Json.decode($(setting[i]), StickerPack))
