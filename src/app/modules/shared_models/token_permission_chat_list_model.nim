...
QtObject:
  ...
  method roleNames(self: TokenPermissionChatListModel): Table[int, string] =
    {ModelRole.Key.int: "key", ModelRole.ChannelName.int: "channelName"}.toTable
  ...
  method data(
      self: TokenPermissionChatListModel, index: QModelIndex, role: int
  ): QVariant =
  ...
  proc setItems*(
      self: TokenPermissionChatListModel, items: seq[TokenPermissionChatListItem]
  ) =
  ...
  proc renameChatById*(
      self: TokenPermissionChatListModel, chatId: string, newName: string
  ) =
