...
proc newModule*[T](
  ...
  result.controller = controller.newController(
    result, events, generalService, accountsService, devicesService, keycardServiceV2
  )
...
method inputConnectionStringForBootstrapping*[T](
    self: Module[T], connectionString: string
) =
  self.controller.inputConnectionStringForBootstrapping(connectionString)

method finishOnboardingFlow*[T](
    self: Module[T], primaryFlowInt, secondaryFlowInt: int, dataJson: string
): string =
  try:
    ...
    if primaryFlow == PrimaryFlow.CreateProfile:
      ...
      of SecondaryFlow.CreateProfileWithSeedphrase:
        err = self.controller.restoreAccountAndLogin(
          password, seedPhrase, recoverAccount = false, keycardInstanceUID = ""
        )
      ...
      of SecondaryFlow.CreateProfileWithKeycardExistingSeedphrase:
        ...
        raise newException(
          ValueError, "Unknown secondary flow for CreateProfile: " & $secondaryFlow
        )
    ...
    # LOGIN PRIMARY FLOW
      ...
      of SecondaryFlow.LoginWithSeedphrase:
        err = self.controller.restoreAccountAndLogin(
          password, seedPhrase, recoverAccount = true, keycardInstanceUID = ""
        )
      of SecondaryFlow.LoginWithSyncing:
        self.controller.inputConnectionStringForBootstrapping(
          data["connectionString"].str
        )
      of SecondaryFlow.LoginWithKeycard:
        ...
        raise newException(
          ValueError, "Unknown secondary flow for Login: " & $secondaryFlow
        )
      if err != "":
