...
const dummyUsage = account_constants.ZERO_ADDRESS
...
type AuthenticationReason {.pure.} = enum
  AddingAccount = 0
...
proc newModule*[T](
  ...
  result.controller = controller.newController(
    result, events, accountsService, walletAccountService, savedAddressService,
    keycardService,
  )
...
method loadForAddingAccount*[T](self: Module[T], addingWatchOnlyAccount: bool) =
  ...
  var items = keypairs.buildKeyPairsList(
...
method loadForEditingAccount*[T](self: Module[T], address: string) =
  ...
  var addressDetailsItem = newDerivedAddressItem(
    order = 0,
  ...
  if accountDto.walletType == WalletTypeWatch:
    ...
    if selectedOrigin.isNil or selectedOrigin.getKeyUid().len == 0:
      error "selected address for editing is not known among identified keypairs",
        address = address
      return
...
proc tryKeycardSync[T](self: Module[T]) =
  ...
  let dataForKeycardToSync = SharedKeycarModuleArgs(
    pin: self.controller.getPin(), keyUid: self.controller.getAuthenticatedKeyUid()
  )
...
proc getNumOfAddressesToGenerate[T](self: Module[T]): int =
  ...
  return
    self.controller.getNumOfAddressesToGenerateForKeypair(selectedOrigin.getKeyUid())
...
proc fetchAddressForDerivationPath[T](self: Module[T]) =
  ...
  if selectedOrigin.getPairType() == KeyPairType.Profile.int or
    ...
    if not self.isKeyPairAlreadyAdded(selectedOrigin.getKeyUid()):
      self.controller.fetchAddressesFromNotImportedSeedPhrase(
        self.controller.getSeedPhrase(), paths
      )
      return
...
method onUserAuthenticated*[T](
    self: Module[T], pin: string, password: string, keyUid: string
) =
  if password.len == 0:
  ...
  if self.authenticationReason == AuthenticationReason.AddingAccount:
    ...
    if selectedOrigin.getPairType() == KeyPairType.PrivateKeyImport.int:
      self.doAddAccount()
        # we're sure that we need to add an account from priv key from here, cause derivation is not possible for imported priv key
      return
...
proc isAuthenticationNeededForSelectedOrigin[T](self: Module[T]): bool =
  ...
  if selectedOrigin.getKeyUid() == self.controller.getAuthenticatedKeyUid():
    return false
  if not selectedOrigin.getMigratedToKeycard() and
      self.controller.getAuthenticatedKeyUid() ==
...
method changePrivateKey*[T](self: Module[T], privateKey: string) =
  ...
  if genAccDto.address.len == 0:
    ...
    return
  self.view.setScanningForActivityIsOngoing(true)
  self.view.setPrivateKeyAccAddress(
    newDerivedAddressItem(
      order = 0, address = genAccDto.address, publicKey = genAccDto.publicKey
    )
  )
        ...
        if d.path == path:
          items.add(
            newDerivedAddressItem(
              ...
              path = d.path,
              alreadyCreated = d.alreadyCreated,
            )
          )
          break
...
method onDerivedAddressesFetched*[T](
    self: Module[T], derivedAddresses: seq[DerivedAddressDto], error: string
) =
  if error.len > 0:
...
method onAddressesFromNotImportedMnemonicFetched*[T](
    self: Module[T], derivations: Table[string, DerivedAccountDetails], error: string
) =
  if error.len > 0:
...
method onDerivedAddressesFromKeycardFetched*[T](
    self: Module[T],
  ...
  for i in 0 ..< paths.len:
    ...
    derivedAddresses.add(
      DerivedAddressDto(
        address: keycardEvent.generatedWalletAccounts[i].address,
...
method onAddressDetailsFetched*[T](
    self: Module[T], derivedAddresses: seq[DerivedAddressDto], error: string
) =
  ...
  if derivedAddresses.len == 1:
    ...
    if currStateObj.stateType() == StateType.EnterPrivateKey:
      if cmpIgnoreCase(
        self.view.getPrivateKeyAccAddress().getAddress(),
        addressDetailsItem.getAddress(),
      ) == 0:
        self.view.setPrivateKeyAccAddress(addressDetailsItem)
        return
      ...
      if selectedOrigin.getPairType() == KeyPairType.Unknown.int and
          ...
          cmpIgnoreCase(
            self.view.getWatchOnlyAccAddress().getAddress(),
            addressDetailsItem.getAddress(),
          ) == 0:
        self.view.setWatchOnlyAccAddress(addressDetailsItem)
        return
      let selectedAddress = self.view.getSelectedDerivedAddress()
      if cmpIgnoreCase(selectedAddress.getAddress(), addressDetailsItem.getAddress()) ==
          0:
        addressDetailsItem.setPublicKey(selectedAddress.getPublicKey())
        addressDetailsItem.setPath(selectedAddress.getPath())
        self.view.setSelectedDerivedAddress(addressDetailsItem)
      self.view.derivedAddressModel().updateDetailsForAddressAndBubbleItToTop(
        addressDetailsItem.getAddress(), addressDetailsItem.getHasActivity()
      )
      return
    error "derived addresses received in the state in which the app doesn't expect them"
    return
  error "unknown error, since the length of the response is not expected",
    length = derivedAddresses.len
...
proc doAddAccount[T](self: Module[T]) =
  ...
  if selectedOrigin.getPairType() == KeyPairType.Profile.int:
    ...
    path = "m"
      # from private key an address for path `m` is generate (corresponds to the master key address)
    rootWalletMasterKey = ""
  ...
  if not savedAddressDto.isNil:
    self.view.sendConfirmSavedAddressRemovalSignal(
      savedAddressDto.name, savedAddressDto.address
    )
    return
  ...
  if addingNewKeyPair:
    if selectedOrigin.getPairType() == KeyPairType.PrivateKeyImport.int:
      ...
      if not success:
        error "failed to store new private key account",
          address = selectedAddrItem.getAddress()
    elif selectedOrigin.getPairType() == KeyPairType.SeedImport.int:
      ...
      if not success:
        error "failed to store new seed phrase account",
          address = selectedAddrItem.getAddress()
  else:
