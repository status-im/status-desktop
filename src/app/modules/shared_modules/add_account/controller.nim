...
const UNIQUE_WALLET_SECTION_ADD_ACCOUNTS_MODULE_IDENTIFIER* =
  "WalletSection-AddAccountsModule"
...
proc init*(self: Controller) =
  ...
  handlerId = self.events.onWithUUID(SIGNAL_WALLET_ACCOUNT_DERIVED_ADDRESSES_FETCHED) do(
    e: Args
  ):
  ...
  handlerId = self.events.onWithUUID(
    SIGNAL_WALLET_ACCOUNT_DERIVED_ADDRESSES_FROM_MNEMONIC_FETCHED
  ) do(e: Args):
    let args = DerivedAddressesArgs(e)
    self.delegate.onDerivedAddressesFromMnemonicFetched(
      args.derivedAddresses, args.error
    )
  self.connectionIds.add(handlerId)

  handlerId = self.events.onWithUUID(
    SIGNAL_DERIVED_ADDRESSES_FROM_NOT_IMPORTED_MNEMONIC_FETCHED
  ) do(e: Args):
    let args = DerivedAddressesFromNotImportedMnemonicArgs(e)
    self.delegate.onAddressesFromNotImportedMnemonicFetched(
      args.derivations, args.error
    )
  self.connectionIds.add(handlerId)
...
proc authenticateOrigin*(self: Controller, keyUid = "") =
  let data = SharedKeycarModuleAuthenticationArgs(
    uniqueIdentifier: UNIQUE_WALLET_SECTION_ADD_ACCOUNTS_MODULE_IDENTIFIER,
    keyUid: keyUid,
  )
...
proc fetchDerivedAddresses*(self: Controller, derivedFrom: string, paths: seq[string]) =
  ...
  if self.getPin().len > 0:
    hashPassword = false
  self.walletAccountService.fetchDerivedAddresses(
    self.getPassword(), derivedFrom, paths, hashPassword
  )
...
proc addWalletAccount*(
    self: Controller,
    createKeystoreFile, doPasswordHashing: bool,
  ...
  if createKeystoreFile:
    ...
    if password.len == 0:
      info "cannot create keystore file if provided password is empty",
        name = name, address = address
      return false
  let err = self.walletAccountService.addWalletAccount(
    password, doPasswordHashing, name, address, path, publicKey, keyUid, accountType,
...
proc addNewPrivateKeyKeypair*(
  ...
  if password.len == 0:
    ...
    return false
  let err = self.walletAccountService.addNewPrivateKeyKeypair(
    privateKey, password, doPasswordHashing, keyUid, keyPairName, rootWalletMasterKey,
    account,
  )
  if err.len > 0:
    info "adding new keypair from private key failed",
      keypairName = keypairName, keyUid = keyUid
    return false
  return true
...
proc addNewSeedPhraseKeypair*(
  ...
  let err = self.walletAccountService.addNewSeedPhraseKeypair(
    seedPhrase, password, doPasswordHashing, keyUid, keypairName, rootWalletMasterKey,
    accounts,
  )
  if err.len > 0:
...
proc fetchAddressesFromNotImportedSeedPhrase*(
...
proc connectKeycardReponseSignal(self: Controller) =
  self.connectionKeycardResponse = self.events.onWithUUID(SIGNAL_KEYCARD_RESPONSE) do(
    ...
    self.disconnectKeycardReponseSignal()
    self.delegate.onDerivedAddressesFromKeycardFetched(
      args.flowType, args.flowEvent, self.tmpPaths
    )
...
proc fetchAddressesFromKeycard*(self: Controller, bip44Paths: seq[string]) =
  ...
  self.keycardService.startExportPublicFlow(
    bip44Paths, exportMasterAddr = true, exportPrivateAddr = false, pin = self.getPin()
  )
