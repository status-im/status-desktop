proc ensureReaderAndCardPresence*(
  ...
  if keycardFlowType == ResponseTypeValueKeycardFlowResult and keycardEvent.error.len > 0 and
  ...
  if state.flowType == FlowType.FactoryReset or state.flowType == FlowType.Authentication or
      state.flowType == FlowType.Sign or state.flowType == FlowType.UnlockKeycard or
    ...
    if keycardFlowType == ResponseTypeValueKeycardFlowResult and
        ...
        (keycardEvent.error == ErrorNoReader or keycardEvent.error == ErrorReaderList):
    ...
    if keycardFlowType == ResponseTypeValueInsertCard and keycardEvent.error.len > 0 and
    ...
    if keycardFlowType == ResponseTypeValueCardInserted:
      controller.setKeycardData(
        updatePredefinedKeycardData(
          ...
          PredefinedKeycardData.WronglyInsertedCard,
          add = false,
        )
      )
  ...
  if state.flowType == FlowType.SetupNewKeycard:
    if keycardFlowType == ResponseTypeValueKeycardFlowResult and
        keycardEvent.error.len > 0 and (
      keycardEvent.error == ErrorConnection or keycardEvent.error == ErrorNoReader or
...
proc ensureReaderAndCardPresenceAndResolveNextState*(
  ...
  let ensureState =
    ensureReaderAndCardPresence(state, keycardFlowType, keycardEvent, controller)
  ...
  if state.flowType == FlowType.FactoryReset:
    ...
    if keycardFlowType == ResponseTypeValueEnterPUK and keycardEvent.error.len == 0:
      if keycardEvent.pinRetries == 0 and keycardEvent.pukRetries > 0:
        return createState(StateType.FactoryResetConfirmation, state.flowType, nil)
    if keycardFlowType == ResponseTypeValueSwapCard and keycardEvent.error.len > 0:
    ...
    if keycardFlowType == ResponseTypeValueEnterPairing and keycardEvent.error.len > 0:
    ...
    if keycardFlowType == ResponseTypeValueKeycardFlowResult:
      ...
      if keycardEvent.error.len == 0:
        if keycardEvent.cardMetadata.name.len > 0 and
            keycardEvent.cardMetadata.walletAccounts.len > 0:
  ...
  if state.flowType == FlowType.SetupNewKeycard:
    if keycardFlowType == ResponseTypeValueSwapCard and keycardEvent.error.len > 0:
      ...
      if keycardEvent.error == ErrorFreePairingSlots:
        controller.setKeycardData(
          updatePredefinedKeycardData(
            controller.getKeycardData(),
            PredefinedKeycardData.UseGeneralMessageForLockedState,
            add = true,
          )
        )
    ...
    if keycardFlowType == ResponseTypeValueEnterNewPIN and keycardEvent.error.len > 0 and
      ...
      return
        createState(StateType.RecognizedKeycard, state.flowType, state.getBackState)
  ...
  if state.flowType == FlowType.ImportFromKeycard:
    controller.setKeyPairForProcessing(newKeyPairItem(keyUid = keycardEvent.keyUid))
      # must set keypair in case of running some other flow which needs e.g. keyuid. like unlock flow
    ...
    if keycardFlowType == ResponseTypeValueEnterPIN and keycardEvent.error.len == 0:
      return createState(StateType.RecognizedKeycard, state.flowType, nil)
    if keycardFlowType == ResponseTypeValueEnterPUK and keycardEvent.error.len == 0:
      if keycardEvent.pinRetries == 0 and keycardEvent.pukRetries > 0:
        controller.setKeycardData(
    ...
    if keycardFlowType == ResponseTypeValueSwapCard and keycardEvent.error.len > 0:
      ...
      if keycardEvent.error == ErrorFreePairingSlots:
        ...
        controller.setKeycardData(
          updatePredefinedKeycardData(
            controller.getKeycardData(),
            PredefinedKeycardData.DisableSeedPhraseForUnlock,
            add = true,
          )
        )
        return createState(StateType.MaxPairingSlotsReached, state.flowType, nil)
    ...
    if keycardFlowType == ResponseTypeValueEnterPIN:
      if keycardEvent.keyUid == controller.getKeyUidWhichIsBeingAuthenticating():
        if singletonInstance.userProfile.getUsingBiometricLogin():
          if keycardEvent.error.len > 0 and keycardEvent.error == ErrorPIN:
  ...
  if state.flowType == FlowType.ChangePairingCode:
    ...
    if keycardFlowType == ResponseTypeValueKeycardFlowResult:
      ...
      if keycardEvent.error.len == 0:
        return
          createState(StateType.ChangingKeycardPairingCodeSuccess, state.flowType, nil)
      ...
      if state.stateType == StateType.ChangingKeycardPairingCode:
        return
          createState(StateType.ChangingKeycardPairingCodeFailure, state.flowType, nil)
  ...
  if state.flowType == FlowType.CreateCopyOfAKeycard:
    if isPredefinedKeycardDataFlagSet(
      ...
      if controller.getKeycardUid().len > 0 and
          controller.getKeycardUid() == keycardEvent.instanceUID:
      ...
      if keycardFlowType == ResponseTypeValueEnterMnemonic and keycardEvent.error.len > 0 and
  ...
  if state.flowType == FlowType.MigrateFromAppToKeycard:
    if keycardEvent.keyUid.len > 0 and
        keycardEvent.keyUid != controller.getKeyPairForProcessing().getKeyUid():
      return createState(StateType.WrongKeycard, state.flowType, nil)
    if keycardFlowType == ResponseTypeValueSwapCard and keycardEvent.error.len > 0:
      if keycardEvent.error == ErrorNotAKeycard:
        return createState(StateType.NotKeycard, state.flowType, nil)
...
proc readingKeycard*(
  ...
  if state.flowType == FlowType.UnlockKeycard or state.flowType == FlowType.RenameKeycard or
      state.flowType == FlowType.ChangeKeycardPin or
      state.flowType == FlowType.ChangeKeycardPuk or
      state.flowType == FlowType.ChangePairingCode or
      state.flowType == FlowType.MigrateFromAppToKeycard or (
    state.flowType == FlowType.CreateCopyOfAKeycard and
    not isPredefinedKeycardDataFlagSet(
      controller.getKeycardData(), PredefinedKeycardData.CopyFromAKeycardPartDone
    )
  ) or
    ...
    let ensureKeycardPresenceState =
      ensureReaderAndCardPresence(state, keycardFlowType, keycardEvent, controller)
    if ensureKeycardPresenceState.isNil: # means the keycard is inserted
      let nextState = ensureReaderAndCardPresenceAndResolveNextState(
        state, keycardFlowType, keycardEvent, controller
      )
