type MaxPinRetriesReachedState* = ref object of State

proc newMaxPinRetriesReachedState*(
    flowType: FlowType, backState: State
): MaxPinRetriesReachedState =
...
method executePrePrimaryStateCommand*(
    self: MaxPinRetriesReachedState, controller: Controller
) =
  if self.flowType == FlowType.MigrateFromAppToKeycard:
    controller.terminateCurrentFlow(
      lastStepInTheCurrentFlow = true,
      nextFlow = FlowType.UnlockKeycard,
      forceFlow = controller.getForceFlow(),
      nextKeyUid = controller.getKeyPairForProcessing().getKeyUid(),
      returnToFlow = FlowType.MigrateFromAppToKeycard,
    )
...
method getNextPrimaryState*(
  ...
  if self.flowType == FlowType.RenameKeycard or
    ...
    controller.runSharedModuleFlow(
      FlowType.UnlockKeycard, controller.getKeyPairForProcessing().getKeyUid()
    )
...
method executeCancelCommand*(self: MaxPinRetriesReachedState, controller: Controller) =
  if self.flowType == FlowType.Authentication or self.flowType == FlowType.Sign or
