type CreatingAccountOldSeedPhraseState* = ref object of State
  paths: seq[string]
  addresses: seq[string]

proc newCreatingAccountOldSeedPhraseState*(
    flowType: FlowType, backState: State
): CreatingAccountOldSeedPhraseState =
...
proc resolveAddresses(
    self: CreatingAccountOldSeedPhraseState,
...
proc addAccountsToWallet(
    self: CreatingAccountOldSeedPhraseState, controller: Controller
): bool =
  let kpForProcessing = controller.getKeyPairForProcessing()
...
proc doMigration(self: CreatingAccountOldSeedPhraseState, controller: Controller) =
  ...
  var kpDto = KeycardDto(
    keycardUid: controller.getKeycardUid(),
...
proc runStoreMetadataFlow(
  ...
  controller.runStoreMetadataFlow(
    kpForProcessing.getName(), controller.getPin(), self.paths
  )
...
method getNextTertiaryState*(
  ...
  if self.flowType == FlowType.SetupNewKeycardOldSeedPhrase:
    if not controller.getAddingMigratedKeypairSuccess():
      return
        createState(StateType.CreatingAccountOldSeedPhraseFailure, self.flowType, nil)
  ...
  if self.flowType == FlowType.SetupNewKeycardOldSeedPhrase:
    if controller.getCurrentKeycardServiceFlow() == KCSFlowType.ExportPublic:
      if keycardFlowType == ResponseTypeValueKeycardFlowResult and
        ...
        if not self.resolveAddresses(controller, keycardEvent):
          return createState(
            StateType.CreatingAccountOldSeedPhraseFailure, self.flowType, nil
          )
    ...
    if controller.getCurrentKeycardServiceFlow() == KCSFlowType.StoreMetadata:
      if keycardFlowType == ResponseTypeValueKeycardFlowResult and
          keycardEvent.error.len == 0:
        return
          createState(StateType.CreatingAccountOldSeedPhraseSuccess, self.flowType, nil)
      return
        createState(StateType.CreatingAccountOldSeedPhraseFailure, self.flowType, nil)
