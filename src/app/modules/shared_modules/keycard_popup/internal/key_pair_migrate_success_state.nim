type KeyPairMigrateSuccessState* = ref object of State

proc newKeyPairMigrateSuccessState*(
    flowType: FlowType, backState: State
): KeyPairMigrateSuccessState =
...
method executeCancelCommand*(self: KeyPairMigrateSuccessState, controller: Controller) =
  ...
  if self.flowType == FlowType.MigrateFromKeycardToApp:
    if controller.getKeyPairForProcessing().getKeyUid() ==
...
method executePrePrimaryStateCommand*(
    self: KeyPairMigrateSuccessState, controller: Controller
) =
  if self.flowType == FlowType.SetupNewKeycard:
    let profileMigrated = controller.getSelectedKeyPairIsProfile()
    controller.terminateCurrentFlow(lastStepInTheCurrentFlow = true)
...
method executePreSecondaryStateCommand*(
  ...
  if self.flowType == FlowType.MigrateFromKeycardToApp:
    ...
    controller.terminateCurrentFlow(
      ...
      nextFlow = FlowType.FactoryReset,
