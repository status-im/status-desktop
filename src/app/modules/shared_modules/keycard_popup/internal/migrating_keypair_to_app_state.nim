type MigratingKeypairToAppState* = ref object of State
  migrationOk: bool

proc newMigratingKeypairToAppState*(
    flowType: FlowType, backState: State
): MigratingKeypairToAppState =
...
method executePrePrimaryStateCommand*(
    self: MigratingKeypairToAppState, controller: Controller
) =
  if self.flowType == FlowType.MigrateFromKeycardToApp:
    ...
    if migratingProfile:
      ...
      if singletonInstance.localAccountSettings.getStoreToKeychainValue() ==
          LS_VALUE_STORE:
        controller.tryToStoreDataToKeychain(newPassword)
      controller.convertKeycardProfileKeypairToRegular(sp, password, newPassword)
      return
    controller.migrateNonProfileKeycardKeypairToApp(
      kpForProcessing.getKeyUid(),
      sp,
      password,
