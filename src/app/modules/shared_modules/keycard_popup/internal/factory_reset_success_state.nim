type FactoryResetSuccessState* = ref object of State

proc newFactoryResetSuccessState*(
    flowType: FlowType, backState: State
): FactoryResetSuccessState =
...
method executePrePrimaryStateCommand*(
    self: FactoryResetSuccessState, controller: Controller
) =
  if self.flowType == FlowType.FactoryReset:
    controller.terminateCurrentFlow(lastStepInTheCurrentFlow = true)
  elif self.flowType == FlowType.SetupNewKeycard:
    controller.setSelectedKeypairAsKeyPairForProcessing()
      # we need to update keypair for next state (e.g. if user run into factory reset for example)
    ...
    controller.setMetadataFromKeycard(controller.getMetadataForKeycardCopy())
      # we need to update keypair for next state
    controller.runLoadAccountFlow(
      seedPhraseLength = 0, seedPhrase = "", pin = controller.getPinForKeycardCopy()
    )
