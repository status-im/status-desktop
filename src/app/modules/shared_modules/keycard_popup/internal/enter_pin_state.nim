type EnterPinState* = ref object of State
...
method getNextPrimaryState*(self: EnterPinState, controller: Controller): State =
  if self.flowType == FlowType.DisplayKeycardContent or
      self.flowType == FlowType.FactoryReset or self.flowType == FlowType.SetupNewKeycard or
  ...
  if self.flowType == FlowType.CreateCopyOfAKeycard:
    if isPredefinedKeycardDataFlagSet(
      ...
      return createState(StateType.FactoryResetConfirmation, self.flowType, self)
  if self.flowType == FlowType.Authentication or self.flowType == FlowType.Sign:
...
method executeCancelCommand*(self: EnterPinState, controller: Controller) =
  if self.flowType == FlowType.FactoryReset or self.flowType == FlowType.SetupNewKeycard or
      ...
      self.flowType == FlowType.SetupNewKeycardOldSeedPhrase or
      self.flowType == FlowType.ImportFromKeycard or
      self.flowType == FlowType.Authentication or self.flowType == FlowType.Sign or
      self.flowType == FlowType.DisplayKeycardContent or
      self.flowType == FlowType.RenameKeycard or
      self.flowType == FlowType.ChangeKeycardPin or
      self.flowType == FlowType.ChangeKeycardPuk or
...
method resolveKeycardNextState*(
  ...
  let state =
    ensureReaderAndCardPresence(self, keycardFlowType, keycardEvent, controller)
  if not state.isNil:
    return state
  if self.flowType == FlowType.FactoryReset:
    if keycardFlowType == ResponseTypeValueEnterPIN and keycardEvent.error.len > 0 and
  ...
  if self.flowType == FlowType.ImportFromKeycard:
    ...
    if keycardFlowType == ResponseTypeValueKeycardFlowResult:
      ...
      let accountItem =
        controller.getKeyPairHelper().getAccountsModel().getItemAtIndex(0)
  ...
  if self.flowType == FlowType.Authentication or self.flowType == FlowType.Sign:
    if keycardFlowType == ResponseTypeValueEnterPIN and keycardEvent.error.len > 0 and
      ...
      controller.setRemainingAttempts(keycardEvent.pinRetries)
      if keycardEvent.pinRetries > 0:
        if singletonInstance.userProfile.getUsingBiometricLogin() and
            not controller.usePinFromBiometrics():
  ...
  if self.flowType == FlowType.RenameKeycard:
    if keycardFlowType == ResponseTypeValueEnterPIN and keycardEvent.error.len > 0 and
      ...
      controller.setKeycardData(
        updatePredefinedKeycardData(
          controller.getKeycardData(), PredefinedKeycardData.HideKeyPair, add = true
        )
      )
      return createState(StateType.MaxPinRetriesReached, self.flowType, nil)
    if keycardFlowType == ResponseTypeValueEnterPIN and keycardEvent.error.len == 0:
