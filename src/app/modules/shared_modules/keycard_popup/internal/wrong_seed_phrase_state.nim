type WrongSeedPhraseState* = ref object of State
  verifiedSeedPhrase: bool

proc newWrongSeedPhraseState*(
    flowType: FlowType, backState: State
): WrongSeedPhraseState =
...
method executePreBackStateCommand*(self: WrongSeedPhraseState, controller: Controller) =
  controller.setKeycardData(
    updatePredefinedKeycardData(
      controller.getKeycardData(), PredefinedKeycardData.WrongSeedPhrase, add = false
    )
  )

method executePrePrimaryStateCommand*(
    self: WrongSeedPhraseState, controller: Controller
) =
  controller.setKeycardData(
    updatePredefinedKeycardData(
      controller.getKeycardData(), PredefinedKeycardData.WrongSeedPhrase, add = false
  ...
  if self.flowType == FlowType.SetupNewKeycard:
    ...
    self.verifiedSeedPhrase =
      controller.validSeedPhrase(sp) and
      keyUid == controller.getSelectedKeyPairDto().keyUid
  ...
  if self.flowType == FlowType.UnlockKeycard:
    ...
    if not self.verifiedSeedPhrase:
      controller.setKeycardData(
        updatePredefinedKeycardData(
          controller.getKeycardData(), PredefinedKeycardData.WrongSeedPhrase, add = true
        )
      )
...
method getNextPrimaryState*(self: WrongSeedPhraseState, controller: Controller): State =
  if self.flowType == FlowType.SetupNewKeycard or
    ...
    if migratingProfile:
      let backState =
        findBackStateWithTargetedStateType(self, StateType.EnterSeedPhrase)
...
method executeCancelCommand*(self: WrongSeedPhraseState, controller: Controller) =
  if self.flowType == FlowType.SetupNewKeycard or self.flowType == FlowType.UnlockKeycard or
      self.flowType == FlowType.CreateCopyOfAKeycard or
      self.flowType == FlowType.MigrateFromKeycardToApp:
    ...
    if keycardFlowType == ResponseTypeValueKeycardFlowResult and
      ...
      controller.setKeycardData(
        updatePredefinedKeycardData(
          ...
          PredefinedKeycardData.WrongSeedPhrase,
          add = false,
        )
      )
