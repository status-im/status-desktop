type ManageKeycardAccountsState* = ref object of State
  success: bool

proc newManageKeycardAccountsState*(
    flowType: FlowType, backState: State
): ManageKeycardAccountsState =
...
method getNextPrimaryState*(
    self: ManageKeycardAccountsState, controller: Controller
): State =
  ...
  if self.flowType == FlowType.ImportFromKeycard:
    let numOfProcessedAccounts =
      controller.getKeyPairForProcessing().getAccountsModel().getCount()
    let totalNumOfAccountsToBeProcessed =
      controller.getKeyPairHelper().getAccountsModel().getCount()
    if numOfProcessedAccounts < totalNumOfAccountsToBeProcessed:
      let accountItem = controller.getKeyPairHelper().getAccountsModel().getItemAtIndex(
          numOfProcessedAccounts
        ) # numOfProcessedAccounts is index of next acc which need to be processed
      if accountItem.isNil:
        # should never be here
        return createState(StateType.ImportingFromKeycardFailure, self.flowType, nil)
      controller.getKeyPairForProcessing().addAccount(
