...
QtObject:
  type KeycardModel* = ref object of QAbstractListModel
  ...
  method roleNames(self: KeycardModel): Table[int, string] =
    {ModelRole.Keycard.int: "keycard"}.toTable
  ...
  proc setLockedForKeycardWithKeycardUid*(
      self: KeycardModel, keycardUid: string, locked: bool
  ) =
    for i in 0 ..< self.items.len:
      if (self.items[i].getKeycardUid() == keycardUid):
  ...
  proc setLockedForKeycardsWithKeyUid*(
      self: KeycardModel, keyUid: string, locked: bool
  ) =
    for i in 0 ..< self.items.len:
      if (self.items[i].getKeyUid() == keyUid):
        self.items[i].setLocked(locked)
  ...
  proc setNameForKeycardWithKeycardUid*(
      self: KeycardModel, keycardUid: string, name: string
  ) =
    for i in 0 ..< self.items.len:
      if (self.items[i].getKeycardUid() == keycardUid):
  ...
  proc removeAccountsFromKeycardWithKeycardUid*(
      self: KeycardModel,
      keycardUid: string,
    ...
    for i in 0 ..< self.items.len:
      if (self.items[i].getKeycardUid() == keycardUid):
        ...
        if removeKeycardItemIfHasNoAccounts and
            self.items[i].getAccountsModel().getCount() == 0:
          self.removeItem(i)

  proc removeAccountsFromKeycardsWithKeyUid*(
      self: KeycardModel,
      keyUid: string,
      accountsToRemove: seq[string],
      removeKeycardItemIfHasNoAccounts: bool,
