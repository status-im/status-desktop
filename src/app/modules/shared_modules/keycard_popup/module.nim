...
method onTokensRebuilt*[T](self: Module[T], accountAddresses: seq[string], groupedTokens: seq[GroupedTokenItem]) =
  ...
  for address in accountAddresses:
    let accAdd = address
    for gToken in groupedTokens:
      let filteredBalances = gToken.balancesPerAccount.filter(b =>  b.account == accAdd)
      for balance in filteredBalances:
          totalTokenBalance += self.controller.parseCurrencyValue(gToken.key, balance.balance)
