...
proc newModule*[T](
  ...
  result.controller = controller.newController(
    result, events, accountsService, walletAccountService, devicesService
  )
...
method load*[T](self: Module[T], keyUid: string, mode: ImportKeypairModuleMode) =
  ...
  if mode == ImportKeypairModuleMode.SelectKeypair:
    ...
    if keypair.isNil:
      ...
      return
    let keypairItem = buildKeypairItem(keypair, areTestNetworksEnabled = false)
      # testnetworks are irrelevant in this context
  ...
  if derivedAddresses.len == 1:
    ...
    if currStateObj.stateType() == StateType.ImportPrivateKey:
      if cmpIgnoreCase(
        ...
        return
  error "ki_unknown error, since the length of the response is not expected",
...
method generateConnectionStringForExporting*[T](self: Module[T]) =
  ...
  if currStateObj.stateType() == StateType.ExportKeypair:
    ...
    if keyUid.len > 0:
      keyUids.add(keyUid)
    let (connectionString, err) = self.controller.generateConnectionStringForExportingKeypairsKeystores(
      keyUids, self.tmpPassword
    )
    if err.len > 0:
...
method onUserAuthenticated*[T](
  ...
  if currStateObj.stateType() == StateType.ImportQr:
    ...
    if keyUid.len > 0:
      keyUids.add(keyUid)
    let res = self.controller.inputConnectionStringForImportingKeypairsKeystores(
      keyUids, self.view.getConnectionString(), password
    )
    if res.len > 0:
      error "ki_unable to make a keypair operable", errDesription = res
      return
  if currStateObj.stateType() == StateType.ImportPrivateKey:
    let res = self.controller.makePrivateKeyKeypairFullyOperable(
      self.controller.getGeneratedAccount().keyUid,
  ...
  if currStateObj.stateType() == StateType.ImportSeedPhrase:
    let res = self.controller.makeSeedPhraseKeypairFullyOperable(
      self.controller.getGeneratedAccount().keyUid,
