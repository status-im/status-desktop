...
proc signBuyingStickersTxLocally*(self: Controller, password: string): string =
  return self.stickerService.signBuyingStickersTxLocally(password)

proc sendBuyingStickersTxWithSignatureAndWatch*(self: Controller, chainId: int, packId: string, fromAddress: string,
  signature: string): StickerBuyResultArgs =
  return self.stickerService.sendBuyingStickersTxWithSignatureAndWatch(chainId, packId, fromAddress, signature)
