...
type StickerRoles {.pure.} = enum
  Url = UserRole + 1
  Hash = UserRole + 2
  PackId = UserRole + 3

QtObject:
  type StickerList* = ref object of QAbstractListModel
  ...
  proc newStickerList*(
      delegate: io_interface.AccessInterface, stickers: seq[Item] = @[]
  ): StickerList =
    new(result, delete)
  ...
  method rowCount(self: StickerList, index: QModelIndex = nil): int =
    self.stickers.len
  ...
  method data(self: StickerList, index: QModelIndex, role: int): QVariant =
    ...
    case stickerRole
    of StickerRoles.Url:
      result = newQVariant(sticker.getURL)
    of StickerRoles.Hash:
      result = newQVariant(sticker.getHash)
    of StickerRoles.PackId:
      result = newQVariant(sticker.getPackId)
  ...
  proc addStickerToList*(self: StickerList, sticker: Item) =
    if (
      self.stickers.any(
        proc(existingSticker: Item): bool =
          return existingSticker.getHash == sticker.getHash
      )
    ):
      return
