...
QtObject:
  ...
  proc updateItemDetails*(self: Model, id, name, icon: string, isIdenticon: bool, trustStatus: TrustStatus) =
    ...
    for i in 0 ..< self.items.len:
      if(self.items[i].id == id):
        ...
        let index = self.createIndex(i, 0, nil)
        self.dataChanged(index, index, @[ModelRole.Name.int, ModelRole.Icon.int, ModelRole.IsIdenticon.int, ModelRole.TrustStatus.int])
        return
