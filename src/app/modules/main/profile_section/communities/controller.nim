...
proc newController*(
    delegate: io_interface.AccessInterface, communityService: community_service.Service
): Controller =
  result = Controller()
...
proc shareCommunityToUsers*(
    self: Controller, communityID: string, pubKeys: string, inviteMessage: string
): string =
  result =
    self.communityService.shareCommunityToUsers(communityID, pubKeys, inviteMessage)
