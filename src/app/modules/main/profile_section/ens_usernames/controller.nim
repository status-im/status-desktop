...
proc init*(self: Controller) =
  ...
  self.events.on(SIGNAL_ENS_USERNAME_DETAILS_FETCHED) do(e: Args):
    ...
    self.delegate.onDetailsForEnsUsername(
      args.chainId, args.ensUsername, args.address, args.pubkey, args.isStatus,
      args.expirationTime,
    )

  self.events.on(SIGNAL_ENS_TRANSACTION_SENT) do(e: Args):
    let args = EnsTxResultArgs(e)
    self.delegate.ensTransactionSent(
      args.transactionType, args.chainId, args.ensUsername, args.txHash, args.error
    )

  self.events.on(SIGNAL_ENS_TRANSACTION_CONFIRMED) do(e: Args):
    let args = EnsTransactionArgs(e)
    self.delegate.ensTransactionConfirmed(
      args.transactionType, args.ensUsername, args.txHash
    )

  self.events.on(SIGNAL_ENS_TRANSACTION_REVERTED) do(e: Args):
    let args = EnsTransactionArgs(e)
    self.delegate.ensTransactionReverted(
      args.transactionType, args.ensUsername, args.txHash
    )
...
proc checkEnsUsernameAvailability*(
    self: Controller, desiredEnsUsername: string, statusDomain: bool
) =
...
proc getAllMyEnsUsernames*(
    self: Controller, includePendingEnsUsernames: bool
): seq[EnsUsernameDto] =
...
proc setPreferredName*(self: Controller, preferredName: string) =
  if (self.settingsService.savePreferredName(preferredName)):
    ...
    info "an error occurred saving prefered ens username",
      methodName = "setPreferredName"
...
proc fixPreferredName*(self: Controller, ignoreCurrentValue: bool = false) =
  ...
  if (
    not ignoreCurrentValue and singletonInstance.userProfile.getPreferredName().len > 0
  ):
    return
