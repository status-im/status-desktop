...
QtObject:
  ...
  proc createCommunity*(self: View, name: string, description: string, 
    ...
    self.delegate.createCommunity(name, description, access, ensOnly, color, image, aX, aY, bX, bY)
  ...
  proc leaveCurrentCommunity*(self: View) {.slot.} =
    self.delegate.leaveCommunity(self.activeCommunity.communityItem.id)
  ...
  proc inviteUsersToCommunity*(self: View, pubKeysJSON: string) {.slot.} =
  ...
  proc removeUserFromCommunity*(self: View, pubKey: string) {.slot.} =
    self.delegate.removeUserFromCommunity(self.activeCommunity.id(), pubKey)
  ...
  proc acceptRequestToJoinCommunity*(self: View, requestId: string) {.slot.} =
    self.delegate.acceptRequestToJoinCommunity(self.activeCommunity.id(), requestid)

  proc declineRequestToJoinCommunity*(self: View, requestId: string) {.slot.} =
    self.delegate.declineRequestToJoinCommunity(self.activeCommunity.id(), requestId)
  ...
  proc exportCommunity*(self: View, communityId: string) {.slot.} =
