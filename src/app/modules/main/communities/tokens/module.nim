...
proc signOnKeycard(self: Module) =
  ...
  for h, (r, s, v) in self.tempResolvedSignatures.pairs:
    ...
    self.controller.runSignFlow(self.tempPin, self.tempAddressFrom, txForKcFlow)
    break
...
method prepareSignaturesForTransactions*(self:Module, txForSigning: RouterTransactionsForSigningDto) =
  ...
  try:
    ...
    if txForSigning.signingDetails.signOnKeycard:
      self.tempAddressFrom = txForSigning.signingDetails.addressPath
...
method computeDeployTokenOwnerFee*(self: Module, uuid: string, communityId: string, fromAddress: string, ownerName: string,
  ...
  echo "\n\nNIM MODULE    computeDeployTokenOwnerFee  isDeployed: ", isDeployed
  if isDeployed:
...
method suggestedRoutesReady*(self: Module, uuid: string, sendType: SendType, ethCurrency: CurrencyAmount, fiatCurrency: CurrencyAmount, errCode: string, errDescription: string) =
  if sendType != SendType.CommunityBurn and
    ...
    sendType != SendType.CommunitySetSignerPubKey:
      return
  self.view.emitSuggestedRoutesReadySignal(uuid, ethCurrency, fiatCurrency, errCode, errDescription)
