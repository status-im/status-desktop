...
proc newCommunityTokensModule*(
  ...
  result.controller = controller.newCommunityTokensController(
...
method load*(self: Module) =
  singletonInstance.engine.setRootContextProperty(
    "communityTokensModule", self.viewVariant
  )
...
proc getTokenAndAmountList(
    self: Module, communityId: string, tokensJsonString: string
): seq[CommunityTokenAndAmount] =
  try:
    ...
    for token in tokensJson:
      ...
      if tokenDto.tokenType == TokenType.Unknown:
        ...
        return @[]
      result.add(
        CommunityTokenAndAmount(
          communityToken: tokenDto, amount: amountStr.parse(Uint256)
        )
      )
  except Exception as e:
    error "Error getTokenAndAmountList", msg = e.msg

proc getTokenAddressFromPermissions(
    self: Module,
    communityDto: CommunityDto,
    chainId: int,
    permissionType: TokenPermissionType,
): string =
...
proc getOwnerAndMasterTokensAddresses(
    self: Module, communityId: string, chainId: int
): (string, string, bool) =
  let communityDto = self.controller.getCommunityById(communityId)
  let ownerTokenAddress = self.getTokenAddressFromPermissions(
    communityDto, chainId, TokenPermissionType.BecomeTokenOwner
  )
  let masterTokenAddress = self.getTokenAddressFromPermissions(
    communityDto, chainId, TokenPermissionType.BecomeTokenMaster
  )
  return (
    ownerTokenAddress,
    masterTokenAddress,
    ownerTokenAddress != "" and masterTokenAddress != "",
  )
...
method airdropTokens*(
    ...
    walletsJsonString: string,
    addressFrom: string,
) =
  self.tempTokenAndAmountList =
    self.getTokenAndAmountList(communityId, tokensJsonString)
...
method computeAirdropFee*(
  ...
  if len(tokenAndAmountList) == 0:
    return
  self.controller.computeAirdropFee(
    tokenAndAmountList,
    walletsJsonString.parseJson.to(seq[string]),
    addressFrom,
    requestId,
  )

proc getWalletAndAmountListFromJson(
    self: Module, collectiblesToBurnJsonString: string
): seq[WalletAndAmount] =
...
method selfDestructCollectibles*(
    self: Module,
    communityId: string,
    collectiblesToBurnJsonString: string,
    contractUniqueKey: string,
    addressFrom: string,
) =
  self.tempWalletAndAmountList =
    self.getWalletAndAmountListFromJson(collectiblesToBurnJsonString)
  self.tempCommunityId = communityId
...
method deployCollectibles*(
    ...
    selfDestruct: bool,
    chainId: int,
    imageCropInfoJson: string,
) =
  let (ownerTokenAddress, masterTokenAddress, isDeployed) =
    self.getOwnerAndMasterTokensAddresses(communityId, chainId)
  if not isDeployed:
  ...
  self.tempDeploymentParams.tokenUri =
    utl.changeCommunityKeyCompression(communityId) & "/"
  ...
  let base65Image =
    singletonInstance.utils.formatImagePath(croppedImage["imagePath"].getStr)
...
proc createOwnerAndMasterDeploymentParams(
    self: Module, communityId: string
): (DeploymentParameters, DeploymentParameters) =
  let communityDto = self.controller.getCommunityById(communityId)
  let commName = communityDto.name
  let commNameShort =
    try:
      commName[0 .. 2].toUpper
    except:
      commName.toUpper
  return (
...
method deployOwnerToken*(
  ...
  let base65Image =
    singletonInstance.utils.formatImagePath(croppedImage["imagePath"].getStr)
  (self.tempOwnerDeploymentParams, self.tempMasterDeploymentParams) =
    self.createOwnerAndMasterDeploymentParams(communityId)
...
method removeCommunityToken*(
    self: Module, communityId: string, chainId: int, address: string
) =
...
method onUserAuthenticated*(self: Module, password: string) =
  ...
  if password.len == 0:
    ...
    if self.tempContractAction == ContractAction.Deploy:
      self.controller.deployContract(
        self.tempCommunityId, self.tempAddressFrom, password, self.tempDeploymentParams,
        self.tempChainId,
      )
    elif self.tempContractAction == ContractAction.Airdrop:
      self.controller.airdropTokens(
        self.tempCommunityId, password, self.tempTokenAndAmountList,
        self.tempWalletAddresses, self.tempAddressFrom,
      )
    elif self.tempContractAction == ContractAction.SelfDestruct:
      self.controller.selfDestructCollectibles(
        self.tempCommunityId, password, self.tempWalletAndAmountList,
        self.tempContractUniqueKey, self.tempAddressFrom,
      )
    elif self.tempContractAction == ContractAction.Burn:
      self.controller.burnTokens(
        self.tempCommunityId, password, self.tempContractUniqueKey, self.tempAmount,
        self.tempAddressFrom,
      )
    elif self.tempContractAction == ContractAction.DeployOwnerToken:
      self.controller.deployOwnerContracts(
        self.tempCommunityId, self.tempAddressFrom, password,
        self.tempOwnerDeploymentParams, self.tempMasterDeploymentParams,
        self.tempChainId,
      )
    elif self.tempContractAction == ContractAction.SetSigner:
      self.controller.setSigner(
        password, self.tempCommunityId, self.tempChainId, self.tempContractAddress,
        self.tempAddressFrom,
      )

method onDeployFeeComputed*(
    self: Module,
...
method onSelfDestructFeeComputed*(
    self: Module,
    ethCurrency: CurrencyAmount,
    fiatCurrency: CurrencyAmount,
    errorCode: ComputeFeeErrorCode,
...
method computeDeployFee*(
    self: Module,
    communityId: string,
    chainId: int,
    accountAddress: string,
    tokenType: TokenType,
    isOwnerDeployment: bool,
    requestId: string,
) =
  if isOwnerDeployment:
    let (ownerDeploymentParams, masterDeploymentParams) =
      self.createOwnerAndMasterDeploymentParams(communityId)
    self.controller.computeDeployOwnerContractsFee(
      chainId, accountAddress, communityId, ownerDeploymentParams,
      masterDeploymentParams, requestId,
    )
  else:
    self.controller.computeDeployFee(chainId, accountAddress, tokenType, requestId)

method computeSetSignerFee*(
    self: Module,
    chainId: int,
    contractAddress: string,
    addressFrom: string,
...
method computeSelfDestructFee*(
  ...
  let walletAndAmountList =
    self.getWalletAndAmountListFromJson(collectiblesToBurnJsonString)
  self.controller.computeSelfDestructFee(
    walletAndAmountList, contractUniqueKey, addressFrom, requestId
  )

method computeBurnFee*(
    self: Module,
    contractUniqueKey: string,
    amount: string,
    addressFrom: string,
  ...
  self.controller.computeBurnFee(
    contractUniqueKey, amount.parse(Uint256), addressFrom, requestId
  )
...
proc createUrl(self: Module, chainId: int, transactionHash: string): string =
  ...
  result =
    if network != nil:
      network.blockExplorerURL & "/tx/" & transactionHash
    else:
      ""
...
method onCommunityTokenDeployStateChanged*(
    self: Module,
    communityId: string,
    chainId: int,
...
method onOwnerTokenDeployStateChanged*(
    self: Module,
    communityId: string,
    chainId: int,
    transactionHash: string,
...
method onOwnerTokenDeployStarted*(
    self: Module, communityId: string, chainId: int, transactionHash: string
) =
  let url = self.createUrl(chainId, transactionHash)
  self.view.emitOwnerTokenDeploymentStarted(communityId, url)

method onRemoteDestructStateChanged*(
    self: Module,
    communityId: string,
    tokenName: string,
    chainId: int,
...
method onCommunityTokenReceived*(
  ...
  self.view.emitCommunityTokenReceived(
    name, symbol, image, communityId, communityName, balance, chainId, txHash, isFirst,
    tokenType, accountName, accountAddress,
  )

method onSetSignerStateChanged*(
    self: Module,
    communityId: string,
    chainId: int,
    transactionHash: string,
...
method onSendOwnerTokenStateChanged*(
    self: Module,
    chainId: int,
    transactionHash: string,
    tokenName: string,
...
method asyncGetOwnerTokenDetails*(self: Module, communityId: string) =
  ...
  let (chainId, contractAddress) =
    self.tempOwnerTokenCommunity.getOwnerTokenAddressFromPermissions()
