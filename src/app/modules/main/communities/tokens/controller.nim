...
proc init*(self: Controller) =
  ...
  self.events.on(SIGNAL_COMPUTE_SELF_DESTRUCT_FEE) do(e:Args):
    ...
    self.delegate.onSelfDestructFeeComputed(args.ethCurrency, args.fiatCurrency, args.errorCode, args.requestId)
  self.events.on(SIGNAL_COMPUTE_BURN_FEE) do(e:Args):
    let args = ComputeFeeArgs(e)
    self.delegate.onBurnFeeComputed(args.ethCurrency, args.fiatCurrency, args.errorCode, args.requestId)
  self.events.on(SIGNAL_COMPUTE_SET_SIGNER_FEE) do(e:Args):
    let args = ComputeFeeArgs(e)
    self.delegate.onSetSignerFeeComputed(args.ethCurrency, args.fiatCurrency, args.errorCode, args.requestId)
  self.events.on(SIGNAL_COMPUTE_AIRDROP_FEE) do(e:Args):
    let args = AirdropFeesArgs(e)
    self.delegate.onAirdropFeesComputed(args)
  self.events.on(SIGNAL_REMOTE_DESTRUCT_STATUS) do(e: Args):
    let args = RemoteDestructArgs(e)
    self.delegate.onRemoteDestructStateChanged(args.communityToken.communityId, args.communityToken.name, args.communityToken.chainId, args.transactionHash, args.status)
  self.events.on(SIGNAL_BURN_STATUS) do(e: Args):
    let args = RemoteDestructArgs(e)
    self.delegate.onBurnStateChanged(args.communityToken.communityId, args.communityToken.name, args.communityToken.chainId, args.transactionHash, args.status)
  self.events.on(SIGNAL_AIRDROP_STATUS) do(e: Args):
    let args = AirdropArgs(e)
    self.delegate.onAirdropStateChanged(args.communityToken.communityId, args.communityToken.name, args.communityToken.chainId, args.transactionHash, args.status)
  ...
  self.events.on(SIGNAL_SET_SIGNER_STATUS) do(e: Args):
    ...
    self.delegate.onSetSignerStateChanged(args.communityId, args.chainId, args.transactionHash, args.status)
  ...
  self.events.on(SIGNAL_OWNER_TOKEN_SENT) do(e: Args):
    ...
    self.delegate.onSendOwnerTokenStateChanged(args.chainId, args.txHash, args.tokenName, args.status)
  ...
  self.events.on(SIGNAL_SUGGESTED_ROUTES_READY) do(e:Args):
    ...
    self.delegate.suggestedRoutesReady(args.uuid, args.sendType, args.totalCostEthCurrency, args.totalCostFiatCurrency, args.errCode, args.errDescription)
