...
const UNIQUE_DEPLOY_COLLECTIBLES_COMMUNITY_TOKENS_MODULE_IDENTIFIER* =
  "communityTokensModule-DeployCollectibles"
...
proc init*(self: Controller) =
  self.events.on(SIGNAL_SHARED_KEYCARD_MODULE_USER_AUTHENTICATED) do(e: Args):
    ...
    if args.uniqueIdentifier !=
        UNIQUE_DEPLOY_COLLECTIBLES_COMMUNITY_TOKENS_MODULE_IDENTIFIER:
      return
  ...
  self.events.on(SIGNAL_COMPUTE_DEPLOY_FEE) do(e: Args):
    ...
    self.communityTokensModule.onDeployFeeComputed(
      args.ethCurrency, args.fiatCurrency, args.errorCode, args.requestId
    )
  self.events.on(SIGNAL_COMPUTE_SELF_DESTRUCT_FEE) do(e: Args):
    let args = ComputeFeeArgs(e)
    self.communityTokensModule.onSelfDestructFeeComputed(
      args.ethCurrency, args.fiatCurrency, args.errorCode, args.requestId
    )
  self.events.on(SIGNAL_COMPUTE_BURN_FEE) do(e: Args):
    let args = ComputeFeeArgs(e)
    self.communityTokensModule.onBurnFeeComputed(
      args.ethCurrency, args.fiatCurrency, args.errorCode, args.requestId
    )
  self.events.on(SIGNAL_COMPUTE_SET_SIGNER_FEE) do(e: Args):
    let args = ComputeFeeArgs(e)
    self.communityTokensModule.onSetSignerFeeComputed(
      args.ethCurrency, args.fiatCurrency, args.errorCode, args.requestId
    )
  ...
  self.events.on(SIGNAL_COMMUNITY_TOKEN_DEPLOYMENT_STARTED) do(e: Args):
    ...
    self.communityTokensModule.onCommunityTokenDeployStateChanged(
      args.communityToken.communityId, args.communityToken.chainId,
      args.transactionHash, args.communityToken.deployState,
    )
  self.events.on(SIGNAL_COMMUNITY_TOKEN_DEPLOY_STATUS) do(e: Args):
    let args = CommunityTokenDeployedStatusArgs(e)
    self.communityTokensModule.onCommunityTokenDeployStateChanged(
      args.communityId, args.chainId, args.transactionHash, args.deployState
    )
  self.events.on(SIGNAL_OWNER_TOKEN_DEPLOYMENT_STARTED) do(e: Args):
    let args = OwnerTokenDeploymentArgs(e)
    self.communityTokensModule.onOwnerTokenDeployStarted(
      args.ownerToken.communityId, args.ownerToken.chainId, args.transactionHash
    )
  self.events.on(SIGNAL_OWNER_TOKEN_DEPLOY_STATUS) do(e: Args):
    let args = OwnerTokenDeployedStatusArgs(e)
    self.communityTokensModule.onOwnerTokenDeployStateChanged(
      args.communityId, args.chainId, args.transactionHash, args.deployState
    )
  self.events.on(SIGNAL_REMOTE_DESTRUCT_STATUS) do(e: Args):
    let args = RemoteDestructArgs(e)
    self.communityTokensModule.onRemoteDestructStateChanged(
      args.communityToken.communityId, args.communityToken.name,
      args.communityToken.chainId, args.transactionHash, args.status,
    )
  self.events.on(SIGNAL_BURN_STATUS) do(e: Args):
    let args = RemoteDestructArgs(e)
    self.communityTokensModule.onBurnStateChanged(
      args.communityToken.communityId, args.communityToken.name,
      args.communityToken.chainId, args.transactionHash, args.status,
    )
  self.events.on(SIGNAL_AIRDROP_STATUS) do(e: Args):
    let args = AirdropArgs(e)
    self.communityTokensModule.onAirdropStateChanged(
      args.communityToken.communityId, args.communityToken.name,
      args.communityToken.chainId, args.transactionHash, args.status,
    )
  self.events.on(SIGNAL_OWNER_TOKEN_RECEIVED) do(e: Args):
    let args = OwnerTokenReceivedArgs(e)
    self.communityTokensModule.onOwnerTokenReceived(
      args.communityId, args.communityName, args.chainId, args.contractAddress
    )
  self.events.on(SIGNAL_COMMUNITY_TOKEN_RECEIVED) do(e: Args):
    let args = CommunityTokenReceivedArgs(e)
    if args.isWatchOnlyAccount:
      return
    self.communityTokensModule.onCommunityTokenReceived(
      args.name,
      args.symbol,
      args.image,
      args.communityId,
  ...
  self.events.on(SIGNAL_SET_SIGNER_STATUS) do(e: Args):
    ...
    self.communityTokensModule.onSetSignerStateChanged(
      args.communityId, args.chainId, args.transactionHash, args.status
    )
  ...
  self.events.on(SIGNAL_OWNER_TOKEN_OWNER_ADDRESS) do(e: Args):
    ...
    self.communityTokensModule.onOwnerTokenOwnerAddress(
      args.chainId, args.contractAddress, args.address, args.addressName
    )
  self.events.on(SIGNAL_OWNER_TOKEN_SENT) do(e: Args):
    let args = OwnerTokenSentArgs(e)
    self.communityTokensModule.onSendOwnerTokenStateChanged(
      args.chainId, args.txHash, args.tokenName, args.status
    )
...
proc deployContract*(
  ...
  self.communityTokensService.deployContract(
    communityId, addressFrom, password, deploymentParams, chainId
  )
...
proc deployOwnerContracts*(
  ...
  self.communityTokensService.deployOwnerContracts(
    communityId, addressFrom, password, ownerDeploymentParams, masterDeploymentParams,
    chainId,
  )

proc removeCommunityToken*(
    self: Controller, communityId: string, chainId: int, address: string
) =
...
proc airdropTokens*(
    ...
    communityId: string,
    password: string,
    tokensAndAmounts: seq[CommunityTokenAndAmount],
    walletAddresses: seq[string],
    addressFrom: string,
) =
  self.communityTokensService.airdropTokens(
    communityId, password, tokensAndAmounts, walletAddresses, addressFrom
  )
...
proc computeAirdropFee*(
  ...
  self.communityTokensService.computeAirdropFee(
    tokensAndAmounts, walletAddresses, addressFrom, requestId
  )
...
proc selfDestructCollectibles*(
  ...
  self.communityTokensService.selfDestructCollectibles(
    communityId, password, walletAndAmounts, contractUniqueKey, addressFrom
  )
...
proc burnTokens*(
  ...
  self.communityTokensService.burnTokens(
    communityId, password, contractUniqueKey, amount, addressFrom
  )
...
proc setSigner*(
  ...
  self.communityTokensService.setSigner(
    password, communityId, chainId, contractAddress, addressFrom
  )

proc authenticateUser*(self: Controller, keyUid = "") =
  let data = SharedKeycarModuleAuthenticationArgs(
    uniqueIdentifier: UNIQUE_DEPLOY_COLLECTIBLES_COMMUNITY_TOKENS_MODULE_IDENTIFIER,
    keyUid: keyUid,
  )
...
proc computeDeployFee*(
  ...
  self.communityTokensService.computeDeployFee(
    chainId, accountAddress, tokenType, requestId
  )
...
proc computeSetSignerFee*(
  ...
  self.communityTokensService.computeSetSignerFee(
    chainId, contractAddress, addressFrom, requestId
  )
...
proc computeDeployOwnerContractsFee*(
  ...
  self.communityTokensService.computeDeployOwnerContractsFee(
    chainId, accountAddress, communityId, ownerDeploymentParams, masterDeploymentParams,
    requestId,
  )
...
proc computeSelfDestructFee*(
  ...
  self.communityTokensService.computeSelfDestructFee(
    walletAndAmountList, contractUniqueKey, addressFrom, requestId
  )

proc findContractByUniqueId*(
    self: Controller, contractUniqueKey: string
): CommunityTokenDto =
...
proc computeBurnFee*(
  ...
  self.communityTokensService.computeBurnFee(
    contractUniqueKey, amount, addressFrom, requestId
  )
