...
import
  ../../../../../../app_service/service/community_tokens/community_collectible_owner
...
QtObject:
  ...
  proc getItemIndex(self: TokenModel, chainId: int, contractAddress: string): int =
    for i in 0 ..< self.items.len:
      if self.items[i].tokenDto.address == contractAddress and
          self.items[i].tokenDto.chainId == chainId:
        return i
    return -1

  proc updateDeployState*(
      self: TokenModel, chainId: int, contractAddress: string, deployState: DeployState
  ) =
  ...
  proc updateRemoteDestructedAddresses*(
    ...
    self.items[itemIdx].tokenOwnersModel.updateRemoteDestructState(
  ...
  proc updateSupply*(
    ...
    if self.items[itemIdx].tokenDto.supply != supply or
  ...
  proc updateRemainingSupply*(
      self: TokenModel, chainId: int, contractAddress: string, remainingSupply: Uint256
  ) =
    let itemIdx = self.getItemIndex(chainId, contractAddress)
  ...
  proc setCommunityTokenOwners*(
    ...
    self.items[itemIdx].tokenOwnersModel.setItems(
      owners.map(
        proc(owner: CommunityCollectibleOwner): TokenOwnersItem =
          ...
          result = initTokenOwnersItem(
            ...
            self.items[itemIdx].remoteDestructedAddresses,
          )
      )
    )
    let index = self.createIndex(itemIdx, 0, nil)
    defer:
      index.delete
    self.dataChanged(
      index, index, @[ModelRole.TokenOwnersModel.int, ModelRole.TokenHoldersLoading.int]
    )

  proc setCommunityTokenHoldersLoading*(
      self: TokenModel, chainId: int, contractAddress: string, value: bool
  ) =
    let itemIdx = self.getItemIndex(chainId, contractAddress)
  ...
  proc removeItemByChainIdAndAddress*(
      self: TokenModel, chainId: int, contractAddress: string
  ) =
    let itemIdx = self.getItemIndex(chainId, contractAddress)
    if itemIdx == -1:
      return
  ...
  method data(self: TokenModel, index: QModelIndex, role: int): QVariant =
    ...
    of ModelRole.ContractUniqueKey:
      result =
        newQVariant(contractUniqueKey(item.tokenDto.chainId, item.tokenDto.address))
    ...
    of ModelRole.RemotelyDestructState:
      let destructStatus =
        if len(item.remoteDestructedAddresses) > 0:
          ContractTransactionStatus.InProgress.int
        else:
          ContractTransactionStatus.Completed.int
