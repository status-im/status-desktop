...
QtObject:
  ...
  proc chainId*(self: TokenDataItem): int {.slot.} =
    result = self.chainId

  QtProperty[int] chainId:
    read = chainId

  proc txHash*(self: TokenDataItem): string {.slot.} =
    result = self.txHash

  QtProperty[string] txHash:
    read = txHash

  proc walletAddress*(self: TokenDataItem): string {.slot.} =
    result = self.walletAddress

  QtProperty[string] walletAddress:
    read = walletAddress

  proc isFirst*(self: TokenDataItem): bool {.slot.} =
    result = self.isFirst

  QtProperty[bool] isFirst:
    read = isFirst

  proc communiyId*(self: TokenDataItem): string {.slot.} =
    result = self.communiyId

  QtProperty[string] communiyId:
    read = communiyId

  proc amount*(self: TokenDataItem): string {.slot.} =
    result = self.amount

  QtProperty[string] amount:
    read = amount

  proc name*(self: TokenDataItem): string {.slot.} =
    result = self.name

  QtProperty[string] name:
    read = name

  proc symbol*(self: TokenDataItem): string {.slot.} =
    result = self.symbol

  QtProperty[string] symbol:
    read = symbol

  proc imageUrl*(self: TokenDataItem): string {.slot.} =
    result = self.imageUrl

  QtProperty[string] imageUrl:
    read = imageUrl

  proc tokenType*(self: TokenDataItem): int {.slot.} =
