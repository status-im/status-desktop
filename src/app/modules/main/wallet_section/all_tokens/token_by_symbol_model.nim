...
QtObject:
  ...
  method data(self: TokensBySymbolModel, index: QModelIndex, role: int): QVariant =
    ...
    of ModelRole.Sources:
      result = newQVariant(
        SOURCES_DELIMITER & item.sources.join(SOURCES_DELIMITER) & SOURCES_DELIMITER
      )
    ...
    of ModelRole.Description:
      result =
        if not item.communityId.isEmptyOrWhitespace:
          newQVariant(
            self.delegate.getCommunityTokenDescription(item.addressPerChainId)
          )
        else:
          if self.delegate.getTokensDetailsLoading():
            newQVariant("")
          else:
            newQVariant(self.delegate.getTokenDetails(item.symbol).description)
    of ModelRole.WebsiteUrl:
      result =
        if not item.communityId.isEmptyOrWhitespace or
            self.delegate.getTokensDetailsLoading():
          newQVariant("")
        else:
          newQVariant(self.delegate.getTokenDetails(item.symbol).assetWebsiteUrl)
  ...
  proc modelsUpdated*(self: TokensBySymbolModel) =
    ...
    for index in countup(0, tokensList.len - 1):
      ...
      let symbol =
        if tokensList[index].communityId.isEmptyOrWhitespace:
          tokensList[index].symbol
        else:
          ""
      self.tokenMarketDetails.add(
        newMarketDetailsItem(self.marketValuesDelegate, symbol)
      )
    self.endResetModel()
  ...
  proc tokensMarketValuesUpdated*(self: TokensBySymbolModel) =
    if not self.delegate.getTokensMarketValuesLoading():
      if self.delegate.getTokenBySymbolList().len > 0:
        ...
        let lastindex =
          self.createIndex(self.delegate.getTokenBySymbolList().len - 1, 0, nil)
        defer:
          index.delete
        defer:
          lastindex.delete
        self.dataChanged(
          index,
          lastindex,
          @[ModelRole.MarketDetails.int, ModelRole.MarketDetailsLoading.int],
        )
  ...
  proc tokensDetailsAboutToUpdate*(self: TokensBySymbolModel) =
    if self.delegate.getTokenBySymbolList().len > 0:
      ...
      self.dataChanged(
        ...
        lastindex,
        @[
          ModelRole.Description.int, ModelRole.WebsiteUrl.int,
          ModelRole.DetailsLoading.int,
        ],
      )
  ...
  proc tokenPreferencesUpdated*(self: TokensBySymbolModel) =
    if self.delegate.getTokenBySymbolList().len > 0:
      ...
      defer:
        lastindex.delete
      self.dataChanged(
        index, lastindex, @[ModelRole.Visible.int, ModelRole.Position.int]
      )
