...
QtObject:
  ...
  method data(self: FlatTokensModel, index: QModelIndex, role: int): QVariant =
    ...
    of ModelRole.Description:
      result =
        if not item.communityId.isEmptyOrWhitespace:
          newQVariant(
            self.delegate.getCommunityTokenDescription(item.chainId, item.address)
          )
        else:
    ...
    of ModelRole.WebsiteUrl:
      ...
      result =
        if not tokenDetails.isNil:
          newQVariant(tokenDetails.assetWebsiteUrl)
        else:
          newQVariant("")
  ...
  proc modelsUpdated*(self: FlatTokensModel) =
    ...
    for token in self.delegate.getFlatTokensList():
      let symbol = if token.communityId.isEmptyOrWhitespace: token.symbol else: ""
  ...
  proc marketDetailsDataChanged(self: FlatTokensModel) =
    if self.delegate.getFlatTokensList().len > 0:
      ...
      let lastindex =
        self.createIndex(self.delegate.getFlatTokensList().len - 1, 0, nil)
      defer:
        index.delete
      defer:
        lastindex.delete
