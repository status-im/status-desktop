...
method getTokenBalance*(
  self: Module, address: string, chainId: int, tokensKey: string
): CurrencyAmount
...
proc convertSendToNetworkToNetworkItem(
    self: Module, network: SendToNetwork
): NetworkRouteItem =
...
proc convertNetworkDtoToNetworkRouteItem(
    self: Module, network: network_service_item.NetworkItem
): NetworkRouteItem =
  result = initNetworkRouteItem(
    network.chainId,
    network.layer,
    ...
    self.getTokenBalance(
      self.view.getSelectedSenderAccountAddress(),
      network.chainId,
      self.view.getSelectedAssetKey(),
    ),
  )

proc convertSuggestedFeesDtoToGasFeesItem(
    self: Module, gasFees: SuggestedFeesDto
): GasFeesItem =
...
proc convertTransactionPathDtoToSuggestedRouteItem(
    self: Module, path: TransactionPathDto
): SuggestedRouteItem =
...
method authenticateAndTransfer*(
    self: Module, fromAddr: string, uuid: string, slippagePercentage: float
) =
...
method suggestedRoutesReady*(
  ...
  let paths =
    suggestedRoutes.best.map(x => self.convertTransactionPathDtoToSuggestedRouteItem(x))
  let suggestedRouteModel = newSuggestedRouteModel()
  suggestedRouteModel.setItems(paths)
  let gasTimeEstimate =
    self.convertFeesDtoToGasEstimateItem(suggestedRoutes.gasTimeEstimate)
  let networks =
    suggestedRoutes.toNetworks.map(x => self.convertSendToNetworkToNetworkItem(x))
  let toNetworksRouteModel = newNetworkRouteModel()
  toNetworksRouteModel.setItems(networks)
  self.view.updatedNetworksWithRoutes(
    paths, self.controller.getChainsWithNoGasFromError(errCode, errDescription)
  )
...
method suggestedRoutes*(
  ...
  self.controller.suggestedRoutes(
    ...
    amountOut, disabledFromChainIDs, disabledToChainIDs, lockedInAmounts,
    extraParamsTable,
  )
...
method filterChanged*(
    self: Module, addresses: seq[string], chainIds: seq[int], isDirty: bool
) =
...
method splitAndFormatAddressPrefix*(
    self: Module, text: string, updateInStore: bool
): string {.slot.} =
  ...
  for word in plainText(text).split(':'):
    if word.startsWith("0x"):
      ...
      if not chainColor.isEmptyOrWhitespace():
        ...
        editedText =
          editedText & "<span style='color: " & chainColor & "'>" & word & "</span>" &
          ":"
