...
proc init*(self: Controller) =
  ...
  self.events.on(SIGNAL_OWNER_TOKEN_SENT) do(e: Args):
    ...
    self.delegate.transactionWasSent(
      args.uuid, args.chainId, approvalTx = false, args.txHash, error = ""
    )
  ...
  self.events.on(SIGNAL_SUGGESTED_ROUTES_READY) do(e: Args):
    ...
    self.delegate.suggestedRoutesReady(
      args.uuid, args.suggestedRoutes, args.errCode, args.errDescription
    )
...
proc getTokenBalance*(
    self: Controller, address: string, chainId: int, tokensKey: string
): CurrencyAmount =
  return currencyAmountToItem(
    self.walletAccountService.getTokenBalance(address, chainId, tokensKey),
    self.walletAccountService.getCurrencyFormat(tokensKey),
  )

proc authenticate*(self: Controller, keyUid = "") =
  let data = SharedKeycarModuleAuthenticationArgs(
    uniqueIdentifier: UNIQUE_WALLET_SECTION_SEND_MODULE_IDENTIFIER, keyUid: keyUid
  )
...
proc suggestedRoutes*(
  ...
  self.transactionService.suggestedRoutes(
...
proc buildTransactionsFromRoute*(
    self: Controller, uuid: string, slippagePercentage: float
): string =
  return self.transactionService.buildTransactionsFromRoute(uuid, slippagePercentage)

proc signMessage*(
    self: Controller, address: string, hashedPassword: string, hashedMessage: string
...
proc sendRouterTransactionsWithSignatures*(
    self: Controller, uuid: string, signatures: TransactionsSignatures
): string =
...
proc getChainsWithNoGasFromError*(
    self: Controller, errCode: string, errDescription: string
): Table[int, string] =
