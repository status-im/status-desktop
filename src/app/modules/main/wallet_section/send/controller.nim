...
proc getTokenBalance*(self: Controller, address: string, chainId: int, tokenKey: string): CurrencyAmount =
  return currencyAmountToItem(self.walletAccountService.getTokenBalance(address, chainId, tokenKey), self.walletAccountService.getCurrencyFormat(tokenKey))
