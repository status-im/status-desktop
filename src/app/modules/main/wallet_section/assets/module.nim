...
proc newModule*(
  ...
  result.controller = newController(
    result, walletAccountService, networkService, tokenService, currencyService
  )
...
method load*(self: Module) =
  singletonInstance.engine.setRootContextProperty(
    "walletSectionAssets", self.viewVariant
  )
...
method getGroupedAccountAssetsDataSource*(
    self: Module
): GroupedAccountAssetsDataSource =
  return (
    getGroupedAccountsAssetsList: proc(): var seq[GroupedTokenItem] =
      self.controller.getGroupedAccountsAssetsList()
  )
