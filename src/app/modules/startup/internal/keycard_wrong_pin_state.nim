type KeycardWrongPinState* = ref object of State
  pinValid: bool

proc newKeycardWrongPinState*(
    flowType: FlowType, backState: State
): KeycardWrongPinState =
...
method resolveKeycardNextState*(
  ...
  if keycardFlowType == ResponseTypeValueEnterPIN and keycardEvent.error.len > 0 and
    ...
    if keycardEvent.pinRetries > 0:
      return self
    return createState(
      StateType.KeycardMaxPinRetriesReached, self.flowType, self.getBackState
    )
