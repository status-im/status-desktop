type UserProfileImportSeedPhraseState* = ref object of State

proc newUserProfileImportSeedPhraseState*(
    flowType: FlowType, backState: State
): UserProfileImportSeedPhraseState =
...
method executeBackCommand*(
    self: UserProfileImportSeedPhraseState, controller: Controller
) =
...
method getNextPrimaryState*(
  ...
  return createState(
    StateType.UserProfileEnterSeedPhrase, FlowType.FirstRunNewUserImportSeedPhrase, self
  )

method executeSecondaryCommand*(
    self: UserProfileImportSeedPhraseState, controller: Controller
) =
...
method resolveKeycardNextState*(
    self: UserProfileImportSeedPhraseState,
