type KeycardInsertKeycardState* = ref object of State

proc newKeycardInsertKeycardState*(
    flowType: FlowType, backState: State
): KeycardInsertKeycardState =
...
method resolveKeycardNextState*(
    self: KeycardInsertKeycardState,
    keycardFlowType: string,
    keycardEvent: KeycardEvent,
    controller: Controller,
): State =
  let state = ensureReaderAndCardPresenceAndResolveNextOnboardingState(
  ...
  if keycardFlowType == ResponseTypeValueCardInserted:
    ...
    return
      createState(StateType.KeycardInsertedKeycard, self.flowType, self.getBackState)
  return nil
