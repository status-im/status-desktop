type UserProfileEnterSeedPhraseState* = ref object of State
  successfulImport: bool
  enteredMnemonicMatchTargetedKeyUid: bool

proc newUserProfileEnterSeedPhraseState*(
    flowType: FlowType, backState: State
): UserProfileEnterSeedPhraseState =
...
method executeBackCommand*(
    self: UserProfileEnterSeedPhraseState, controller: Controller
) =
...
method executePrimaryCommand*(
  ...
  if self.flowType == FlowType.FirstRunNewUserImportSeedPhrase or
    ...
    self.successfulImport =
      controller.validateMnemonicForImport(controller.getSeedPhrase())
    return
  ...
  if self.flowType == FlowType.FirstRunNewUserImportSeedPhraseIntoKeycard:
    controller.storeSeedPhraseToKeycard(
      controller.getSeedPhraseLength(), controller.getSeedPhrase()
    )
  if self.flowType == FlowType.FirstRunOldUserKeycardImport:
  ...
  if self.flowType == FlowType.AppLogin:
    self.enteredMnemonicMatchTargetedKeyUid =
      controller.keyUidMatchSelectedLoginAccount(keyUid)
    if not self.enteredMnemonicMatchTargetedKeyUid:
      controller.setKeycardData(
        updatePredefinedKeycardData(
          controller.getKeycardData(), PredefinedKeycardData.WrongSeedPhrase, add = true
...
method resolveKeycardNextState*(
    self: UserProfileEnterSeedPhraseState,
