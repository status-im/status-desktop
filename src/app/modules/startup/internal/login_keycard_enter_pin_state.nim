type LoginKeycardEnterPinState* = ref object of State

proc newLoginKeycardEnterPinState*(
    flowType: FlowType, backState: State
): LoginKeycardEnterPinState =
...
method getNextTertiaryState*(
    self: LoginKeycardEnterPinState, controller: Controller
): State =
  if self.flowType == FlowType.AppLogin:
...
method resolveKeycardNextState*(
    self: LoginKeycardEnterPinState,
    keycardFlowType: string,
    keycardEvent: KeycardEvent,
    controller: Controller,
): State =
  let state =
    ensureReaderAndCardPresenceLogin(self, keycardFlowType, keycardEvent, controller)
  if not state.isNil:
    return state
  if self.flowType == FlowType.AppLogin:
    ...
    if keycardFlowType == ResponseTypeValueEnterPUK and keycardEvent.error.len == 0:
      if keycardEvent.pinRetries == 0 and keycardEvent.pukRetries > 0:
        return
          createState(StateType.LoginKeycardMaxPinRetriesReached, self.flowType, nil)
