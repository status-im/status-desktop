proc ensureReaderAndCardPresenceOnboarding*(
    state: State,
  ...
  if keycardFlowType == ResponseTypeValueKeycardFlowResult and keycardEvent.error.len > 0:
    ...
    if keycardEvent.error == ErrorNoReader or keycardEvent.error == ErrorReaderList:
...
proc ensureReaderAndCardPresenceAndResolveNextOnboardingState*(
    state: State,
    keycardFlowType: string,
    keycardEvent: KeycardEvent,
    controller: Controller,
): State =
  let ensureState = ensureReaderAndCardPresenceOnboarding(
  ...
  if state.flowType == FlowType.FirstRunOldUserKeycardImport:
    ...
    controller.setKeycardData(
      updatePredefinedKeycardData(
        controller.getKeycardData(), PredefinedKeycardData.MaxPUKReached, add = false
      )
    )
    controller.setKeycardData(
      updatePredefinedKeycardData(
        controller.getKeycardData(),
        PredefinedKeycardData.MaxPairingSlotsReached,
        add = false,
      )
    )
    ...
    if keycardFlowType == ResponseTypeValueEnterPUK and keycardEvent.error.len == 0:
      if keycardEvent.pinRetries == 0 and keycardEvent.pukRetries > 0:
        return
          createState(StateType.KeycardMaxPinRetriesReached, state.flowType, backState)
    if keycardFlowType == ResponseTypeValueSwapCard and keycardEvent.error.len > 0 and
    ...
    if keycardFlowType == ResponseTypeValueSwapCard and keycardEvent.error.len > 0 and
      ...
      controller.setKeycardData(
        updatePredefinedKeycardData(
          controller.getKeycardData(), PredefinedKeycardData.MaxPUKReached, add = true
        )
      )
      return
        createState(StateType.KeycardMaxPukRetriesReached, state.flowType, backState)
    if keycardFlowType == ResponseTypeValueSwapCard and keycardEvent.error.len > 0 and
      ...
      return
        createState(StateType.KeycardMaxPairingSlotsReached, state.flowType, backState)

  if state.flowType == FlowType.AppLogin:
    if keycardFlowType == ResponseTypeValueSwapCard and keycardEvent.error.len > 0 and
        keycardEvent.error == RequestParamPUKRetries:
      return createState(
        StateType.KeycardMaxPukRetriesReached, state.flowType, state.getBackState
      )
