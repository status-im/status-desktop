type KeycardRepeatPinState* = ref object of State
  oldKeycardUid: string

proc newKeycardRepeatPinState*(
    flowType: FlowType, backState: State
): KeycardRepeatPinState =
...
method resolveKeycardNextState*(
  ...
  if self.flowType == FlowType.FirstRunNewUserImportSeedPhraseIntoKeycard:
    if keycardFlowType == ResponseTypeValueKeycardFlowResult and
      ...
      let backState =
        findBackStateWithTargetedStateType(self, StateType.UserProfileImportSeedPhrase)
  ...
  if self.flowType == FlowType.FirstRunOldUserKeycardImport:
      ...
      if keycardFlowType == ResponseTypeValueKeycardFlowResult:
        if keycardEvent.error.len == 0:
          ...
          controller.runLoadAccountFlow(
            controller.getSeedPhraseLength(),
            controller.getSeedPhrase(),
            controller.getPin(),
            puk = "",
            factoryReset = true,
          )
    ...
    if controller.getCurrentKeycardServiceFlow() == KCSFlowType.RecoverAccount or
      ...
      if keycardFlowType == ResponseTypeValueEnterPUK:
        if keycardEvent.error.len > 0 and keycardEvent.error == RequestParamPUK:
          controller.setRemainingAttempts(keycardEvent.pukRetries)
          controller.setPukValid(false)
        if keycardEvent.pukRetries > 0:
          return createState(StateType.KeycardPinSet, self.flowType, self.getBackState)
        return createState(
          StateType.KeycardMaxPukRetriesReached, self.flowType, self.getBackState
        )
      ...
      if keycardFlowType == ResponseTypeValueSwapCard and keycardEvent.error.len > 0 and
        ...
        return createState(
          StateType.KeycardMaxPairingSlotsReached, self.flowType, self.getBackState
        )
      if keycardFlowType == ResponseTypeValueKeycardFlowResult:
        controller.setKeycardEvent(keycardEvent)
        controller.setPukValid(true)
        if controller.getRecoverKeycardUsingSeedPhraseWhileLoggingIn():
          controller.addToKeycardUidPairsToCheckForAChangeAfterLogin(
            self.oldKeycardUid, keycardEvent.instanceUID
          )
          let md = controller.getMetadataFromKeycard()
          let paths = md.walletAccounts.map(a => a.path)
          controller.runStoreMetadataFlow(
            cardName = md.name, pin = controller.getPin(), walletPaths = paths
          )
          return
        let backState =
          findBackStateWithTargetedStateType(self, StateType.RecoverOldUser)
        return createState(StateType.KeycardPinSet, self.flowType, backState)
  ...
  if self.flowType == FlowType.LostKeycardReplacement:
    if keycardFlowType == ResponseTypeValueKeycardFlowResult:
      ...
      if main_constants.IS_MACOS:
        let backState =
          findBackStateWithTargetedStateType(self, StateType.LostKeycardOptions)
        return createState(StateType.KeycardPinSet, self.flowType, backState)
      return createState(StateType.KeycardPinSet, self.flowType, nil)
