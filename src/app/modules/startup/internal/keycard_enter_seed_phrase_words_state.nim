type KeycardEnterSeedPhraseWordsState* = ref object of State

proc newKeycardEnterSeedPhraseWordsState*(
    flowType: FlowType, backState: State
): KeycardEnterSeedPhraseWordsState =
...
method executeBackCommand*(
    self: KeycardEnterSeedPhraseWordsState, controller: Controller
) =
  controller.setPin("")
...
method resolveKeycardNextState*(
    self: KeycardEnterSeedPhraseWordsState,
  ...
  if self.flowType == FlowType.FirstRunNewUserNewKeycardKeys:
    if keycardFlowType == ResponseTypeValueKeycardFlowResult and
      ...
      let backState =
        findBackStateWithTargetedStateType(self, StateType.WelcomeNewStatusUser)
