type LostKeycardOptionsState* = ref object of State

proc newLostKeycardOptionsState*(
    flowType: FlowType, backState: State
): LostKeycardOptionsState =
...
method executeBackCommand*(self: LostKeycardOptionsState, controller: Controller) =
  if controller.isSelectedAccountAKeycardAccount() and (
    self.flowType == FlowType.LostKeycardReplacement or
...
method getNextSecondaryState*(
    self: LostKeycardOptionsState, controller: Controller
): State =
  if controller.isSelectedAccountAKeycardAccount():
    return createState(
      StateType.UserProfileEnterSeedPhrase, FlowType.LostKeycardConvertToRegularAccount,
      self,
    )

method resolveKeycardNextState*(
    self: LostKeycardOptionsState,
