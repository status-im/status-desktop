proc ensureReaderAndCardPresenceLogin*(
    state: State,
...
proc ensureReaderAndCardPresenceAndResolveNextLoginState*(
    state: State,
    keycardFlowType: string,
    keycardEvent: KeycardEvent,
    controller: Controller,
  ...
  let ensureState =
    ensureReaderAndCardPresenceLogin(state, keycardFlowType, keycardEvent, controller)
  ...
  if state.flowType == FlowType.AppLogin:
    ...
    if keycardFlowType == ResponseTypeValueEnterPIN:
      ...
      if keycardEvent.error.len > 0:
        if keycardEvent.error == RequestParamPIN:
          ...
          return
            createState(StateType.LoginKeycardMaxPinRetriesReached, state.flowType, nil)
    ...
    if keycardFlowType == ResponseTypeValueSwapCard and keycardEvent.error.len > 0:
      ...
      if keycardEvent.error == RequestParamPUKRetries:
        return
          createState(StateType.LoginKeycardMaxPukRetriesReached, state.flowType, nil)
      if keycardEvent.error == RequestParamFreeSlots:
        return
          createState(StateType.LoginKeycardMaxPairingSlotsReached, state.flowType, nil)
