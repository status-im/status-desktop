...
def test_change_account_order_by_drag_and_drop(main_screen: MainWindow, user_account, address: str, default_name,
    ...
    with step('Verify accounts in wallet settings'):
        account_order = main_screen.left_panel.open_settings().open_wallet_settings().open_account_order()
    ...
    with step('Drag first account to the end of the list'):
        account_order.drag_account(default_name, 1, 200)
        time.sleep(1)
    #
    # with step('Verify the account order'):
    #     with step('Account order is correct in wallet settings'):
    #         assert account_order.accounts[0]['name'] == name
    #         assert account_order.accounts[1]['name'] == second_name
    #         assert account_order.accounts[2]['name'] == default_name
    #     with step('Account order is correct in wallet'):
    #         wallet = main_screen.left_panel.open_wallet()
    #         assert wallet.left_panel.accounts[0].name == name
    #         assert wallet.left_panel.accounts[1].name == second_name
    #         assert wallet.left_panel.accounts[2].name == default_name
    #
    with step('Drag second account to the top of the list'):
        # account_order = main_screen.left_panel.open_settings().open_wallet_settings().open_account_order()
        account_order.drag_account(second_name, 1, -110)
        time.sleep(4)

    with step('Verify the account order'):
        # with step('Account order is correct in wallet settings'):
        #     assert account_order.accounts[0]['name'] == second_name
        #     assert account_order.accounts[1]['name'] == name
        #     assert account_order.accounts[2]['name'] == default_name
        with step('Account order is correct in wallet'):
            wallet = main_screen.left_panel.open_wallet()
            assert wallet.left_panel.accounts[0].name == second_name
            assert wallet.left_panel.accounts[1].name == name
            assert wallet.left_panel.accounts[2].name == default_name
