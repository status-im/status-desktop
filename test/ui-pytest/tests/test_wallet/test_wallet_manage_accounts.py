...
def test_seed_phrase_imported_account(main_screen: MainWindow, user_account,
    ...
    with step('Create imported seed phrase wallet account'):
        ...
        SigningPhrasePopup().wait_until_appears().confirm_phrase()
        account_popup = wallet.left_panel.open_add_account_popup()
        account_popup.set_name(name).set_emoji(emoji).set_color(color).set_origin_seed_phrase(seed_phrase.split()).save()
        AuthenticatePopup().wait_until_appears().authenticate(user_account.password)
        account_popup.wait_until_hidden()
...
@allure.testcase('https://ethstatus.testrail.net/index.php?/cases/view/703036','Manage an account created from the generated seed phrase')
...
@allure.testcase('https://ethstatus.testrail.net/index.php?/cases/view/703514','Choosing Use Keycard when adding account')
