...
from gui.screens.onboarding import AllowNotificationsView, OnboardingWelcomeToStatusView, SyncResultView, SyncDeviceFoundView, \
    YourEmojihashAndIdenticonRingView, OnboardingProfileSyncedView, OnboardingBiometricsView
    ...
    with multiple_instances(user_data=None) as aut_one, multiple_instances(user_data=None) as aut_two:
        ...
        with step('Paste sync code on second instance and wait until device is synced'):
            ...
            assert driver.waitFor(
                lambda: 'Profile sync in progress' in str(profile_syncing_view.profile_synced_view_header.object.text), 5000)
            assert driver.waitForObject(profile_syncing_view.log_in_button.real_name, 15000), \
                    f'Log in button is not shown within 15 seconds'
            assert driver.waitFor(
                lambda: 'Profile synced' in str(profile_syncing_view.profile_synced_view_header.object.text), 5000)
