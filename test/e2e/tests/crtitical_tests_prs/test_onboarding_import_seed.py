...
def test_import_and_reimport_random_seed(main_window, aut: AUT, user_account):
    ...
    with step('Restart application and try re-importing seed phrase again'):
        ...
        enter_seed_view.continue_button.click()
        
    # TODO: add verification of error message when it is implemented, current flow leads to dead end
    # https: // github.com / status - im / status - desktop / issues / 17248
    # with step('Verify that keys already exist popup appears and text is correct'):
    #     assert enter_seed_view. == KeysExistText.KEYS_EXIST_TITLE.value
