...
def test_back_up_recovery_phrase_sign_out(
    ...
    with step('Open online identifier and check the data'):
        ...
        assert online_identifier._identicon_ring.is_visible, f'Identicon ring is not present when it should'
        assert str(online_identifier.object.compressedPubKey) is not None, f'Public key is not present'

    with step('Verify that user avatar background color'):
        avatar_color = str(main_screen.left_panel.profile_button.object.identicon.asset.color.name).upper()
