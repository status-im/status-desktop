...
def test_wallet_load_time_main_ui(main_screen, user_data, user_account, tmp_path):
...
def test_wallet_load_time_assets(main_screen, user_data, user_account, tmp_path):
    os.environ['STATUS_RUNTIME_TEST_MODE'] = 'True'  # to omit banners
    ...
    for i in range(5):
        ...
        with step(f'Iteration {i + 1}: Open wallet tab again and record load time'):
            wallet_screen, load_time = main_screen.left_panel.open_wallet_and_log_load_time_for_total_balance()
            load_times.append(load_time)
            line = f"[{i + 1}/5] Total balance load time: {load_time:.3f} seconds"
            report_lines.append(line)
            print(line)
            LOG.info(line)

    average_time = sum(load_times) / len(load_times) if load_times else 0.0
    average_line = f"Average total balace load time over {len(load_times)} runs: {average_time:.3f} seconds"
    print(average_line)
    LOG.info(average_line)
    ...
    report_lines.append(average_line)
    report_text = "\n".join(report_lines)
    report_file = tmp_path / "balance_load_times.txt"
    report_file.write_text(report_text, encoding="utf-8")

    with step('Attach total balance load times to Allure'):
        allure.attach(report_text, name='Total balance load times (text)', attachment_type=AttachmentType.TEXT)
        allure.attach.file(str(report_file), name='Total balance load times (file)', attachment_type=AttachmentType.TEXT)
