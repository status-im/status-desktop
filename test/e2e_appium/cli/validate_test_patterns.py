...
class TestPatternValidator:
    ...
    def validate_project(self, test_dir: Path) -> Dict[str, List[str]]:
        ...
        for test_file in test_dir.rglob("test_*.py"):
            ...
            if validation_result['status'] == 'compliant':
                ...
                results['warnings'].append(f"{test_file}: {validation_result['message']}")
            else:
    ...
    def validate_file(self, file_path: Path) -> Dict[str, str]:
        ...
        try:
            ...
            return {'status': 'compliant', 'message': 'All tests follow proper patterns'}
            
        except Exception as e:
            return {'status': 'error', 'message': f'Failed to parse file: {e}'}
    
    def _validate_test_class(self, test_class: ast.ClassDef, content: str) -> Dict[str, str]:
        ...
        # Find test methods
        test_methods = [node for node in test_class.body 
        ...
        return {'status': 'compliant', 'message': 'All test methods properly configured'}
    
    def _validate_test_method(self, test_method: ast.FunctionDef, content: str) -> Dict[str, str]:
        """Validate a single test method."""
        method_name = test_method.name
        
        # Check for @lambdatest_reporting decorator
        has_decorator = any(
            (isinstance(dec, ast.Name) and dec.id == 'lambdatest_reporting') or
            (isinstance(dec, ast.Attribute) and dec.attr == 'lambdatest_reporting')
            for dec in test_method.decorator_list
        )
        
        if has_decorator:
            return {'status': 'compliant', 'message': f'{method_name} uses decorator pattern'}
        ...
        if has_explicit_reporting:
            return {'status': 'compliant', 'message': f'{method_name} uses explicit reporting'}
        ...
        if has_template_pattern:
            return {'status': 'compliant', 'message': f'{method_name} uses template pattern'}
...
def main():
    ...
    parser.add_argument('--test-dir', default='test/e2e_appium/tests', 
    ...
    # Summary
    total = len(results['compliant']) + len(results['warnings']) + len(results['errors'])
    if total > 0:
        compliance_rate = len(results['compliant']) / total * 100
        print(f"📊 Compliance Rate: {compliance_rate:.1f}% ({len(results['compliant'])}/{total})")
