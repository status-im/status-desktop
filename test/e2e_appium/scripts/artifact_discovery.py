...
class GitHubArtifactDiscovery:
    ...
    def list_artifacts(self, workflow_name: str = "Android Build APK", 
                      days_back: int = 30, per_page: int = 50) -> List[Dict]:
                ...
                if artifacts_response.status_code == 200:
                    ...
                    for artifact in artifacts:
                        ...
                        artifacts_with_metadata.append({
                            'name': artifact['name'],
                            'id': artifact['id'],
                            'size_mb': round(artifact['size_in_bytes'] / 1024 / 1024, 1),
        ...
        return sorted(artifacts_with_metadata, key=lambda x: x['created_at'], reverse=True)
    ...
    def find_latest_by_architecture(self, architecture: str = 'x86_64', 
                                   branch: str = None, days_back: int = 7) -> Optional[Dict]:
    ...
    def search_artifacts(self, query: str = None, architecture: str = None, 
                        branch: str = None, days_back: int = 30) -> List[Dict]:
        ...
        if query:
            ...
            results = [a for a in results if 
                      ...
                      query_lower in a['commit_message'].lower() or
                      query_lower in a['actor'].lower()]
    ...
    def print_artifacts_table(self, artifacts: List[Dict], max_rows: int = 20):
        ...
        # Table header
        print(f"{'Name':<25} {'Arch':<8} {'Branch':<15} {'Run#':<6} {'Size':<8} {'Age':<12} {'Actor':<12}")
        print("-" * 95)
        ...
        for i, artifact in enumerate(artifacts[:max_rows]):
            ...
            print(f"{name:<25} {artifact['architecture']:<8} {branch:<15} "
...
def main():
    parser = argparse.ArgumentParser(description="Discover GitHub artifacts intelligently")
    parser.add_argument("--token", help="GitHub token (or set GITHUB_TOKEN env var)")
    parser.add_argument("--repo", default="status-im/status-desktop", help="Repository")
    parser.add_argument("--architecture", choices=['x86_64', 'arm64', 'arm', 'x86'], 
    ...
    parser.add_argument("--format", choices=['table', 'json', 'name'], default='table',
                       help="Output format")
        ...
        if args.latest:
            ...
            if artifact:
                if args.format == 'json':
                    ...
                    print(f"   Commit: {artifact['commit_sha']} - {artifact['commit_message']}")
