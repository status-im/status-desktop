...
class PerformanceTracker:
    ...
    def __init__(self, logger: logging.Logger, threshold_ms: int = 1000, 
        ...
        if self.enable_analytics:
            try:
                from .performance_analytics import PerformanceAnalytics, PerformanceMetric
                self.analytics = PerformanceAnalytics()
                self.PerformanceMetric = PerformanceMetric
            except ImportError:
                self.logger.warning("Performance analytics unavailable - running without historical analysis")
    ...
    def end(self, success: bool = True, **result_context):
        ...
        if self.enable_analytics and hasattr(self, 'analytics'):
            try:
                ...
                test_name = result_context.get('test_name', self.context.get('test_name', 'unknown_test'))
                session_id = result_context.get('session_id', self.context.get('session_id', 'unknown_session'))
                ...
                try:
                    ...
                    if 'tablet' in device_name_lower or 'ipad' in device_name_lower or 'tab s8' in device_name_lower or 'galaxy tab' in device_name_lower:
                        device_type = "tablet"
                    elif 'desktop' in device_name_lower or 'chrome' in device_name_lower:
                        device_type = "desktop"
                    else:
                        device_type = "phone"
        ...
        self.logger.log(
            level,
            f"{emoji} {self.operation_name}: {duration_ms}ms",
            extra=log_data
        )
...
def log_test_start(test_name: str, **context):
    ...
    logger.info(f"🧪 Starting test: {test_name}", extra={'test_name': test_name, **context})
...
def log_element_action(action: str, locator: str, success: bool = True, duration_ms: int = 0, **context):
...
def log_session_info(session_id: str, action: str, **context):
    ...
    logger.info(f"🔄 Session {action}: {session_id}", extra={'session_id': session_id, **context}) 