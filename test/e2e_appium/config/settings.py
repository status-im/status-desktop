...
class TestConfig:
    ...
    device_name: str = field(default_factory=lambda: os.getenv('DEVICE_NAME', 'Galaxy Tab S8'))
    platform_name: str = field(default_factory=lambda: os.getenv('PLATFORM_NAME', 'android'))
    platform_version: str = field(default_factory=lambda: os.getenv('PLATFORM_VERSION', '14'))
    device_orientation: str = field(default_factory=lambda: os.getenv('DEVICE_ORIENTATION', 'landscape'))
    
    default_timeout: int = field(default_factory=lambda: int(os.getenv('DEFAULT_TIMEOUT', '30')))
    element_wait_timeout: int = field(default_factory=lambda: int(os.getenv('ELEMENT_WAIT_TIMEOUT', '30')))
    element_click_timeout: int = field(default_factory=lambda: int(os.getenv('ELEMENT_CLICK_TIMEOUT', '10')))
    element_find_timeout: int = field(default_factory=lambda: int(os.getenv('ELEMENT_FIND_TIMEOUT', '15')))
    
    status_app_url: str = field(default_factory=lambda: os.getenv('STATUS_APP_URL', 'lt://'))
    
    lt_hub_url: str = "https://mobile-hub.lambdatest.com/wd/hub"
    build_name: str = field(default_factory=lambda: os.getenv('BUILD_NAME', 'E2E_Appium Tests'))
    test_name: str = field(default_factory=lambda: os.getenv('TEST_NAME', 'Automated Test Run'))
    idle_timeout: int = 600
    
    log_level: str = field(default_factory=lambda: os.getenv('LOG_LEVEL', 'INFO'))
    enable_screenshots: bool = field(default_factory=lambda: os.getenv('ENABLE_SCREENSHOTS', 'true').lower() == 'true')
    enable_video_recording: bool = field(default_factory=lambda: os.getenv('ENABLE_VIDEO_RECORDING', 'true').lower() == 'true')
    ...
    enable_xml_report: bool = field(default_factory=lambda: os.getenv('ENABLE_XML_REPORT', 'true').lower() == 'true')
    enable_html_report: bool = field(default_factory=lambda: os.getenv('ENABLE_HTML_REPORT', 'true').lower() == 'true')
    enable_junit_report: bool = field(default_factory=lambda: os.getenv('ENABLE_JUNIT_REPORT', 'true').lower() == 'true')
    
    enable_performance_analytics: bool = field(default_factory=lambda: os.getenv('E2E_ENABLE_PERFORMANCE_ANALYTICS', 'false').lower() in ('true', '1', 'yes', 'on'))
    performance_report_days: int = field(default_factory=lambda: int(os.getenv('E2E_PERFORMANCE_REPORT_DAYS', '7')))
    ...
    local_appium_server: str = field(default_factory=lambda: os.getenv('LOCAL_APPIUM_SERVER', 'http://localhost:4723'))
    ...
    def _validate_required_fields(self):
        ...
        if test_environment in ['lambdatest', 'lt']:
            if not self.lt_username:
                errors.append("LT_USERNAME environment variable is required for LambdaTest execution")
            
            if not self.lt_access_key:
                errors.append("LT_ACCESS_KEY environment variable is required for LambdaTest execution")
            ...
            if not self.lt_username or not self.lt_access_key:
                warnings.append("LambdaTest credentials not set (OK for local development)")
    ...
    def _validate_urls(self):
        if self.status_app_url and not (self.status_app_url.startswith('lt://') or 
                                       self.status_app_url.startswith('http')):
            raise ValueError("STATUS_APP_URL must be a LambdaTest app ID (lt://) or valid URL")
    ...
    def summary(self) -> Dict[str, Any]:
        return {
            ...
            'lt_access_key': '***' + self.lt_access_key[-4:] if self.lt_access_key else 'NOT SET',
