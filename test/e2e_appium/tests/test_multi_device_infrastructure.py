...
class TestMultiDeviceInfrastructure(MultiDeviceTestBase):
    ...
    async def test_session_pool_creates_two_sessions(self, test_environment, request):
        ...
        try:
            ...
            async with self.async_device_step("device_0", "Validate driver_0 session"):
                assert driver_0 is not None, "driver_0 is None"

            async with self.async_device_step("device_1", "Validate driver_1 session"):
                assert driver_1 is not None, "driver_1 is None"

            async with self.async_device_step("device_0", "Check unique session ids"):
                session_id_0 = getattr(driver_0, "session_id", None)
                assert session_id_0 is not None, "driver_0 has no session_id"

            async with self.async_device_step("device_1", "Check unique session ids"):
    ...
    @pytest.mark.smoke
    @pytest.mark.asyncio
    @pytest.mark.xfail(reason="deliberate failure to test per-device reporting")
    async def test_onboarded_devices_fixture(self, onboarded_devices):
        """Test that the onboarded_devices fixture provisions ready-to-use contexts."""

        self.logger.info("Testing onboarded_devices fixture")

        assert onboarded_devices is not None, "onboarded_devices fixture should not be None"
        assert len(onboarded_devices) == 2, f"Expected 2 devices, got {len(onboarded_devices)}"

        device_1 = onboarded_devices["device_0"]
        device_2 = onboarded_devices["device_1"]
        ...
        async with self.async_device_step(device_1, "Validate onboarded user"):
            assert device_1.user is not None, "device_0 should have onboarded user"

        async with self.async_device_step(device_2, "Validate onboarded user"):
            assert device_2.user is not None, "device_1 should have onboarded user"

        async with self.async_device_step(device_1, "Ensure device display names are unique"):
            ...
            assert device_1.user.display_name == "00000", "device_0 display name should be '00000'"

        self.logger.info(
            "onboarded_devices fixture validated: sessions %s, %s",
            session_id_1,
            session_id_2,
        )
    ...
    @pytest.mark.xfail(reason="deliberate failure to test per-device reporting")
    @pytest.mark.no_autouse_onboarded
    @pytest.mark.asyncio
    async def test_onboarded_devices_helper(self, request, test_environment):
        async with self.onboarded_devices(
            request,
            test_environment,
            count=5,
        ) as devices:
            assert len(devices) == 5, "Expected 5 devices from helper"

            for name, ctx in devices.contexts.items():
                async with self.async_device_step(
                    ctx, f"Validate onboarded user for {name}"
                ):
                    assert ctx.user is not None, f"{name} should have onboarded user"

            # Deliberate failure test for device_0 only - to verify per-device reporting
            async with self.async_device_step(devices["device_0"], "Validate device_0 display name"):
                assert devices["device_0"].user.display_name == "00000", (
                    f"device_0 display name should be '00000' but got '{devices['device_0'].user.display_name}'"
                )
    ...
    async def test_session_pool_throttle_config_defaults(self):
        ...
        from core.session_pool import PoolConfig
    ...
    async def test_session_pool_queue_retry_logic(self):
        ...
        from core.session_pool import SessionPool, PoolConfig
        from utils.exceptions import SessionManagementError
