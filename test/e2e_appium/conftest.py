...
def pytest_configure(config):
    global _logging_setup, _capacity_manager, _capacity_payload, _counter_manager
...
def pytest_runtest_makereport(item, call):
    ...
    for session_managers, pool in stash_entries:
        # First: Cleanup all drivers (close sessions)
        try:
            try:
            ...
            if loop_running:
                ...
                try:
                    ...
                    logger.debug(
                        "Completed cleanup for pool on dedicated event loop"
                    )
                finally:
                    cleanup_loop.close()
            else:
                ...
                logger.debug("Completed cleanup for pool")
        except Exception as e:
            logger.warning("Failed to cleanup pool in hook: %s", e)

        # Then: Report status via REST API (after drivers are closed)
        # Use simple pass/fail based on test outcome
        ...
        for name, session_manager in session_managers.items():
            ...
            # Report via REST API only (drivers are already closed)
