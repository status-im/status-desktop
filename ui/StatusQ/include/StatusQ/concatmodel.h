...
class ConcatModel : public QAbstractListModel, public QQmlParserStatus
{
    ...
    Q_PROPERTY(QByteArray markerRoleName READ markerRoleName
...
public:
    ...
    void setMarkerRoleName(const QByteArray& markerRoleName);
    const QByteArray& markerRoleName() const;
...
private:
    ...
    QVector<int> translateRoles(int sourceIndex, const QVector<int>& roles) const;

    QList<SourceModel*> m_sources;
    QStringList m_expectedRoles;

    QByteArray m_markerRoleName = s_defaultMarkerRoleName;
    int m_markerRole = 0;
