...
Item {
    ...
    property bool keycardUsage: false
    ...
    Connections {
        ...
        onAccountImportError: {
            ...
            if(root.keycardUsage){
                importSeedError.text += qsTr("\nKeys are stored into the keycard.")
            }
    ...
    MessageDialog {
        ...
        onVisibleChanged: {
            if(root.keycardUsage && !visible) {
                root.keycardStore.getOutOfTheKeycardFlow()
            }
        }
    }
    ...
    Item {
        ...
        StatusBaseText {
            ...
            anchors.horizontalCenter: parent.horizontalCenter
            text: root.keycardUsage? qsTr("Import seed phrase") : qsTr("Enter seed phrase")
        }
            ...
            onClicked: {
                ...
                if (root.keycardUsage) {
                    if (Utils.isMnemonic(mnemonicString) &&
                        root.keycardStore.checkSeedPhrase(root.mnemonicInput.length, mnemonicString)) {
                        root.keycardStore.nextState();
                        root.mnemonicInput = [];
                    } else {
