...
QtObject {
    ...
    function sendMessage(chatId, event, text, replyMessageId, fileUrlsAndSources) {
        ...
        if (fileUrlsAndSources.length > 0) {
            const resizedImagesPaths = ImageUtils.resizeImages(fileUrlsAndSources)
            chatContentModule.inputAreaModule.sendImages(JSON.stringify(resizedImagesPaths), textMsg.trim(), replyMessageId)
            result = true
        } else {
            if (textMsg.trim() !== "") {
