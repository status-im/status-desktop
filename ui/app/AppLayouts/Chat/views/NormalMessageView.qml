...
Item {
    ...
    anchors.topMargin: root.messageStore.authorCurrentMsg !== root.messageStore.authorPrevMsg ? Style.current.smallPadding : 0
    ...
    property bool headerRepeatCondition: (root.messageStore.authorCurrentMsg !== root.messageStore.authorPrevMsg
                                         || root.messageStore.shouldRepeatHeader || dateGroupLbl.visible)
    ...
    Rectangle {
        ...
        id: chatBox
        color: {
            if (root.messageStore.isSticker) {
                return Style.current.background 
            }
            if (isImage) {
                return "transparent"
            }
            return root.messageStore.isCurrentUser ? Style.current.primary : Style.current.secondaryBackground
        }
        border.color: root.messageStore.isSticker ? Style.current.border : Style.current.transparent
        border.width: 1
        height: {
