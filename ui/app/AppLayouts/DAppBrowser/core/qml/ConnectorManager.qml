...
Item {
    ...
    // SIGNALS (events for Eip1193ProviderAdapter)
    ...
    function request(args) {
        ...
        console.log("[ConnectorManager] Request:", method, "requestId:", requestId)
        ...
        // Direct call to Nim connectorCallRPC
        if (connectorController) {
            console.log("[ConnectorManager] Calling connectorCallRPC with requestId:", requestId)
    ...
    function shouldProcessSignal(event) {
        // Filter by origin (normalize trailing slashes)
        ...
        // Filter by clientId if present in the event
    ...
    Connections {
        ...
        function onConnected(payload) {
            console.log("[ConnectorManager] connected signal received")
            try {
                ...
                if (data.chainId) {
                    const newChainId = data.chainId  // decimal from status-go
        ...
        function onDisconnected(payload) {
            console.log("[ConnectorManager] disconnected signal received")
            try {
                const data = JSON.parse(payload)
                if (!shouldProcessSignal(data)) return
                clearState()
        ...
        function onConnectorCallRPCResult(requestId, payload) {
            console.log("[ConnectorManager] connectorCallRPCResult received - requestId:", requestId)
        ...
        function onChainIdSwitched(payload) {
            console.log("[ConnectorManager] chainIdSwitched signal received")
            try {
                const data = JSON.parse(payload)
