    ...
    contentItem: Item {
        ...
        ColumnLayout {
            ...
            StatusBaseText {
                ...
                Layout.maximumWidth: browserConnectionModal.width - Theme.padding
            ...
            AccountSelector {
                ...
                selectedAddress: WalletStore.dappBrowserAccount.address
                onCurrentAccountAddressChanged: {
                    if (!browserConnectionModal.currentAddress) {
                        // We just set the account for the first time. Nothing to do here
                        browserConnectionModal.currentAddress = currentAccountAddress
                        return
                    }
                    if (browserConnectionModal.currentAddress === currentAccountAddress) {
                        return
                    }

                    browserConnectionModal.currentAddress = currentAccountAddress
                    Web3ProviderStore.web3ProviderInst.dappsAddress = currentAccountAddress;

                    if (currentAccountAddress) {
                        Web3ProviderStore.web3ProviderInst.dappsAddress = currentAccountAddress;
                        WalletStore.switchAccountByAddress(currentAccountAddress)
                    }
                }
            }
