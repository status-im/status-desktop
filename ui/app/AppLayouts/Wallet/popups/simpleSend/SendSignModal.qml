        ...
        function updateCustomFields() {
            ...
            if (!customBaseFeeDirty) {
                if (selectedFeeMode === root.selectedFeeMode) {
                    customBaseFee = !!root.customBaseFee? SQUtils.Utils.weiToGWei(root.customBaseFee).toFixed() : "0"
                } else {
                    customBaseFee = SQUtils.Utils.weiToGWei(root.normalBaseFee).toFixed()
                }
            ...
            if (!customPriorityFeeDirty) {
                if (selectedFeeMode === root.selectedFeeMode) {
                    customPriorityFee = !!root.customPriorityFee? SQUtils.Utils.weiToGWei(root.customPriorityFee).toFixed() : "0"
                } else {
                    customPriorityFee = SQUtils.Utils.weiToGWei(root.normalPriorityFee).toFixed()
                }
        ...
        onConfirmClicked: {
            ...
            if (selectedFeeMode === StatusFeeOption.Type.Custom) {
                if (!!customPriorityFee && !!customBaseFee) {
                    const baseFeeWei = SQUtils.Utils.gweiToWei(customBaseFee)
                    const priorityFeeWei = SQUtils.Utils.gweiToWei(customPriorityFee)
