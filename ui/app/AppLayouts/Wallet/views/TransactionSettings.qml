...
Rectangle {
    ...
    QtObject {
        ...
        function recalculateCustomPrice() {
            ...
            if (estimatedTime >= 60) {
                optionCustom.additionalText = qsTr(">60s")
                return
            }
            optionCustom.additionalText = estimatedTime
        }
    }
    ...
    ColumnLayout {
        ...
        ColumnLayout {
                ...
                ColumnLayout {
                    ...
                    StatusInput {
                        ...
                        readonly property bool displayTooLowAmountWarning: {
                            ...
                            const currentValue = SQUtils.AmountsArithmetic.fromString(root.currentGasAmount)
                            const decreasedCurrentValue = SQUtils.AmountsArithmetic.times(currentValue, SQUtils.AmountsArithmetic.fromString("0.9")) // up to -10% is acceptable
                            const enteredValue = SQUtils.AmountsArithmetic.fromString(customGasAmountInput.text)
                            customGasAmountInput.minAmount = decreasedCurrentValue.toFixed()
                            return decreasedCurrentValue.cmp(enteredValue) === 1
                        }
                        ...
                        readonly property bool displayTooHighAmountWarning: {
                            ...
                            const currentValue = SQUtils.AmountsArithmetic.fromString(root.currentGasAmount)
                            const increasedCurrentValue = SQUtils.AmountsArithmetic.times(currentValue, SQUtils.AmountsArithmetic.fromString("1.2")) // up to 20% higher value is acceptable
                            const enteredValue = SQUtils.AmountsArithmetic.fromString(customGasAmountInput.text)
                            customGasAmountInput.maxAmount = increasedCurrentValue.toFixed()
                            return enteredValue.cmp(increasedCurrentValue) === 1
                        }
                        ...
                        bottomLabelMessageLeftCmp.text: customGasAmountInput.displayTooLowAmountWarning?
                                                            qsTr("Too low (should be %1 or more)").arg(customGasAmountInput.minAmount)
                                                          : customGasAmountInput.displayTooHighAmountWarning?
                                                                qsTr("Too high (should be less than %1)").arg(customGasAmountInput.maxAmount)
            ...
            StatusButton {
                Layout.preferredWidth: parent.width
                enabled: !d.customMode || customBaseFeeInput.valid && customPriorityFeeInput.valid && customGasAmountInput.valid && customNonceInput.valid
                text: qsTr("Confirm")
