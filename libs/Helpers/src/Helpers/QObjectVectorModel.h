...
public:
    ...
    const T* at(size_t pos) const { return m_objects.at(pos).get(); };
    std::shared_ptr<T> get(size_t pos) { return m_objects.at(pos); };
    size_t size() const { return m_objects.size(); };
    void clear() { m_objects.clear(); };

    void push_back(std::shared_ptr<T> newValue) {
    ...
    void set(size_t row, std::shared_ptr<T> newVal) {
