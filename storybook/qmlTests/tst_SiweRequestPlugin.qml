...
Item {
    ...
    Component {
        ...
        SiweRequestPlugin {
            ...
            readonly property SignalSpy displayToastMessageSpy: SignalSpy { target: siwePlugin; signalName: "displayToastMessage" }
            ...
            sdk: SDKMock { }
            store: StoreMock { }
            accountsModel: AccountsModelMock { }
            networksModel: ChainsModelMock { }
        }
    }

    component SDKMock: WalletConnectSDKBase {
        id: sdk
        ...
        populateAuthPayload: function(id, payload, chains, methods) {
            sdk.populateAuthPayloadResult(id, {}, "")
        }

        formatAuthMessage: function(id, request, iss) {
            sdk.formatAuthMessageResult(id, {}, "")
        }

        acceptSessionAuthenticate: function(id, auths) {
            sdk.acceptSessionAuthenticateResult(id, {}, "")
        }

        rejectSessionAuthenticate: function(id, error) {
            sdk.rejectSessionAuthenticateResult(id, {}, "")
        }

        buildAuthObject: function(id, payload, signedData, account) {
            sdk.buildAuthObjectResult(id, {}, "")
        }
    }

    component StoreMock: DAppsStore {
    ...
    component ChainsModelMock: ListModel {
        ListElement { chainId: 1 }
        ListElement { chainId: 2 }
    }

    component AccountsModelMock: ListModel {
    ...
    TestCase {
        ...
        function test_ApproveRequestExpired() {
            ...
            tryCompare(componentUnderTest.siweFailedSpy, "count", 1)
            compare(componentUnderTest.displayToastMessageSpy.count, 1)
            compare(componentUnderTest.displayToastMessageSpy.signalArguments[0][0], "Request expired")
        }
